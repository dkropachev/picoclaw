...
func (replacement ValidatedReplacement) Use(
	...
	checkObserved := func(
		...
		if err != nil || objectType != fileidentity.ObjectTypeRegular || !retainedIdentity.Valid() {
	...
	func() {
		...
		useErr = use(operationCtx, stage, checkExact)
	}()
	stopScope()
	...
	return errors.Join(useErr, guardErr, digestErr)
}
...
func invokeStagedLiveVerification(
	...
	state := &validatedReplacementState{
		...
		stage: seal.path, stageIdentity: seal.identity, stageDigest: seal.digest,
		stageFile: seal.file, scope: scope,
	}
	defer func() {
...
func sealValidatedReplacementStage(
	...
	return &validatedReplacementSeal{
		path: path, identity: identity, file: file, digest: digest,
	}, nil
}
