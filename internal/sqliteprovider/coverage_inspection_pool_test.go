...
func TestCoverageInspectedPoolIdentityAndCleanup(t *testing.T) {
	...
	if _, err := retainInspectedPool(firstPath, nil, firstInfo, time.Second, owner1); err == nil {
	...
	if _, err := retainInspectedPool(firstPath, database, nil, time.Second, owner1); err == nil {
		t.Fatal("retained pool without identity")
	}
	if retained, err := retainInspectedPool(
		firstPath, database, firstInfo, time.Second, owner1,
	); err != nil || retained != database {
		t.Fatalf("retain = %p, %v", retained, err)
	}
	if retained, err := inspectedPoolFor(
		firstPath, firstInfo, time.Second, owner2,
	); err != nil || retained != database {
		t.Fatalf("lookup = %p, %v", retained, err)
	}
	if _, err := inspectedPoolFor(firstPath, secondInfo, time.Second, new(atomic.Bool)); err == nil {
	...
	if retained, err := retainInspectedPool(
		firstPath, duplicate, firstInfo, time.Second, owner3,
	); err != nil || retained != database {
	...
	if _, err := retainInspectedPool(
		firstPath, conflicting, firstInfo, 2*time.Second, new(atomic.Bool),
	); err == nil {
	...
	if _, err := retainInspectedPool(
		firstPath, changed, secondInfo, time.Second, new(atomic.Bool),
	); err == nil {
		t.Fatal("duplicate retain accepted changed identity")
	}
	if err := releaseInspectedPool(firstPath, changed, new(atomic.Bool)); err != nil {
		t.Fatalf("mismatched release = %v", err)
	}
	for _, owner := range []*atomic.Bool{owner1, owner2, owner3} {
		if err := releaseInspectedPool(firstPath, database, owner); err != nil {
			t.Fatal(err)
		}
	}
	if retained, err := inspectedPoolFor(
		firstPath, firstInfo, time.Second, new(atomic.Bool),
	); err != nil || retained != nil {
		t.Fatalf("post-close lookup = %p, %v", retained, err)
	}
	if adopted, err := adoptInspectedPool(
		secondPath, time.Second, new(atomic.Bool),
	); err != nil || adopted != nil {
...
func TestCoverageInspectionPoolAdoptionFailurePhases(t *testing.T) {
	...
	retain := func(t *testing.T, references int) (*sql.DB, []*atomic.Bool) {
		...
		generation, err := inspectedGenerationIdentity(path, info)
		if err != nil {
			t.Fatal(err)
		}
		inspectedPools.values[key] = inspectedPool{
			database: database, main: info, generation: generation,
	...
	for _, test := range []struct {
		...
		{name: "references", references: 2, timeout: time.Second,
			lstat:     func(string) (os.FileInfo, error) { return info, nil },
			directory: noopDirectory, secure: noopSecure},
		{name: "timeout", references: 1, timeout: 2 * time.Second,
			lstat:     func(string) (os.FileInfo, error) { return info, nil },
			directory: noopDirectory, secure: noopSecure},
		{name: "stat", references: 1, timeout: time.Second,
			lstat:     func(string) (os.FileInfo, error) { return nil, canary },
			directory: noopDirectory, secure: noopSecure},
		{name: "directory", references: 1, timeout: time.Second,
			lstat:     func(string) (os.FileInfo, error) { return info, nil },
			directory: func(string) error { return canary }, secure: noopSecure},
		{name: "generation", references: 1, timeout: time.Second,
			lstat:     func(string) (os.FileInfo, error) { return info, nil },
		...
		t.Run(test.name, func(t *testing.T) {
			...
			adopted, err := adoptInspectedPoolWithGeneration(
				path, test.timeout, owners[0], test.lstat, test.directory, test.secure,
			)
			...
			for _, owner := range owners {
				if err := releaseInspectedPool(path, database, owner); err != nil {
					t.Fatal(err)
				}
			}
		})
	...
	adopted, err := adoptInspectedPoolWithGeneration(
		path, time.Second, owners[0], func(string) (os.FileInfo, error) { return info, nil },
		noopDirectory, noopSecure,
	)
...
func TestCoverageInspectionPoolReleaseKeyFailure(t *testing.T) {
	...
	if err := releaseInspectedPoolWithKey(
		"store.db", nil, new(atomic.Bool), func(string) (string, error) { return "", canary },
	); !errors.Is(err, canary) {
		t.Fatalf("release key error = %v", err)
	}
}
...
func TestCoverageInspectionDatabasePhases(t *testing.T) {
	...
	if _, _, _, err := inspectDatabase(
		t.Context(), path, info, openConfiguredProviderScript(t, canary),
	...
	cancel()
	if _, _, _, err := inspectDatabase(
		canceled, path, info, openProviderScript(t),
	...
	if _, _, _, err := inspectDatabase(
		t.Context(), path, otherInfo, openProviderScript(t),
	); !IsInspectionIntegrity(err) {
		t.Fatalf("identity failure = %v", err)
	}
	if _, _, _, err := inspectDatabaseWithGeneration(
		t.Context(), path, info, openProviderScript(t),
	...
	if _, _, _, err := inspectDatabaseWithGeneration(
		t.Context(), path, info, openProviderScript(t, steps...),
		func(string) (os.FileInfo, error) { return info, nil },
		func(string) error { return nil }, busy,
	); !errors.Is(err, canary) {
	...
	for _, phase := range []struct {
		...
		t.Run("canceled "+phase.name, func(t *testing.T) {
			...
			cancel()
			if _, _, _, err := inspectDatabaseWithGeneration(
				ctx, path, info, openProviderScript(t, phase.steps...),
				func(string) (os.FileInfo, error) { return info, nil },
	...
	if _, _, _, err := inspectDatabase(t.Context(), path, info, openProviderScript(t,
	...
	if err != nil || current == nil || version != 4 || objects != 2 {
