...
func TestStagedMigrationRevalidatesAfterDomainValidation(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "schema version",
			mutate: func(ctx context.Context, stage string) error {
				database, err := openOfflineStage(ctx, stage, time.Second)
				if err != nil {
					return err
				}
				return errors.Join(SetSchemaVersion(ctx, database, 2), database.Close())
			},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			err = migrateStagedFixture(
				...
				installStagedFixtureTable, test.mutate,
			)
			if err == nil || database.CodeOf(err) == database.CodeOutcomeUnknown {
				t.Fatalf("post-validation mutation error = %v", err)
			}
			...
			if !bytes.Equal(before, after) {
				t.Fatal("post-validation mutation changed live target")
			}
...
func TestStagedMigrationPostReplacementFailuresAreUnknown(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if database.CodeOf(err) != database.CodeOutcomeUnknown || !errors.Is(err, injected) {
