...
func TestCoverageDiagnosticBusyClassification(t *testing.T) {
	canary := errors.New("busy canary")
	if err := controlDiagnosticErrorWithClassifier(
		t.Context(), canary, errIntegrityCheck, func(error) bool { return true },
	); !errors.Is(err, canary) {
		t.Fatalf("busy diagnostic error = %v", err)
	}
}
...
func TestCoverageProviderPreparationConfigurationAndMaintenanceEdges(t *testing.T) {
	...
	if _, err := maintainOfflineFixture(t, t.Context(), missing, 100*time.Millisecond); err != nil {
	...
	if _, err := maintainOfflineFixture(t, t.Context(), corrupt, 100*time.Millisecond); !IsMaintenanceIntegrity(err) {
