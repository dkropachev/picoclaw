...
func newMatrixHarness(t *testing.T) *matrixHarness {
	...
	return &matrixHarness{home: home, client: client, store: store, storeID: storeID}
}
...
func TestTypedStoreAndHandlerRejectInvalidAuthority(t *testing.T) {
	...
	defer restore()
	if handler, err := sqliteadapter.NewBrokerHandler(t.TempDir(), &config.Config{}); handler != nil ||
		database.CodeOf(err) != database.CodeUnauthorized {
		t.Fatalf("unauthorized sqliteadapter.NewBrokerHandler() = %#v, %v", handler, err)
	}
}
