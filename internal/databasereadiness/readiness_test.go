...
func TestProbePoolCanBeAdoptedAndSurvivesSnapshotClose(t *testing.T) {
	...
	db, err := snapshot.Adopt("global/auth", inspectionTimeout)
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
...
func TestProbePoolCannotBeAdoptedAfterClaimAuthorityExpires(t *testing.T) {
	...
	if database, err := snapshot.Adopt("global/auth", inspectionTimeout); err == nil || database != nil {
		if database != nil {
			_ = database.Close()
		}
...
func TestSnapshotAdoptAndCloseAreOneShotAndRaceSafe(t *testing.T) {
	...
	go func() {
		<-start
		db, adoptErr := snapshot.Adopt("global/auth", inspectionTimeout)
...
func acquireReadinessLease(t *testing.T, home string) (*databaseclaims.Lease, *database.Fence) {
	...
	lease, err := databaseclaims.Acquire(storecatalog.Options{Home: home, Config: &config.Config{}}, fence)
	if err != nil {
		t.Fatal(err)
	}
...
func createReadinessDatabase(
	...
	if _, err := db.Exec("CREATE TABLE storage_import_horizons (component TEXT PRIMARY KEY)"); err != nil {
		t.Fatal(err)
	}
	if horizon {
		if _, err := db.Exec("INSERT INTO storage_import_horizons(component) VALUES ('auth')"); err != nil {
			t.Fatal(err)
		}
	}
