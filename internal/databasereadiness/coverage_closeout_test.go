...
func TestSnapshotBoundariesAndDetachedStatuses(t *testing.T) {
	...
	invalid := &Snapshot{statuses: []database.StoreStatus{{ID: "bad id", Readiness: database.StoreReady}}}
	if invalid.Statuses() != nil {
		t.Fatal("invalid snapshot exposed statuses")
	}
	valid := &Snapshot{statuses: []database.StoreStatus{{ID: "global/auth", Readiness: database.StoreReady}}}
...
func TestGenerationExclusionAndLegacyAliasBoundaries(t *testing.T) {
	...
	if excluded, err := excludedLegacyFile(alias, aliasInfo, exclusions); excluded ||
		!errors.Is(err, errLegacyIntegrity) {
		t.Fatalf("hardlinked legacy exclusion = %t, %v", excluded, err)
	}
	if excluded, err := excludedLegacyFile(
		generation, info, generationSet{paths: map[string]struct{}{generationPathKey(generation): {}}},
	); err != nil || !excluded {
		t.Fatalf("exact generation exclusion = %t, %v", excluded, err)
	}
	if excluded, err := excludedLegacyFile(alias, aliasInfo, generationSet{}); err != nil || excluded {
...
func TestLegacyDiscoveryHelperBoundaries(t *testing.T) {
	...
	if found, err := walkLegacyDirectory(t.Context(), root, nil, 0, generationSet{}, budget); found ||
		!errors.Is(err, errLegacyIntegrity) {
		t.Fatalf("nil expected directory = %t, %v", found, err)
	}
	...
	if found, err := walkLegacyDirectory(t.Context(), root, fileInfo, 0, generationSet{}, budget); found ||
		!errors.Is(err, errLegacyIntegrity) {
		t.Fatalf("file expected as directory = %t, %v", found, err)
	}
	other := t.TempDir()
	if found, err := walkLegacyDirectory(t.Context(), root, mustReadinessInfo(t, other), 0, generationSet{}, budget); found ||
		!errors.Is(err, errLegacyIntegrity) {
		t.Fatalf("changed directory identity = %t, %v", found, err)
	}
	...
	if found, err := walkLegacyDirectory(
		t.Context(), missingReadinessPath(root), mustReadinessInfo(t, other), 0, generationSet{}, freshBudget,
	); found || !errors.Is(err, errLegacyIntegrity) {
	...
	cancelWalk()
	if found, err := walkLegacyDirectory(
		canceledWalk, root, info, 0, generationSet{}, freshBudget,
	...
	skippedInfo := mustReadinessInfo(t, skippedRoot)
	...
	if found, err := walkLegacyDirectory(
		t.Context(), skippedRoot, skippedInfo, 0, generationSet{}, skippedBudget,
	); found || !errors.Is(err, errLegacyIntegrity) {
	...
	nestedInfo := mustReadinessInfo(t, nestedRoot)
	nestedBudget, err := newLegacyDiscoveryBudget(defaultLegacyDiscoveryLimits())
	if err != nil {
		t.Fatal(err)
	}
	if found, err := walkLegacyDirectory(
		t.Context(), nestedRoot, nestedInfo, 0, generationSet{}, nestedBudget,
	); err != nil || !found {
	...
	if found, err := walkLegacyDirectory(
		loopCanceled, nestedRoot, nestedInfo, 0, generationSet{}, loopBudget,
	); found || !errors.Is(err, context.Canceled) {
		t.Fatalf("directory-entry context cancellation = %t, %v", found, err)
	}
	renamedRoot := t.TempDir()
	renamedInfo := mustReadinessInfo(t, renamedRoot)
	...
	if found, err := walkLegacyDirectory(
		actionContext, renamedRoot, renamedInfo, 0, generationSet{}, renamedBudget,
	); found || !errors.Is(err, errLegacyIntegrity) {
...
func mustReadinessInfo(t *testing.T, path string) os.FileInfo {
	t.Helper()
