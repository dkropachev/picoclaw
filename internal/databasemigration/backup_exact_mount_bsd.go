...
func openExactBackupChild(root *os.Root, relative string) (*os.File, error) {
	return openExactBackupChildByMountIdentity(root, relative, bsdExactBackupMountIdentity)
}

func bsdExactBackupMountIdentity(file *os.File) (exactBackupMountIdentity, error) {
	if file == nil {
	...
	return exactBackupMountIdentity{
		mechanism: "bsd-fstatfs",
		value: [2]uint64{
			uint64(uint32(stat.Fsid.Val[0])), uint64(uint32(stat.Fsid.Val[1])),
		},
...
func validateExactBackupOpenedMount(root, child *os.File) error {
	rootIdentity, rootErr := bsdExactBackupMountIdentity(root)
	childIdentity, childErr := bsdExactBackupMountIdentity(child)
