...
func TestCatalogFingerprintHasExactStableLowercaseEncoding(t *testing.T) {
	...
	wantOrder := []database.StoreID{catalog.specs[0].ID, catalog.specs[1].ID}
	...
	if got := []database.StoreID{catalog.specs[0].ID, catalog.specs[1].ID}; !slices.Equal(got, wantOrder) {
...
func TestCatalogFingerprintLengthFramesAmbiguousValues(t *testing.T) {
	...
	if first.specs[0].ID.String()+first.specs[0].Domain !=
		second.specs[0].ID.String()+second.specs[0].Domain {
