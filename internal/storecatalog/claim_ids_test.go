...
func TestClaimIDValidationAndInvalidCatalogBoundaries(t *testing.T) {
	...
	validSpec := Spec{ID: "global/auth", Domain: "auth", Path: filepath.Join(t.TempDir(), "auth.db")}
	for _, catalog := range []*Catalog{
		nil,
		{},
		{home: validSpec.Path},
		{home: validSpec.Path, specs: []Spec{{ID: "bad id", Path: validSpec.Path}}},
		{home: validSpec.Path, specs: []Spec{{ID: "global/auth"}}},
		{home: validSpec.Path, specs: []Spec{{ID: "global/auth", Path: validSpec.Path, LegacyRoots: []string{""}}}},
	} {
