...
func TestCatalogRequiredChannelsMatchRuntimeActivation(t *testing.T) {
	...
	for id, required := range map[string]bool{
		"channel/wecom":  true,
		"channel/weixin": true,
...
func mustChannelID(t *testing.T, channelType, name string) string {
	t.Helper()
...
func TestChannelIdentitiesPreserveExactNamesAndDoNotMutateConfig(t *testing.T) {
	...
	if suffix := upper[strings.LastIndexByte(upper, '-')+1:]; len(suffix) != 16 {
