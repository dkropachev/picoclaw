...
func TestBuildCataloguesDynamicRuntimeDomainsPerConfiguredWorkspace(t *testing.T) {
	...
	wanted := map[database.StoreID]struct {
		path     string
		required bool
	}{
		...
		database.StoreID(agentPrefix + "/workflows"): {filepath.Join(agent, "state", "workflows.db"), true},
		database.StoreID(agentPrefix + "/eventing"):  {filepath.Join(agent, "eventing", "events.db"), true},
		database.StoreID(agentPrefix + "/cron"):      {filepath.Join(agent, "cron", "jobs.db"), true},
	}
