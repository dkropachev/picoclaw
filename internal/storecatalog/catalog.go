...
func Build(input any, configurations ...*config.Config) (*Catalog, error) {
	options, err := normalizeCatalogOptions(input, configurations)
	if err != nil {
		return nil, err
	}
...
func Project(input any, configurations ...*config.Config) (*Catalog, error) {
	options, err := normalizeCatalogOptions(input, configurations)
	if err != nil {
		return nil, err
	}
...
// normalizeCatalogOptions preserves the active broker's legacy composition
// entrypoint while keeping the provider-catalog facade's explicit Options path
// free of ambient filesystem context.
func normalizeCatalogOptions(input any, configurations []*config.Config) (Options, error) {
	switch value := input.(type) {
	case Options:
		if len(configurations) != 0 {
			return Options{}, errors.New("database catalog options are invalid")
		}
		return value, nil
	case string:
		if len(configurations) != 1 {
			return Options{}, errors.New("database catalog configuration is required")
		}
		...
		if configPath != "" && !filepath.IsAbs(configPath) {
			...
			if err != nil {
				return Options{}, fmt.Errorf("database catalog config path: %w", err)
			}
			configPath = absolute
		}
		userHome, _ := os.UserHomeDir()
		return Options{
			Home: value, Config: configuration, ConfigPath: configPath, UserHome: userHome,
			allowMissingHome: true,
		}, nil
	default:
		return Options{}, errors.New("database catalog options are invalid")
	}
...
func build(options Options, inspectGenerations bool) (*Catalog, error) {
	...
	homeInfo, err := os.Lstat(canonicalHome)
	missingProjectedHome := errors.Is(err, os.ErrNotExist) && options.allowMissingHome && !inspectGenerations
	if !missingProjectedHome && (err != nil || !homeInfo.IsDir() || homeInfo.Mode()&os.ModeSymlink != 0) {
		return nil, errors.New("database catalog home must be an existing real directory")
	}
	...
	for _, name := range channelNames {
		...
		case config.ChannelMatrix:
			...
			add(Spec{
				ID:     "channel/matrix/" + logicalComponent(name),
		...
		case config.ChannelWhatsAppNative:
			...
			add(Spec{
				ID:     "channel/whatsapp/" + logicalComponent(name),
...
func ChannelStoreID(channelType, name string) (string, bool) {
	...
	id := prefix + logicalComponent(name)
	return id, validStoreID(id)
}
