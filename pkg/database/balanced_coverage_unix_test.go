...
func TestRealFilesystemFailureBoundaries(t *testing.T) {
	...
	if file, err := createOwnerOnlyExclusiveFile(existing, 0o600); err == nil || file != nil {
		t.Fatalf("exclusive existing file = %#v, %v", file, err)
	}
...
func TestRealUnixFileLockFailureBoundaries(t *testing.T) {
	...
	if err := (&PhysicalStoreClaims{files: []*os.File{closed}}).Close(); err == nil {
		t.Fatal("closing physical claims around a closed descriptor succeeded")
	}
...
func TestRealAuthorityAndArtifactFailureBoundaries(t *testing.T) {
	...
	if _, err := artifacts.New("bad\x00home", nil); err == nil {
		t.Fatal("artifact catalog accepted an invalid home")
	}
	catalog, err := artifacts.New(home, config.DefaultConfig())
	if err != nil {
		t.Fatal(err)
	}
	if roots := catalog.ProtectedRootsForDomains(); roots != nil {
		t.Fatalf("empty domain roots = %#v", roots)
	}
	var nilCatalog *artifacts.Catalog
	if roots := nilCatalog.ProtectedRoots(); roots != nil {
		t.Fatalf("nil protected roots = %#v", roots)
	}
	if roots := nilCatalog.ProtectedRootsForDomains("auth"); roots != nil {
		t.Fatalf("nil domain roots = %#v", roots)
	}
...
func TestPhysicalClaimRootRequiresResolvableUserCache(t *testing.T) {
	if claims, err := AcquireCatalogStoreClaims(" invalid ", &config.Config{}); err == nil || claims != nil {
		t.Fatalf("physical claims with invalid catalog = %#v, %v", claims, err)
	}
	t.Setenv("XDG_CACHE_HOME", "")
	t.Setenv("HOME", "")
	if _, err := preparePhysicalClaimRoot(); CodeOf(err) != CodeUnavailable {
		t.Fatalf("home-less physical claim root = %v", err)
	}
	if claims, err := AcquireCatalogStoreClaims(
		t.TempDir(),
		&config.Config{},
	); CodeOf(err) != CodeUnavailable ||
		claims != nil {
		t.Fatalf("physical claims without cache root = %#v, %v", claims, err)
	}
}
...
func TestClientRealStateAndCancellationBoundaries(t *testing.T) {
	...
	requestContext, cancel := context.WithTimeout(t.Context(), 20*time.Millisecond)
	defer cancel()
	if err := client.Call(requestContext, "domain", 1, "wait", EmptyPayload{}, nil); err == nil ||
