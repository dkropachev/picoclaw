...
func lockWeixinStateDatabase(databasePath string) (func(), error) {
	if strings.TrimSpace(databasePath) == "" {
		return nil, errors.New("Weixin state database path is required")
	}
	...
	return lockWeixinStateFile(filepath.Join(lockDirectory, "store"))
}

func openWeixinStateLockFile(path string) (*os.File, error) {
	lockPath := path + ".lock"
	if info, err := os.Lstat(lockPath); err == nil {
		if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 {
			return nil, errors.New("Weixin state lock must be a regular file")
		}
	...
	if statErr != nil || lstatErr != nil || openedInfo == nil || pathInfo == nil ||
		...
		return nil, errors.Join(
			errors.New("Weixin state lock changed while opening"),
			statErr,
			lstatErr,
		)
	}
