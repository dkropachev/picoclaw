...
func classifySandboxExit(err error, output []byte) (Status, int) {
	...
	if sandboxInfrastructureOutput(output) {
		return StatusInfrastructureError, exitCode
	}
	return StatusFailed, exitCode
}

func sandboxInfrastructureOutput(output []byte) bool {
	value := strings.ToLower(strings.ToValidUTF8(string(output), "�"))
	...
	dependencyFetch := strings.Contains(value, "go: downloading ") ||
		strings.Contains(value, "npm err! network") ||
		strings.Contains(value, "pip") && strings.Contains(value, "download")
	return dependencyFetch && (strings.Contains(value, "dial tcp:") ||
		strings.Contains(value, "lookup ") || strings.Contains(value, "connection refused"))
}
