...
func TestRenderSVGUsesNormalizedGraphAndClickableGates(t *testing.T) {
	graph := Graph{
		...
		Flows: []Flow{{
			...
			Nodes: []Node{
				{
					ID:          "github_review_requested",
					Kind:        NodeAction,
					Title:       "GitHub review request",
					Description: "GitHub assigns the pull request.",
				...
				},
				{
					...
					Description:   "Decides whether review can begin.",
				...
				},
				{
					...
					Description: "Blocks code outside the approved charter.",
					Safeguard:   "pr.scope.hard",
				},
			},
			Edges: []Edge{
				{
					From:    "github_review_requested",
					To:      "allow_review",
					Outcome: "allow",
					Label:   "Allow",
					Mode:    EdgeChoice,
	...
	if !strings.Contains(text, `/development/workflow-configurations/default/edit?flow=review&amp;gate=`) {
		t.Fatal("editable gate URL is not XML-safe")
	}
	if strings.Contains(text, `/development/workflow-configurations?config=`) {
		t.Fatal("SVG retains the removed query-addressed workflow configuration route")
	}
}
