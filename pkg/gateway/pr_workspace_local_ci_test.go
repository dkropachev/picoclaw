...
func TestNewPRWorkspaceLocalCIRuntimeUsesPrivateEventState(t *testing.T) {
	...
	if err != nil {
		...
		t.Fatalf("newPRWorkspaceLocalCIRuntime() error = %v", err)
	}
	t.Cleanup(func() {
	...
	root := filepath.Join(workspace, "state", prWorkspaceLocalCIDirectory)
...
func TestSemanticChangedLinesCountsSourceContentBeginningWithTriplePrefix(t *testing.T) {
	diff := "diff --git a/pkg/counter.go b/pkg/counter.go\n" +
		"--- a/pkg/counter.go\n+++ b/pkg/counter.go\n" +
		"@@ -1 +1 @@\n---flag\n+++counter\n"
	if got := semanticChangedLines(diff); got != 2 {
		t.Fatalf("semanticChangedLines() = %d, want 2", got)
	}
}

func TestNewPRWorkspaceLocalCIEvidenceRuntimeNeedsNoSandbox(t *testing.T) {
	workspace := t.TempDir()
	cfg := &config.Config{}
	cfg.Agents.Defaults.Workspace = workspace
	cfg.Events.Ingress.Enabled = true
	...
	ciRuntime, err := newPRWorkspaceLocalCIEvidenceRuntime(cfg)
	if err != nil {
		t.Fatalf("newPRWorkspaceLocalCIEvidenceRuntime() error = %v", err)
	}
	t.Cleanup(func() {
		if closeErr := ciRuntime.Close(); closeErr != nil {
			t.Errorf("Close() error = %v", closeErr)
...
func TestEnsurePrivatePRWorkspaceDirectoryRejectsSymlink(t *testing.T) {
	root := t.TempDir()
	...
	if err := ensurePrivatePRWorkspaceDirectory(link); err == nil {
		t.Fatal("ensurePrivatePRWorkspaceDirectory(symlink) error = nil")
	}
}
