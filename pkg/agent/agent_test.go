...
func (f *fakeMediaChannel) mediaSnapshot() []bus.OutboundMediaMessage {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func TestProcessMessage_SuppressesReasoningWhenThinkingOff(t *testing.T) {
	...
	msgBus := bus.NewMessageBus()
	al := newTestAgentLoopWithStrictModels(cfg, msgBus, &reasoningResponseProvider{})
...
func TestRecordLastChannel(t *testing.T) {
	...
	al2 := newTestAgentLoopWithStrictModels(cfg, msgBus, provider)
...
func TestProcessMessage_MediaToolHandledSkipsFollowUpLLMAndFinalText(t *testing.T) {
	...
	sentMedia := telegramChannel.mediaSnapshot()
	if len(sentMedia) != 1 {
...
func TestProcessMessage_HandledToolProcessesQueuedSteeringBeforeReturning(t *testing.T) {
	...
	if sentMedia := telegramChannel.mediaSnapshot(); len(sentMedia) != 1 {
		t.Fatalf("expected exactly 1 synchronously sent media message, got %d", len(sentMedia))
	}
}
...
func TestRunAgentLoop_ResponseHandledToolPublishesForUserWhenSendResponseDisabled(t *testing.T) {
	...
	for len(telegramChannel.messagesSnapshot()) == 0 && time.Now().Before(deadline) {
	...
	if sentMessages[0].Context.TopicID != "42" {
		t.Fatalf("sent text topic_id = %q, want 42", sentMessages[0].Context.TopicID)
	}
	...
	if sentMessages[0].Scope == nil ||
		sentMessages[0].Scope.Values["chat"] != "forum:chat1/42" {
...
func TestResolveMessageRoute_UsesInboundContextAccount(t *testing.T) {
	...
	msgBus := bus.NewMessageBus()
	al := newTestAgentLoopWithStrictModels(cfg, msgBus, &simpleMockProvider{response: "ok"})
...
func TestProcessMessage_ClearCommandClearsRoutedAgentSession(t *testing.T) {
	...
	al := newTestAgentLoopWithStrictModels(cfg, bus.NewMessageBus(), &countingMockProvider{response: "LLM reply"})
...
func TestProcessMessage_SwitchModelShowModelConsistency(t *testing.T) {
	...
	if !strings.Contains(showResp, "Current Model: deepseek (Provider: openai)") {
...
func TestProcessMessage_SwitchModelRejectsUnknownAlias(t *testing.T) {
	...
	if switchResp != `model alias "missing" is not configured` {
...
func TestProcessMessage_SwitchModelRoutesSubsequentRequestsToSelectedProvider(t *testing.T) {
	...
	cfg := &config.Config{
		...
		ModelAliases: []config.ModelAliasConfig{
			{Name: "local", Model: "Qwen3.5-35B-A3B"},
			{Name: "deepseek", Model: "deepseek-v3.2"},
		},
	}

	msgBus := bus.NewMessageBus()
	provider, _, err := createStrictTestProvider(cfg)
	if err != nil {
	...
	if secondResp != "local reply" {
		t.Fatalf("unexpected response after switch: %q", secondResp)
	}
	if localCalls != 2 {
		t.Fatalf("local calls after switch = %d, want 2", localCalls)
	}
	if localModel != "deepseek-v3.2" {
		t.Fatalf(
			"model after switch = %q, want %q",
			localModel,
			"deepseek-v3.2",
		)
	}
...
func TestProcessMessage_FallbackUsesNextAliasOnSelectedAccount(t *testing.T) {
	workspace := t.TempDir()
...
func TestAgentLoop_UserAttachmentRoutesToImageModelAfterMediaResolution(t *testing.T) {
	...
	al := newTestAgentLoopWithStrictModels(cfg, msgBus, textProvider)
	...
	bindBootstrapProvider(agent.CandidateProviders, agent.ImageCandidates[0], visionProvider)
...
func TestAgentLoop_LoadImageFollowUpRoutesToImageModel(t *testing.T) {
	...
	al := newTestAgentLoopWithStrictModels(cfg, msgBus, planner)
	al.SetMediaStore(media.NewFileMediaStore())
...
func TestHandleReasoning(t *testing.T) {
	newLoop := func(t *testing.T) (*AgentLoop, *bus.MessageBus) {
		...
		msgBus := bus.NewMessageBus()
		return newTestAgentLoopWithStrictModels(cfg, msgBus, &mockProvider{}), msgBus
	}
...
func TestParallelMessageProcessing_SameSessionProcessedSequentially(t *testing.T) {
	...
	al := newTestAgentLoopWithStrictModels(cfg, msgBus, &concurrentMockProvider{
		responseFunc: func(callID int) string {
