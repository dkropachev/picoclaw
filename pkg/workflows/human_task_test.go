...
func TestHumanTaskSuspendsAndResumesExactPersistedWorkflow(t *testing.T) {
	...
	if err := registry.Register(
		...
		func(_ context.Context, args map[string]any, _ ExecutionContext) (map[string]any, error) {
			afterCalls.Add(1)
			receivedSecret.Store(args["token"])
			return map[string]any{"answer": args["answer"], "source": args["source"]}, nil
		},
	); err != nil {
		t.Fatal(err)
	}
	if err := registry.Register(
		"changed",
		func(_ context.Context, _ map[string]any, _ ExecutionContext) (map[string]any, error) {
			t.Fatal("resume executed the changed published workflow instead of the persisted snapshot")
			return nil, nil
		},
	); err != nil {
		t.Fatal(err)
...
jobs:
  main:
    runs-on: picoclaw
    outputs:
      answer: ${{ steps.after.outputs.answer }}
      source: ${{ steps.after.outputs.source }}
    steps:
      - id: before
        uses: function/before
      - id: approve
        uses: human/task
        with:
          ...
          questions:
            - id: approval
              prompt: Approve ${{ steps.before.outputs.value }}?
      ...
      - id: after
        uses: function/after
        with:
          answer: ${{ steps.approve.outputs.response.approved }}
          source: ${{ steps.before.outputs.value }}
...
`)
	...
	started, err := executor.Run(ctx, RunRequest{
		Workflow: workflow, WorkflowRef: "workflows/human.yml",
	})
	if err != nil {
	...
	if beforeCalls.Load() != 1 || afterCalls.Load() != 0 {
		t.Fatalf("calls before=%d after=%d, want 1/0", beforeCalls.Load(), afterCalls.Load())
	}
	...
	task := tasks[0]
	if task.Status != HumanTaskStatusWaiting || task.InputHash != "snapshot-1" ||
		!strings.Contains(canonicalJSON(task.Questions), "original") {
		t.Fatalf("task = %#v", task)
	}

	// Simulate a process restart and a changed published definition. Resume
	// must use the private snapshot persisted with the original run.
	changed := strings.ReplaceAll(mustWorkflowJSON(t, workflow), "function/after", "function/changed")
	if mkdirErr := os.MkdirAll(filepath.Join(workspace, "workflows"), 0o755); mkdirErr != nil {
		t.Fatal(mkdirErr)
	}
	if writeErr := os.WriteFile(
		filepath.Join(workspace, "workflows", "human.yml"),
		[]byte(changed),
		0o600,
	); writeErr != nil {
		t.Fatal(writeErr)
	}
	restarted := &Executor{WorkspaceDir: workspace, Store: NewFileRunStore(workspace), Functions: registry}
	secret := "resume-secret-must-not-be-persisted"
	resumed, err := restarted.ResumeHumanTask(ctx, started.RunID, task.ID, HumanTaskResumeRequest{
		ExpectedRevision: task.Revision,
		InputHash:        task.InputHash,
		ResponseID:       "response-1",
		Response:         map[string]any{"approved": true},
		Secrets:          map[string]string{"resume_token": secret},
	})
	if err != nil {
	...
	if beforeCalls.Load() != 1 || afterCalls.Load() != 1 {
		t.Fatalf("calls before=%d after=%d, want 1/1", beforeCalls.Load(), afterCalls.Load())
	}
	if receivedSecret.Load() != secret {
		t.Fatalf("downstream step received secret %#v, want supplied resume secret", receivedSecret.Load())
	}
	persisted, err := store.GetRun(ctx, started.RunID)
	if err != nil {
		t.Fatal(err)
	}
	if persisted.Jobs["main"].Outputs["answer"] != true ||
		persisted.Jobs["main"].Outputs["source"] != "original" {
		t.Fatalf("persisted job outputs = %#v", persisted.Jobs["main"].Outputs)
	}
	publicJSON, err := json.Marshal(persisted)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHumanTaskReclaimedContinuationRehydratesPersistedSteps(t *testing.T) {
	ctx := context.Background()
	workspace := t.TempDir()
	store := NewFileRunStore(workspace)
	registry := NewFunctionRegistry()
	...
	if err := registry.Register(
		...
		func(_ context.Context, _ map[string]any, _ ExecutionContext) (map[string]any, error) {
			afterCalls.Add(1)
			return map[string]any{"value": "repeated"}, nil
		},
	); err != nil {
		t.Fatal(err)
	}
	workflow := parseWorkflow(t, `
name: Rehydrate reclaimed continuation
on: {manual: {}}
jobs:
  main:
    runs-on: picoclaw
    outputs:
      value: ${{ steps.after.outputs.value }}
    steps:
      - id: wait
        uses: human/task
        with: {title: Continue, questions: [{id: continue}]}
...
`)
	executor := &Executor{WorkspaceDir: workspace, Store: store, Functions: registry}
	started, err := executor.Run(ctx, RunRequest{
		Workflow: workflow, WorkflowRef: "workflows/rehydrate.yml",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	claimed, _, _, err := store.ClaimHumanTask(ctx, started.RunID, task.ID, request)
	if err != nil {
		t.Fatal(err)
	}
	claimed.Steps["main/after"] = StepExecution{
		ID:      "after",
		Status:  RunStatusSucceeded,
		Outputs: map[string]any{"value": "persisted"},
	}
	if updateErr := store.UpdateRun(ctx, claimed); updateErr != nil {
		t.Fatal(updateErr)
	}
	expirePersistedHumanTaskClaim(t, workspace, store, started.RunID)
	resumed, err := executor.ResumeHumanTask(ctx, started.RunID, task.ID, request)
	if err != nil || resumed.Status != RunStatusSucceeded {
		t.Fatalf("ResumeHumanTask() result=%#v err=%v", resumed, err)
	}
	if afterCalls.Load() != 0 {
		t.Fatalf("persisted continuation step repeated %d times", afterCalls.Load())
	}
	persisted, err := store.GetRun(ctx, started.RunID)
	if err != nil {
		t.Fatal(err)
	}
	if persisted.Jobs["main"].Outputs["value"] != "persisted" {
		t.Fatalf("rehydrated outputs = %#v", persisted.Jobs["main"].Outputs)
	}
}
...
func mustWorkflowJSON(t *testing.T, workflow *Workflow) string {
	t.Helper()
