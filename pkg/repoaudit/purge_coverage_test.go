...
func TestRepositoryReviewPurgeRequestBoundaryCoverage(t *testing.T) {
	...
	cancel()
	_, _, canceledErr := store.PurgeAutomationHistory(
		canceled,
		"rra_missing",
		1,
		0,
	...
	if !errors.Is(canceledErr, context.Canceled) {
		t.Fatalf("canceled purge error = %v", canceledErr)
	}
	if _, _, err := store.PurgeAutomationHistory(context.Background(), "bad", 0, -1, purgeTestFence(), ""); !errors.Is(
		err,
...
func TestRepositoryReviewPurgeRecoveryBoundaryCoverage(t *testing.T) {
	...
	if _, _, err := store.loadPurgeIntentForAutomation("bad"); !errors.Is(
		err,
		ErrInvalidAutomation,
	) {
	...
	if _, found, err := store.loadPurgeIntentForAutomation("rra_absent_marker"); err != nil ||
		found {
...
func TestRepositoryReviewPurgePhaseVerificationCoverage(t *testing.T) {
	...
	if _, _, _, err := store.validatePreparedPurgeIntent(emptyIntent); !errors.Is(
		err,
		ErrConflict,
	) {
	...
	wrongVersion.UpdatedAt = automationTestNow
	purgeTestOverwriteAutomation(t, store, wrongVersion)
...
func TestRepositoryReviewPurgeHelperErrorCoverage(t *testing.T) {
	...
	t.Run("remove assignment mismatch", func(t *testing.T) {
		...
		if err := fixture.store.applyPurgeAutomationPhase(fixture.intent); !errors.Is(
			err,
			ErrConflict,
		) {
	...
	t.Run("reset assignment missing", func(t *testing.T) {
		fixture := newPurgePhaseFixture(t, repositoryReviewPurgeAutomationCommitting, false)
		purgeTestDeleteAutomation(t, fixture.store, fixture.automation.ID)
		if err := fixture.store.applyPurgeAutomationPhase(fixture.intent); !errors.Is(
			err,
			ErrConflict,
		) {
	...
	t.Run("run fallback unmatched and ambiguous", func(t *testing.T) {
		...
		if _, found, err := store.resolveRepositoryStateIgnoringPurge(automation); err != nil ||
			found {
