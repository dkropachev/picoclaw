...
func TestRepositoryReviewPurgeRequestBoundaryCoverage(t *testing.T) {
	...
	if err := unboundStore.savePurgeIntent(badIntent); !errors.Is(err, ErrInvalidAutomation) {
		t.Fatalf("unbound purge intent error = %v", err)
	}
	if retained, err := unboundStore.loadIgnoringPurge(assigned.Repository); err != nil ||
		retained.Version == 0 {
