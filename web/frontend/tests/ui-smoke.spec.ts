...
async function expectFlowColorContract(
  gateMap: Locator,
  view: "review" | "implementation",
) {
  const expectedFlow = prLifecycleFlowFixture.flow.flows.find(
  ...
  const configuredWorkflows = Object.values(
    prLifecycleGateProfiles.gate_profiles.default.workflows,
  )
  const gateToneByStageKind = {
  ...
  const expectedNodeTones = expectedFlow.nodes.map((node) => {
    if (node.kind === "action") return { id: node.id, tone: "action" }
    if (!node.editable) return { id: node.id, tone: "safeguard" }

    const workflow = configuredWorkflows.find(
      (candidate) => candidate.decision_point === node.decision_point,
    )
    if (!workflow) return { id: node.id, tone: "gate-user" }
    const stages = workflow.stages.map(
      (stage) => gateToneByStageKind[stage.kind],
    )
    const formats = new Set(stages)
    const format =
      stages.length === 0
        ? "needs-setup"
        : formats.size === 1
          ? stages[0]
          : "mixed"
    return { id: node.id, tone: `gate-${format}` }
  })
  ...
  const result = await gateMap.locator(`[data-flow-view="${view}"]`).evaluate(
    (flow, expected) => {
      const issues: string[] = []
      const isVisibleColor = (value: string) =>
        value !== "" &&
        value !== "none" &&
        value !== "transparent" &&
        !/rgba?\([^)]*[,/]\s*0(?:\.0+)?\s*\)$/i.test(value) &&
        !/\/\s*0(?:\.0+)?\s*\)$/i.test(value)
      const styleSignature = (element: Element) => {
        const style = window.getComputedStyle(element)
        return `${style.backgroundColor}|${style.borderTopColor}`
      }
      const assertDistinct = (
        values: Array<{ tone: string; value: string }>,
        subject: string,
      ) => {
        for (let left = 0; left < values.length; left += 1) {
          for (let right = left + 1; right < values.length; right += 1) {
            if (
              values[left].tone !== values[right].tone &&
              values[left].value === values[right].value
            ) {
              issues.push(
                `${subject}: ${values[left].tone} and ${values[right].tone} are visually identical`,
              )
            }
          }
        }
      ...
      const nodeStyles: Array<{ tone: string; value: string }> = []
      for (const expectedNode of expected.nodes) {
        const nodes = Array.from(
          flow.querySelectorAll<HTMLElement>(
            `[data-flow-node-id="${expectedNode.id}"]`,
          ),
        )
        if (nodes.length !== 1) {
          issues.push(`${expectedNode.id}: expected one colored node`)
          continue
        }
        const node = nodes[0]
        if (node.dataset.flowTone !== expectedNode.tone) {
          issues.push(
            `${expectedNode.id}: expected ${expectedNode.tone} node tone, got ${node.dataset.flowTone ?? "none"}`,
          )
        }
        ...
        if (
          ...
          issues.push(`${expectedNode.id}: node surface is transparent`)
        }
        nodeStyles.push({
          tone: expectedNode.tone,
          value: styleSignature(node),
        })
      }
      assertDistinct(
        nodeStyles.filter(
          (item, index) =>
            nodeStyles.findIndex(
              (candidate) => candidate.tone === item.tone,
            ) === index,
        ),
        "node surfaces",
      )

      const edgeStrokeByKey = new Map<string, string>()
      const routeStyles: Array<{ tone: string; value: string }> = []
      for (const expectedEdge of expected.edges) {
        const paths = Array.from(
          flow.querySelectorAll<SVGPathElement>(
            `[data-flow-visible-edge-key="${expectedEdge.key}"]`,
          ),
        )
        if (paths.length !== 1) {
          issues.push(`${expectedEdge.key}: expected one colored route`)
          continue
        }
        const path = paths[0]
        const layer = path.parentElement
        if (
          path.dataset.flowTone !== expectedEdge.tone ||
          layer?.dataset.flowTone !== expectedEdge.tone
        ) {
          issues.push(
            `${expectedEdge.key}: expected ${expectedEdge.tone} route tone`,
          )
        }
        const stroke = window.getComputedStyle(path).stroke
        if (!isVisibleColor(stroke)) {
          issues.push(`${expectedEdge.key}: route stroke is transparent`)
        }
        edgeStrokeByKey.set(expectedEdge.key, stroke)
        routeStyles.push({ tone: expectedEdge.tone, value: stroke })

        const merged = expectedEdge.merged
        const markerReference = path.getAttribute("marker-end")
        if (merged) {
          if (markerReference != null) {
            issues.push(`${expectedEdge.key}: merged route has an arrow`)
          }
          continue
        }
        const markerID = markerReference?.match(
          /url\(["']?#([^"')]+)["']?\)/,
        )?.[1]
        const marker = markerID
          ? flow.querySelector<SVGMarkerElement>(
              `marker[id="${CSS.escape(markerID)}"]`,
            )
          : null
        if (
          !marker ||
          marker.dataset.flowArrowMarker !== expectedEdge.tone ||
          marker.dataset.flowTone !== expectedEdge.tone
        ) {
          issues.push(
            `${expectedEdge.key}: arrow does not reference its ${expectedEdge.tone} marker`,
          )
          continue
        }
        const arrowhead = marker.querySelector<SVGPathElement>(
          `[data-flow-arrowhead="${expectedEdge.tone}"]`,
        )
        if (
          !arrowhead ||
          arrowhead.dataset.flowTone !== expectedEdge.tone ||
          window.getComputedStyle(arrowhead).fill !== stroke
        ) {
          issues.push(`${expectedEdge.key}: arrowhead does not match its route`)
        }
      }
      const uniqueRouteStyles = routeStyles.filter(
        (item, index) =>
          routeStyles.findIndex((candidate) => candidate.tone === item.tone) ===
          index,
      )
      assertDistinct(uniqueRouteStyles, "route strokes")

      const labels = Array.from(
        flow.querySelectorAll<SVGGElement>("[data-flow-launch-label]"),
      )
      for (const label of labels) {
        const key = label.dataset.flowEdgeKey ?? "unknown"
        const expectedEdge = expected.edges.find((edge) => edge.key === key)
        const surface = label.querySelector<SVGRectElement>(
          "[data-flow-label-surface]",
        )
        const text = label.querySelector<SVGTextElement>(
          "[data-flow-label-text]",
        )
        if (
          !expectedEdge ||
          label.dataset.flowTone !== expectedEdge.tone ||
          surface?.dataset.flowTone !== expectedEdge.tone ||
          surface?.dataset.flowLabelSurface !== expectedEdge.tone ||
          text?.dataset.flowTone !== expectedEdge.tone ||
          text?.dataset.flowLabelText !== expectedEdge.tone
        ) {
          issues.push(`${key}: branch label tone does not match its route`)
          continue
        }
        const routeStroke = edgeStrokeByKey.get(key)
        const surfaceStyle = window.getComputedStyle(surface)
        const textStyle = window.getComputedStyle(text)
        if (
          !isVisibleColor(surfaceStyle.fill) ||
          surfaceStyle.stroke !== routeStroke ||
          textStyle.fill !== routeStroke
        ) {
          issues.push(`${key}: branch label colors do not match its route`)
        }
      }

      for (const target of expected.mergeTargets) {
        const stem = flow.querySelector<SVGLineElement>(
          `[data-flow-merge-stem="${target}"]`,
        )
        const diamond = flow.querySelector<SVGRectElement>(
          `[data-flow-merge-diamond="${target}"]`,
        )
        if (
          !stem ||
          !diamond ||
          stem.parentElement?.dataset.flowTone !== "merge" ||
          stem.dataset.flowTone !== "merge" ||
          diamond.dataset.flowTone !== "merge"
        ) {
          issues.push(`${target}: merge connector has no merge tone`)
          continue
        }
        const stemStroke = window.getComputedStyle(stem).stroke
        const diamondStyle = window.getComputedStyle(diamond)
        if (
          !isVisibleColor(stemStroke) ||
          !isVisibleColor(diamondStyle.fill) ||
          diamondStyle.stroke !== stemStroke
        ) {
          issues.push(`${target}: merge connector colors do not match`)
        }
      }

      const legendSets = expected.legends.map((legend) => ({
        actual: Array.from(
          flow
            .closest("section")
            ?.querySelectorAll<HTMLElement>(
              `[data-flow-legend="${legend.kind}"]`,
            ) ?? [],
        )
          .map((item) => item.dataset.flowTone ?? "")
          .sort(),
        expected: [...legend.tones].sort(),
        kind: legend.kind,
      }))
      for (const legend of legendSets) {
        if (legend.actual.join("|") !== legend.expected.join("|")) {
          issues.push(
            `${legend.kind} legend tones: expected ${legend.expected.join(", ")}, got ${legend.actual.join(", ")}`,
          )
        }
      }

      const gateLegendStyles = Array.from(
        flow
          .closest("section")!
          .querySelectorAll<HTMLElement>('[data-flow-legend="gate-format"]'),
      ).map((item) => ({
        tone: item.dataset.flowTone ?? "none",
        value: styleSignature(item),
      }))
      for (const item of gateLegendStyles) {
        const element = flow
          .closest("section")!
          .querySelector<HTMLElement>(
            `[data-flow-legend="gate-format"][data-flow-tone="${item.tone}"]`,
          )!
        const style = window.getComputedStyle(element)
        if (
          !isVisibleColor(style.backgroundColor) ||
          ...
          issues.push(`${item.tone}: gate legend surface is transparent`)
        }
      }
      assertDistinct(gateLegendStyles, "gate legend surfaces")
    ...
    },
    {
      edges: expectedFlow.edges.map((edge) => ({
        ...
        merged: !edge.loop && (forwardIncomingCounts.get(edge.to) ?? 0) > 1,
        tone: edge.loop ? "return" : edge.mode,
      })),
      legends: [
        {
          kind: "element",
          tones: ["action", "editable-gate", "safeguard"],
        },
        {
          kind: "gate-format",
          tones: [
        ...
        {
          kind: "route",
          tones: ["linear", "choice", "parallel", "optional", "return"],
        },
      ],
      mergeTargets: [...forwardIncomingCounts.entries()]
        .filter(([, count]) => count > 1)
        .map(([target]) => target),
      nodes: expectedNodeTones,
    },
  )
...
async function expectNoSeriousA11yViolations(page: Page, scope?: string) {
...
test("unified pull request workspace combines review, implementation, nudges, and gate profiles", async ({
  ...
  await expectFlowColorContract(gateMap, "review")
  await expectFlowConnectorContract(gateMap, "review")
  const useLightTheme = page.getByRole("button", { name: "Use light theme" })
  const useDarkTheme = page.getByRole("button", { name: "Use dark theme" })
  if ((await useLightTheme.count()) > 0) await useLightTheme.click()
  await expect(useDarkTheme).toBeVisible()
  await expectFlowColorContract(gateMap, "review")
  await expectNoSeriousA11yViolations(page, "[data-flow-revision]")
  await useDarkTheme.click()
  ...
  await expectFlowColorContract(gateMap, "implementation")
