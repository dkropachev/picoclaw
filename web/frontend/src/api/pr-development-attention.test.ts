...
describe("PR development attention API", () => {
  ...
  it("strictly reads a case-owned projection without rounding question numbers", async () => {
    ...
    const projection = await getPRDevelopmentAttention(caseID)

    expect(mockedLauncherFetch).toHaveBeenCalledWith(
      `/api/pr-development/${caseID}/attention`,
      {},
    )
  ...
  it("sends one trimmed fenced response and parses the next state", async () => {
    ...
    expect(mockedLauncherFetch).toHaveBeenCalledWith(
      `/api/pr-development/${caseID}/attention/respond`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  it("accepts a zero conversation version for a case with no prior chat", async () => {
    mockedLauncherFetch.mockImplementation(async () =>
      rawResponse(
        '{"case_version":0,"status":"not_required","can_respond":false,"turns":[]}',
      ),
    )

    await expect(getPRDevelopmentAttention(caseID)).resolves.toMatchObject({
      case_version: 0,
      status: "not_required",
    })
    await expect(
      respondToPRDevelopmentAttention(caseID, 0, responseToken, "continue"),
    ).resolves.toMatchObject({ case_version: 0 })
  ...
  it("rejects unknown private fields, malformed tokens, and inconsistent authority", async () => {
    for (const source of [
      ...
      await expect(getPRDevelopmentAttention(caseID)).rejects.toMatchObject({
        status: 502,
  ...
  it("accepts read-only, admission-recovery, and canceled failure projections", async () => {
    mockedLauncherFetch
      .mockResolvedValueOnce(
        rawResponse(
          '{"case_version":0,"status":"recovery_required","can_respond":false,"turns":[]}',
        ),
      )
      .mockResolvedValueOnce(
        rawResponse(
      ...
      .mockResolvedValueOnce(
        rawResponse(
          '{"case_version":7,"status":"recovery_required","can_respond":false,"turns":[{"status":"recovery_required","title":"Choose a safe contract","questions":[],"response":"Keep v1"}]}',
        ),
      )
      .mockResolvedValueOnce(
        rawResponse(
    ...
    await expect(getPRDevelopmentAttention(caseID)).resolves.toMatchObject({
      status: "recovery_required",
      can_respond: false,
      turns: [{ status: "recovery_required", response: "Keep v1" }],
    })
    await expect(getPRDevelopmentAttention(caseID)).resolves.toMatchObject({
      status: "failed",
  ...
  it("rejects tokens outside actionable turns and canceled turns outside failures", async () => {
    for (const source of [
      ...
      waitingProjection().replace('"status":"waiting"', '"status":"checking"'),
  ...
  it("rejects excessive turns, title, questions, and stored responses", async () => {
    ...
    for (const source of sources) {
      mockedLauncherFetch.mockResolvedValueOnce(rawResponse(source))
      await expect(getPRDevelopmentAttention(caseID)).rejects.toBeInstanceOf(
        PRDevelopmentAttentionAPIError,
      )
    }
  })
  ...
  it("validates response fences and maps only a safe public error", async () => {
    await expect(
      respondToPRDevelopmentAttention(caseID, -1, responseToken, "answer"),
    ).rejects.toMatchObject({ status: 400 })
    await expect(
      respondToPRDevelopmentAttention(caseID, 7, "private-task", "answer"),
    ).rejects.toMatchObject({ status: 400 })
    await expect(
      respondToPRDevelopmentAttention(caseID, 7, responseToken, "   "),
    ).rejects.toMatchObject({ status: 400 })
    await expect(
      respondToPRDevelopmentAttention(caseID, 7, responseToken, "\u0085"),
    ).rejects.toMatchObject({ status: 400 })
