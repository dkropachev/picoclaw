...
import type { DispatchView } from "@/api/events"
...
export function DispatchList({
  ...
  return (
    <section className="border-border bg-card/40 flex min-h-[24rem] min-w-0 flex-col overflow-hidden rounded-lg border lg:min-h-0">
      <div className="border-border flex h-12 shrink-0 items-center justify-between gap-2 border-b px-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <h2 className="truncate text-sm font-medium">
            {t("pages.events.dispatch_list.title", "Global dispatches")}
          </h2>
        </div>
      ...
      <EventScrollRegion
        label={t("pages.events.dispatch_list.region", "Dispatches")}
        className="min-h-0 flex-1 overflow-auto p-2"
        onScroll={handleScroll}
      >
        {loading ? (
          <ListMessage>
            {t("pages.events.dispatch_list.loading", "Loading dispatches…")}
          </ListMessage>
          ...
          <ListError
            message={eventErrorMessage(
              error,
              t(
                "pages.events.dispatch_list.error",
                "Failed to load dispatches.",
              ),
            )}
            retryLabel={t("pages.events.dispatch_list.retry", "Retry")}
            onRetry={onRetry}
          />
        ) : dispatches.length === 0 ? (
          <ListMessage>
            {t(
              "pages.events.dispatch_list.empty",
          ...
          <div className="flex min-w-0 flex-col gap-1.5">
            {dispatches.map((dispatch) => {
              const selected = selectedDispatchID === dispatch.id
              return (
                <button
                  key={dispatch.id}
                  type="button"
                ...
                >
                  ...
                  <div className="text-muted-foreground flex min-w-0 items-center gap-1.5 text-xs">
                    ...
                    <span className="shrink-0">
                      {formatEventDate(dispatch.updated_at)}
                    </span>
                  </div>
            ...
            {hasMore ? (
              ...
              >
                {loadingMore
                  ? t(
                      "pages.events.dispatch_list.loading_more",
                      "Loading more…",
                    )
                  : t("pages.events.dispatch_list.load_more", "Load more")}
              </Button>
            ) : null}
          </div>
        )}
