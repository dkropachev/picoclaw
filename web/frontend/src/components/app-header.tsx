...
export function AppHeader() {
  ...
  return (
    <header className="bg-background/95 supports-backdrop-filter:bg-background/80 sticky top-0 z-50 flex h-14 shrink-0 items-center justify-between px-3 backdrop-blur md:px-4">
      <div className="flex items-center gap-2">
        <SidebarTrigger className="text-muted-foreground hover:bg-muted hover:text-foreground flex h-9 w-9 items-center justify-center rounded-lg sm:hidden [&>svg]:size-5">
          <IconMenu2 />
        </SidebarTrigger>
      ...
      <div className="pointer-events-none absolute left-1/2 hidden h-full -translate-x-1/2 items-center justify-center lg:flex">
        {showNotConnectedHint && (
          <div className="border-border/70 bg-background/80 text-muted-foreground flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs backdrop-blur-md">
      ...
      <div className="text-muted-foreground flex items-center gap-1 text-sm font-medium md:gap-1.5">
        {restartRequired && (
          <Tooltip delayDuration={700}>
            <TooltipTrigger asChild>
              <Button
                variant="secondary"
                size="icon-sm"
                className="bg-amber-500/12 text-amber-700 hover:bg-amber-500/20 hover:text-amber-800 dark:text-amber-300 dark:hover:bg-amber-500/20"
            ...
            <TooltipTrigger asChild>
              ...
              >
                <Button
                  ...
                  size="sm"
                  data-tour="gateway-button"
                  className={`h-8 gap-2 rounded-lg px-3 max-[359px]:w-8 max-[359px]:px-0 ${
                    isStopped
                      ? "bg-emerald-700 text-white hover:bg-emerald-800"
                      : ""
                  } ${!canStart ? "pointer-events-none" : ""}`}
                  onClick={handleGatewayToggle}
                  aria-label={t("header.gateway.action.start")}
                  disabled={
                    gwLoading ||
                    isStarting ||
                    isRestarting ||
                ...
                >
                  ...
                  <span className="text-xs font-semibold max-[359px]:hidden">
                    {isStopping
        ...
        <Separator
          className="mx-2 my-2 max-[359px]:hidden"
          orientation="vertical"
        />
