...
describe("RepositoryReviewRepositoriesPage", () => {
  ...
  it("shows unavailable history counts without rendering zeros", async () => {
    vi.mocked(getRepositoryReviewAutomationDetail).mockResolvedValue({
      ...repositoryDetail,
      capabilities: {
        can_purge_history: false,
    ...
    expect(
      await screen.findByText(/History deletion counts are unavailable/i),
    ).toBeVisible()
    expect(screen.queryByText("Raw findings")).not.toBeInTheDocument()
    expect(
      screen.getByRole("button", { name: "Purge review history" }),
    ).toBeDisabled()
    expect(screen.getByRole("button", { name: "Remove" })).toBeDisabled()
