...
func parseCollectionListRequest(
	...
	if rawLimit := strings.TrimSpace(values.Get("limit")); rawLimit != "" {
		parsed, parseLimitErr := strconv.Atoi(rawLimit)
		if parseLimitErr != nil || parsed < 1 || parsed > collectionquery.MaxPageSize {
			writeCollectionError(
				w,
				http.StatusBadRequest,
				"invalid_page_limit",
	...
	for key, entries := range values {
		if _, ok := allowlist[key]; !ok || len(entries) != 1 {
			writeCollectionError(
				...
				http.StatusBadRequest,
				"invalid_collection_request",
				"Unsupported collection query parameter",
				-1,
				nil,
			)
			return false
...
func writeCollectionPageError(w http.ResponseWriter, err error) {
	...
	writeCollectionError(
		...
		"Failed to page collection results",
		-1,
		nil,
	)
}
