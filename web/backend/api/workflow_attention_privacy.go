...
// workflowRunPrivacySnapshot is one fail-closed view of the run store. Exact
// review-attention run IDs are removed from generic projections, and every
...
func newWorkflowRunPrivacySnapshot(runs []workflows.Run) *workflowRunPrivacySnapshot {
	...
	for index := range runs {
		...
		if reviews.IsAttentionWorkflowRun(&run) {
...
func (snapshot *workflowRunPrivacySnapshot) runMutationAllowed(runID string) bool {
	...
	if !exists || reviews.IsAttentionWorkflowRun(&run) {
		return false
	}
...
func (snapshot *workflowRunPrivacySnapshot) sanitizeRun(
	...
	if snapshot == nil || run == nil || reviews.IsAttentionWorkflowRun(run) ||
