...
func TestLauncherMainHostsAndShutsDownEmbeddedGateway(t *testing.T) {
	...
	brokerHandler := dashboardauth.NewBrokerHandler(
		homePath,
		launcherconfig.PathForAppConfig(configPath),
	)
	_, catalogFingerprint, err := database.LoadCatalogConfiguration(configPath)
	if err != nil {
		t.Fatalf("LoadCatalogConfiguration() error = %v", err)
	}
	brokerServer, err := database.StartServer(context.Background(), database.ServerOptions{
		Home:               homePath,
		CatalogFingerprint: catalogFingerprint,
		RequiredStores:     []database.StoreID{"launcher/auth"},
		StatusProvider: func(context.Context) ([]database.StoreStatus, error) {
			return []database.StoreStatus{{
				ID: "launcher/auth", Readiness: database.StoreReady,
			}}, nil
		},
		Handler:      brokerHandler,
		CloseHandler: brokerHandler.Close,
	})
	if err != nil {
	...
	t.Cleanup(func() {
		...
		_ = brokerServer.Close(context.Background())
	})
