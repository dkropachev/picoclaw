...
| `FR-DATABASE-005` | MUST | A keyed mutation begins or completes within one broker epoch. | Requests with the same exact StoreID, domain, version, operation, idempotency key, and canonical payload coalesce across fresh transport request IDs and deadlines. Replay returns a detached retained response rebound to the current request ID while preserving the stored broker epoch; the same key text under a different StoreID/domain/version/operation occupies a distinct scoped record. | The bounded in-memory registry retains records and result bytes for the whole epoch and never evicts a completed key. | A full registry rejects before dispatch; same-scope key reuse with a different canonical payload conflicts; canceled waiters receive `Deadline`; an outcome exceeding retained-result bounds becomes `OutcomeUnknown` and cannot trigger shutdown. Registry identity is not durable across broker restart. | Eviction, cross-scope replay, transport-ID-dependent semantics, or ambiguous retries could duplicate a committed mutation or return another operation's result. |
...
- `Request.ID` is transport correlation only; keyed handlers derive durable
  identity and mutation semantics from the idempotency key and typed payload.
- Transport protocol, token, current epoch, and deadline are validated before
  registry admission; request ID and deadline are not mutation identity, while
  the registry instance itself supplies the epoch boundary.
- Epoch registry replay does not claim cross-restart durability; a domain that
  needs it must enforce it in the mutation transaction.
...
| `FR-DATABASE-005` | [pkg/database/idempotency_test.go](../../pkg/database/idempotency_test.go), [pkg/database/idempotency_store_test.go](../../pkg/database/idempotency_store_test.go), [pkg/database/protocol_coverage_test.go](../../pkg/database/protocol_coverage_test.go), [pkg/database/store_scope_integration_test.go](../../pkg/database/store_scope_integration_test.go) |
