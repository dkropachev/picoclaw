...
| `FR-DATABASE-IPC-005` | MUST | A client connects or invokes a read or mutation, optionally through `CallStore`, `CallStoreWithOptions`, or `CallOptions.StoreID`. | The client uses only the discovered endpoint/token/epoch, the caller's exact logical StoreID, canonical framed requests, typed results, and structured failures. Store-bound reads may rediscover once after replacement without changing target; a mutation's keyed lost-response retry reuses one exact envelope, while fresh same-epoch calls may use new request IDs/deadlines and still match the same exact StoreID/domain/version/operation/key/canonical-payload identity. A replay response is rebound to the current request ID and retains its stored broker epoch. | `InstallProcessClient` publishes only an in-process client pointer, and inherited authority is consumed once from a bounded canonical environment value. | Store-bound entry points validate their target before discovery or payload work; mutation disconnect becomes `OutcomeUnknown`; noncanonical, mismatched, stale-epoch, or invalid responses fail closed; no provider fallback occurs. Registry replay is epoch-local and does not claim cross-restart durability. | Callers must not infer whether a disconnected mutation committed, derive durable semantics from transport request IDs, retarget a retry, or bypass the broker. |
...
mechanism for trusted assembly, not a catalog constructor: arbitrary logical
bindings cannot resolve or authorize physical storage. The authenticated hidden
supervisor now supplies the closed `FR-DATABASE-PROVIDER-CATALOG` review
projection, full-catalog-bound scope fingerprint, exact selected served and
required IDs, matching unavailable-status set, and an empty handler registry.
No provider, claim, migration, readiness probe, gateway, launcher auto-start,
public command, adapter, or application client is connected by this change.
...
- Mutation transport loss is outcome-unknown even when a read would be safely
  retryable.
- A handler treats `Request.ID` only as transport correlation. Keyed durable
  identity and mutation semantics come from `IdempotencyKey` and typed payload;
  cross-restart guarantees, when needed, belong to the domain transaction.
- Protocol, token, current epoch, and deadline validation precede idempotency
  admission. Request ID and deadline are transport fields, and the registry
  instance—not fingerprint input—provides the epoch boundary.
...
| `FR-DATABASE-IPC-005` | [pkg/database/idempotency_test.go](../../pkg/database/idempotency_test.go), [pkg/database/ipc_additional_test.go](../../pkg/database/ipc_additional_test.go), [pkg/database/ipc_additional_unix_test.go](../../pkg/database/ipc_additional_unix_test.go), [pkg/database/client_store_test.go](../../pkg/database/client_store_test.go), [pkg/database/client_store_retry_unix_test.go](../../pkg/database/client_store_retry_unix_test.go), [pkg/database/store_scope_integration_test.go](../../pkg/database/store_scope_integration_test.go) |
