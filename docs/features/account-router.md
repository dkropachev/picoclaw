...
| `FR-ACCOUNT-ROUTER-005` | MUST  | Router state persists as typed normalized rows in `<workspace>/state/account-router.db`: routers own config hashes/timestamps, accounts own typed health/reasons/counters/usage/full-range timestamps and auth-generation fences, sessions own ordered block affinities, block cursors are relational, and credential invalidations are versioned rows. Every selection/result update acquires the cross-process lock and one `BEGIN IMMEDIATE`, reloads current rows, consumes invalidations, mutates/prunes, consumes again, and commits one versioned generation, preventing stale whole-state overwrites and late pre-renewal auth results. Credential renewal inserts a new generation transactionally and clears only matching auth failures on the next health transaction while advancing non-auth fences unchanged. First open deterministically imports bounded `account_router_state.json` before exact `.auth-invalidation.<32 lowercase hex>` siblings, records payload-free skips/digests, and archives examined bytes without overwrite under `state/legacy-json/account-router-v1/`; malformed SQLite or unsafe sources fail closed without corrupt-file renames, JSON fallback, sidecars, or dual writes. The old file-named constructor remains a deprecated sibling-DB facade. | Account health must survive restarts and concurrent gateway/launcher processes without pinning bad state, losing unrelated updates, leaking errors, or restoring mutable JSON authority. |
...
`state/account-router.db`. Normalized tables store config hashes, typed account
...
paths, and removing or restoring `account-router.db` with its matching WAL,
SHM, and lock directory as one generation; mixed old/new binaries are
unsupported.
...
| `FR-ACCOUNT-ROUTER-003`, `FR-ACCOUNT-ROUTER-008`, `FR-ACCOUNT-ROUTER-004`, `FR-ACCOUNT-ROUTER-005` | [pkg/accountrouter/router_test.go](../../pkg/accountrouter/router_test.go), [pkg/accountrouter/store_sqlite_test.go](../../pkg/accountrouter/store_sqlite_test.go) |
