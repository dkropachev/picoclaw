...
supervisor monitors readiness and owns child cleanup without opening a database,
loading configuration, registering a command, or starting automatically.
...
- Core types/functions: `EnsureOptions`, `EnsureSupervisor`,
  `MonitorSupervisor`, and `ConsumeSupervisorBootstrap`.
...
filesystem mutation. Each attempt retains the root process, completion signal,
and process-group/Job owner until
it is either proven live and released or fully terminated and reaped.
...
| Bootstrap API | `ConsumeSupervisorBootstrap` | Consume one exact same-home capability before later child composition. | `FR-DATABASE-SUPERVISOR-003` |
...
4. Create/sync the exact identity-bound bootstrap, open a private single-link
   ...
   waiter.
5. Probe readiness until the operation deadline. Cleanup waits for attempt
   deadlines in parallel and authenticates status immediately before mutation.
6. Release only a still-live attempt matching the authenticated PID and
...
7. Monitoring retries transient absence with capped exponential backoff,
