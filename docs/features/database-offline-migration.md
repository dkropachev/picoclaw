...
3. Snapshot and verify selected bytes and derive existence from the verified
   ...
   sources and preflights adapter state inside sealed disposable callbacks. A
   current, existing, nonempty neutral-ready generation also runs its optional
   exact validator here; that check never makes `Migrate` required.
...
5. Mint the StoreID-bound immutable source, derive a finite provider child,
   ...
   validate the sole final stage against both its neutral contract and optional
   exact-domain validator before release, retirement, or pinning. Use the
   provider-minted opaque scope to reverify backup and live sources with only
   that exact main excluded; then let the provider recheck, replace, promote,
   activate, and reconcile the generation.
...
- A missing callback is acceptable only when the exact store state needs no
  domain migration.
- A current contract-ready generation skips `Adapter.Migrate` but still runs
  exact validation during preflight and against the provider-created final
  stage. Semantic mismatch fails as integrity before revision 1 or cutover.
...
| `FR-DATABASE-OFFLINE-MIGRATION-003`, `FR-DATABASE-OFFLINE-MIGRATION-004` | [internal/databasemigration/migration_test.go](../../internal/databasemigration/migration_test.go), [internal/databasemigration/migration_backup_integration_test.go](../../internal/databasemigration/migration_backup_integration_test.go), [internal/databasemigration/coverage_engine_run_additional_test.go](../../internal/databasemigration/coverage_engine_run_additional_test.go), [internal/databasemigration/coverage_engine_store_migration_test.go](../../internal/databasemigration/coverage_engine_store_migration_test.go), [internal/databasemigration/coverage_engine_store_closeout_test.go](../../internal/databasemigration/coverage_engine_store_closeout_test.go) |
