...
use the strict automation `/findings` resource. Repository Reviews does not
expose a second occurrence collection or translate retired finding identities.
...
findings** means cross-commit `rrf_*` records.
...
- Non-obvious constraints: repository bytes remain immutable no-tool evidence;
  ...
  responses or durable state; previews are one finding each; issue publication and existing-issue
...
- Candidate-scope failures expose bounded shape-only diagnostics: a
  ...
  detail keeps Continue available from saved review state.
...
| `FR-REPOREVIEW-002` | MUST  | Automation-scoped APIs return compact summaries and independently paged Findings, Raw findings, Findings processing, repository findings, and issue previews while omitting internal checkpoint maps. Run findings are immutable; issue-preview and repository-finding lifecycle mutations use their documented version fence and return only bounded changed-resource envelopes. | The frontend must recover large ledgers, reach older records, and reject stale edits without returning the whole internal state. |
...
| `FR-REPOREVIEW-015` | MUST  | Repository configuration accepts only an optional branch name. Blank resolves through the acquired repository's advertised default branch; `HEAD`, commit hashes, tags, full refs, revision expressions, URLs, query/fragment forms, and invalid Git branch names are rejected before admission. The launcher exposes Review runs, Repositories, and Profiles destinations and places run findings under Review runs while repository aggregates and their issue/lifecycle actions use nested routes below Repositories. Repositories and Profiles are independent standard server-query/paged list-table-grid collections with stable `/repositories/new`, `/repositories/:id`, `/repositories/:id/edit`, `/profiles/new`, `/profiles/:profileID`, and `/profiles/:profileID/edit` routes that preserve collection state. Every repository item and configuration detail exposes its repository-owned findings route. Basic profile identity, execution account, reviewer, issue writer, issue prompt, focus, and Scope are immediately visible in the profile editor; account precedes both model selectors because it constrains their availability. Sizing and task-admission guard remain in Advanced, collapsed by default, and preserve their draft while closed. New profiles default to eight configurable parallel review workers; adding a guard does not silently serialize them because admission uses atomic in-flight reservations. Internal output-token estimation is server-owned and absent from profile/API controls. | Repository review should follow a predictable branch, keep core review intent visible, and give configuration, run evidence, and canonical repository state unambiguous navigation ownership. |
...
| `FR-REPOREVIEW-024` | MUST  | Every occurrence persists the resolved target branch, advertised default branch, and `target_is_default`. A default-branch occurrence may create or join a repository finding. A non-default occurrence may join an already-known finding but cannot create one. A new occurrence after confirmed resolution changes lifecycle to `regressed` while retaining prior resolution history. | Canonical repository state must describe defects on the repository's shipped line without discarding useful corroboration from other branches. |
...
| `FR-REPOREVIEW-028` | MUST  | Repository identity resolution is centralized in `pkg/repoaudit` and shared by controller, API, and gateway for GitHub URL, SSH/SCP, owner/repository shorthand, and absolute local paths. Each configuration resolves exactly one canonical ledger; run IDs and original remote spellings are not fallback lookup authorities. Automation progress exposes current-campaign `raw_findings` and `deduplicated_findings`, and `raw_findings` is a sortable review-collection field. Canonical finding, raw-finding, repository-finding, and issue-preview UI/API routes use separate automation-scoped query/cursor collection resources. Retired report, results, repository-ID, and `rfn_*` routes are not registered or redirected. User-visible copy uses Findings, Raw findings, and repository findings; it never uses Report, review report, durable findings, or internal association-job terminology. | All entry points must count and display the same current campaign while route and copy make raw evidence, deduplicated review findings, and cross-commit aggregates unmistakable. |
...
| `FR-REPOREVIEW-030` | MUST  | Each campaign freezes an ordered catalog over `correctness_state`, `security_trust`, `concurrency_recovery`, and `integration_validation`, binding stable assignment identity to reviewer, prompt revision, and profile hash. Planning emits only missing file-assignment pairs within the profile batch bound; a durable active-run reservation precedes dispatch. Confirmed live-checkpoint attribution is append-only, exact, idempotent, and separately reports assignment-pair progress from fully reviewed files. | Confirmed focus/reviewer work must survive sibling failure and restart without fabricating credit or repeating a proven pair. |
| `FR-REPOREVIEW-031` | MUST  | Before a successful child callback, every confirmed diagnosis is atomically stored as immutable `rrw_*` evidence with context, checkpoint, campaign/path/blob/symbol bucket, insertion order, concrete model/configured alias/account provenance, counters, and one durable deduplication job. Unknown provenance remains empty rather than invented. Bounded private FIFO deduplication creates a diagnosis-verbatim `rdf_*` or attaches ordered raw provenance after rechecking its lease/universe/version fences. Three failures leave safe readable evidence and explicit retry authority. | Concurrent workers must not publish duplicate campaign diagnoses or hide raw evidence needed to audit the decision. |
| `FR-REPOREVIEW-032` | MUST  | Repository state schema 6 stores only canonical `rrw_*`, `rdf_*`, and `rrf_*` records. It does not ingest pre-assignment review results, synthesize raw evidence, retain `rfn_*` shadows, replay historical findings, or translate retired identities and endpoints. Unsupported older repository ledgers fail closed instead of being mutated during ordinary reads. | Every diagnosis layer must have one authority; compatibility projections must not compete with canonical evidence. |
| `FR-REPOREVIEW-033` | MUST  | Each automation exposes one health projection: run counts use selected-campaign membership; repository and processing counts use the canonical ledger; `unrepresented` is the direct pending+processing+failed sum. Findings processing is a typed cursor collection defaulting to `ALL ORDER BY updated DESC`, exposes safe source/detail state, allows selection only for failed sources, and polls only while work is active. Review and repository surfaces separate Run findings, Repository findings, and Findings processing and never render unavailable health as zero. | Operators need one honest coverage view and a bounded path to every failure. |

The following atomic rows are authoritative for the high-risk behavior in
`FR-REPOREVIEW-017` and `FR-REPOREVIEW-029` through `FR-REPOREVIEW-033`.
`FR-REPOREVIEW-017` is limited to collection navigation and presentation,
`FR-REPOREVIEW-029` to checkpoint/campaign authority, `FR-REPOREVIEW-030` to
assignment attribution, `FR-REPOREVIEW-031` to raw admission and native
deduplication, `FR-REPOREVIEW-032` to the canonical-only persistence boundary,
and `FR-REPOREVIEW-033` to health/processing presentation and retry.

| ID | Level | Trigger/Input | Required Output | State Mutation | Failure/Edge | Rationale |
| --- | --- | --- | --- | --- | --- | --- |
| `FR-REPOREVIEW-034` | MUST | A client reads Findings, Raw findings, Findings processing, or repository findings. | The server and UI use the `rrw_*` → `rdf_*` → `rrf_*` taxonomy and `/findings` returns completed current-campaign `rdf_*` records only. | Reads do not promote, synthesize, or remap evidence. | Pending/failed raw records never fall through into `/findings`; repository-wide processing and cross-commit aggregates never masquerade as campaign findings; retired identities return `404`. | Every diagnosis layer must have one stable meaning. |
...
| `FR-REPOREVIEW-041` | MUST | A trusted operator purges history or removes a repository assignment with the expected automation version, canonical repository version, opaque ledger fence, and exact normalized repository confirmation. | Purge returns a fresh idle configuration and removes its canonical repository-review ledger plus the migration-audited imported automation archive; removal deletes the configuration, canonical ledger, and automation archive. Skipped, corrupt, unowned, or digest-drifted archives are retained or fail closed rather than being silently included. GitHub issues, profiles, discussion threads, and generic workflow-run records are untouched. | A `0600` restart-safe purge intent records the canonical ledger identity/version, serializes version-checked SQLite reset/removal and audited archive deletion, and is reconciled before workers start. History is otherwise retained indefinitely. | Any active review, deduplication, mapping, Resolution check, or issue generation/publication blocks the action. Unreadable inventory fails closed with `retention_unavailable` and omits `purge_summary`; stale fences, mismatched confirmation, unsafe intent/archive paths, archive audit drift, or interruption fail/recover without resurrecting state. | Retention must be explicit, destructive, and recoverable across process loss. |
...
| Current campaign coverage    | `current_campaign` and `campaign_history` in the repository ledger          | Controller-authorized single-use identity; exact commit/inventory/profile/scope/required-assignment binding; a frozen deduplication profile/account/model snapshot; bounded monotonic per-path inspected, completed, and unsupported state; and verified run/context/finding provenance. An absent or inexact coverage record is unknown, never an inferred zero. |
...
| Workflow run                 | `workspace/state/workflows.db`                                              | Generic durable typed job/step state and ordered events; automation stores only bounded identities/progress and requires a verified `record` output or authoritative no-op checkpoint.                                                                                                                                                                                                     |
| Controller lease             | `workspace/repository_reviews/.locks/controller.lock`                       | Private non-blocking workspace-wide OS lock held for the launcher controller lifetime; never returned by an API. All store, slot, and per-attempt locks live in this protected `.locks/` directory.                                                                                                                                                                                           |
...
Schema 6 is a deliberate storage boundary. On first open, the SQLite v2
migration removes every older repository ledger and its assigned automation;
it retains profiles and automations that do not reference a retired ledger.
No finding data is converted into the canonical chain.
...
mode. Both actions also delete the migration-audited, digest-matching imported
automation archive represented by the configuration; skipped or uncounted
archives and profile archives remain.
...
with `repository_version`, opaque `ledger_fence`,
...
by the same calculation used by API preflight and the locked store mutation; it
is advisory rather than a substitute for automation and canonical-ledger
fences. The ledger fence binds the canonical ledger identity and version. When
inventory cannot be read, both destructive
capabilities fail closed with one `retention_unavailable` blocker and no
authority to mutate; `purge_summary` is omitted so unavailable counts never
render as zero. An assignment with no ledger has zero numeric version and
...
removes the marker. SQLite ledger rows and migration-audited, digest-matching
imported repository/summary/automation archives are removed consistently and
cannot resurrect purged content.
...
Origins are `ai_generated`, `linked`, or `discovered`. Every preview contains
one finding ID. The same manually linked GitHub issue may be the validated
canonical association for more than one finding.
...
`resolution_pending`, `resolved`, and `regressed` are evidence-driven. A
confirmed merge preserves the most actionable compatible state and records
incompatible issue URLs as a conflict rather than inventing a transition.
...
Retry fix check. Failure details remain hidden whenever the current
resolution-check state is not `failed`.
...
complete detail projections. The protected gateway retains authority for GitHub create, marker
reconciliation, search, and exact-issue re-fetch; the issue-writer model has no
provider tools.
HTTP creation always requires `profile_id`; the split UI does not expose
profileless editing or multi-model actual-review configuration.

The first database open may import legacy profile and automation configuration
JSON in dependency-safe deterministic order. Repository finding ledgers and
summaries are not imported. Examined configuration sources move without replacement to
...
| HTTP | `POST /api/repository-reviews/automations/{automation_id}/purge-history`; destructive `DELETE /api/repository-reviews/automations/{automation_id}` | Require `expected_version`, `expected_repository_version`, opaque `expected_ledger_fence`, and exact normalized `confirm_repository`. Purge preserves a fresh idle configuration; Delete removes the configuration and canonical ledger. Both use the same authoritative blockers/capabilities and restart-safe intent, and never change GitHub issues, profiles, discussion threads, or generic workflow runs. | `FR-REPOREVIEW-041`, `FR-REPOREVIEW-042` |
| HTTP         | `GET /api/repository-reviews/automations/{automation_id}/findings?query=...&cursor=...&limit=...`; `GET .../findings/{finding_id}`; nested `.../sources`; `POST .../findings/status`; `GET .../raw-findings?query=...&cursor=...&limit=...`; `GET .../raw-findings/{source_id}`; `POST .../raw-findings/{source_id}/retry` | Strictly query completed current-campaign findings with source counts and processing counters; load representative diagnosis and paged sources; retry bounded failed repository-mapping status; and independently query, inspect, or retry current-campaign raw records. | `FR-REPOREVIEW-031`, `FR-REPOREVIEW-032`, `FR-REPOREVIEW-037`, `FR-REPOREVIEW-038` |
| HTTP         | `GET /api/repository-reviews/automations/{automation_id}/finding-health`; `GET .../findings-processing?query=...&cursor=...&limit=...`; `GET .../findings-processing/sources/{source_id}`; `POST .../findings-processing/sources/{source_id}/retry`; `POST .../findings-processing/retry` | Project selected-automation run coverage plus canonical-ledger repository/processing health; query every canonical processing source with typed cursor binding; inspect one source; and retry one or 1–200 unique failed sources with explicit ordered partial outcomes, updated counters, and health. | `FR-REPOREVIEW-031`–`FR-REPOREVIEW-033` |
...
| HTTP         | `POST /api/repository-reviews/automations/{automation_id}/issues/generations`; `GET /api/repository-reviews/automations/{automation_id}/issues?query=...&cursor=...&limit=...&generation_id=...`; `GET/PATCH/DELETE /api/repository-reviews/automations/{automation_id}/issues/{draft_id}`; `POST .../issues/{draft_id}/regenerate`; `POST .../issues/publish`; `POST .../issues/{draft_id}/publish` | Reserve and generate at most 200 explicit one-finding previews under one generation ID with four-call concurrency; query/cursor-page compact durable previews with generation-bound cursors and authoritative `publishable`/`publish_blockers`; load one preview with the same `can_publish`/blocker capability projection; version-fence edit, regeneration, or conditional deletion; and publish only an eligible confirmed subset or one eligible preview with per-preview outcomes. Blocked selections fail before gateway/provider initialization. | `FR-REPOREVIEW-005`, `FR-REPOREVIEW-006`, `FR-REPOREVIEW-016`, `FR-REPOREVIEW-018`, `FR-REPOREVIEW-019` |
...
| HTTP | `POST .../findings/{finding_id}/post` | Preserve direct generate-then-publish behavior. Canonical issue authority remains repository-finding based and direct Post re-checks eligibility under the ledger lock. | `FR-REPOREVIEW-016`, `FR-REPOREVIEW-019`, `FR-REPOREVIEW-039`, `FR-REPOREVIEW-042` |
...
1. Durable ledger load accepts the current canonical schema and validates every
   finding, draft, job, and cross-resource reference before returning state.
...
11. New campaign raw findings, contexts, and model projections join by campaign
    ...
    The repository-owned projection reads the complete canonical aggregate
    ledger. Polling observes newly committed checkpoints, deduplication, and
    mapping.
...
17. Purge/removal preflight computes one authoritative capability and blocker
    ...
    resets the automation and removes its version-fenced SQLite ledger plus
    its migration-audited imported automation archive, or removes the
    configuration, ledger, and archive. It
    advances and durably rewrites the intent only
...
- User-visible progress is file-based rather than batch-based. Campaigns show
  resolved files against the selected scope.
...
- Purge and removal are blocked while review, deduplication, repository mapping,
  resolution checking, or issue generation/publication is active. A stale automation/ledger version, incorrect typed
...
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FR-REPOREVIEW-001` | [pkg/repoaudit/store_test.go](../../pkg/repoaudit/store_test.go), [pkg/repoaudit/assignment_run_test.go](../../pkg/repoaudit/assignment_run_test.go) |
| `FR-REPOREVIEW-002` | [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [web/backend/api/repository_review_collections_test.go](../../web/backend/api/repository_review_collections_test.go), [web/frontend/src/api/repository-reviews.test.ts](../../web/frontend/src/api/repository-reviews.test.ts) |
...
| `FR-REPOREVIEW-007` | [pkg/repoaudit/store_test.go](../../pkg/repoaudit/store_test.go), [pkg/repoaudit/assignment_run_test.go](../../pkg/repoaudit/assignment_run_test.go) |
...
| `FR-REPOREVIEW-028` | [pkg/repoaudit/identity.go](../../pkg/repoaudit/identity.go), [pkg/gateway/repository_review_publication_coverage_test.go](../../pkg/gateway/repository_review_publication_coverage_test.go), [web/backend/api/repository_review_details_test.go](../../web/backend/api/repository_review_details_test.go), [web/frontend/src/routes/-repository-reviews-route.test.tsx](../../web/frontend/src/routes/-repository-reviews-route.test.tsx) |
| `FR-REPOREVIEW-029` | [pkg/repoaudit/campaign_foundation_test.go](../../pkg/repoaudit/campaign_foundation_test.go), [pkg/repoaudit/store_test.go](../../pkg/repoaudit/store_test.go), [pkg/repoaudit/coverage_boundaries_test.go](../../pkg/repoaudit/coverage_boundaries_test.go), [pkg/workflows/repository_review_campaign_privacy_test.go](../../pkg/workflows/repository_review_campaign_privacy_test.go), [web/backend/api/repository_review_automations_test.go](../../web/backend/api/repository_review_automations_test.go), [web/backend/api/repository_review_coverage_test.go](../../web/backend/api/repository_review_coverage_test.go), [pkg/gateway/repository_review_issue_sync_coverage_test.go](../../pkg/gateway/repository_review_issue_sync_coverage_test.go) |
| `FR-REPOREVIEW-030` | [pkg/repoaudit/assignment_run_test.go](../../pkg/repoaudit/assignment_run_test.go), [pkg/repoaudit/assignment_timeout_test.go](../../pkg/repoaudit/assignment_timeout_test.go), [pkg/repoaudit/file_attribution_test.go](../../pkg/repoaudit/file_attribution_test.go), [pkg/agent/workflow_managed_ensemble_test.go](../../pkg/agent/workflow_managed_ensemble_test.go), [web/backend/api/repository_review_file_attributions_test.go](../../web/backend/api/repository_review_file_attributions_test.go), [web/backend/api/repository_review_automations_test.go](../../web/backend/api/repository_review_automations_test.go), [web/frontend/src/api/repository-reviews.test.ts](../../web/frontend/src/api/repository-reviews.test.ts), [web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx) |
| `FR-REPOREVIEW-031` | [pkg/repoaudit/deduplication_schema_test.go](../../pkg/repoaudit/deduplication_schema_test.go), [pkg/repoaudit/deduplication_engine_test.go](../../pkg/repoaudit/deduplication_engine_test.go), [pkg/repoaudit/deduplication_worker_test.go](../../pkg/repoaudit/deduplication_worker_test.go), [pkg/repoaudit/lifecycle_test.go](../../pkg/repoaudit/lifecycle_test.go), [web/backend/api/repository_review_dedup_test.go](../../web/backend/api/repository_review_dedup_test.go), [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [web/frontend/src/api/repository-reviews.test.ts](../../web/frontend/src/api/repository-reviews.test.ts), [web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts) |
| `FR-REPOREVIEW-032` | [pkg/repoaudit/sqlite_v2_test.go](../../pkg/repoaudit/sqlite_v2_test.go), [pkg/repoaudit/deduplication_schema_test.go](../../pkg/repoaudit/deduplication_schema_test.go), [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [web/frontend/src/routes/-repository-reviews-route.test.tsx](../../web/frontend/src/routes/-repository-reviews-route.test.tsx) |
| `FR-REPOREVIEW-033` | [pkg/repoaudit/deduplication_worker_test.go](../../pkg/repoaudit/deduplication_worker_test.go), [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [web/frontend/src/api/repository-reviews.test.ts](../../web/frontend/src/api/repository-reviews.test.ts), [web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts), [web/frontend/tests/collection-visual.spec.ts](../../web/frontend/tests/collection-visual.spec.ts) |
...
| `FR-REPOREVIEW-037` | [pkg/repoaudit/deduplication_worker_test.go](../../pkg/repoaudit/deduplication_worker_test.go), [pkg/repoaudit/deduplication_bulk_retry_test.go](../../pkg/repoaudit/deduplication_bulk_retry_test.go), [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go) |
| `FR-REPOREVIEW-038` | [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx) |
...
| `FR-REPOREVIEW-042` | [web/backend/api/repository_review_details_test.go](../../web/backend/api/repository_review_details_test.go), [web/backend/api/repository_review_collections_test.go](../../web/backend/api/repository_review_collections_test.go), [web/backend/api/repository_review_route_docs_test.go](../../web/backend/api/repository_review_route_docs_test.go), [web/backend/api/repository_review_v6_routes_test.go](../../web/backend/api/repository_review_v6_routes_test.go), [docs/reference/repository-reviews-api.md](../reference/repository-reviews-api.md) |
