...
| `FR-REPOREVIEW-041` | MUST | A trusted operator purges history or removes a repository assignment with the expected automation version, primary repository version, opaque composite ledger fence, and exact normalized repository confirmation. | Purge returns a fresh idle configuration and removes every resolved repository-review ledger plus its migration-audited imported legacy archive; removal deletes both configuration and all resolved ledgers/audited archives. Skipped, corrupt, unowned, or digest-drifted archives are retained or fail closed rather than being silently included. GitHub issues, profiles, discussion threads, and generic workflow-run records are untouched. | One typed broker command runs the provider-private phase machine: a `0600` restart-safe purge intent records the complete sorted ledger target/version set, serializes version-checked provider reset/removal and audited legacy-copy deletion, and a broker reconciliation command completes it before workers start; the primary intent fences every target even if a later fence-copy write fails. History is otherwise retained indefinitely. | Any active review, deduplication, mapping, Resolution check, issue generation/publication, or historical consolidation blocks the action. Unreadable inventory fails closed with `retention_unavailable` and omits `purge_summary`; stale versions or composite fence, mismatched confirmation, unsafe intent/archive paths, archive audit drift, or interruption fail/recover without resurrecting state. | Retention must be explicit, destructive, and recoverable across process loss. |
...
| ---------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Review state database        | Opaque repository-review logical `StoreID`; provider-private generation                        | Typed summaries, versions, statuses, timestamps, normalized profile scopes, ordered reviewer/run relationships, and ledger record identities. Bounded JSON BLOBs retain only nested evidence, policy, checkpoint, and aggregate payloads. The private provider generation uses WAL, foreign keys, `synchronous=FULL`, integrity checks, and transactional CAS without exposing its path or handles. |
...
| Controller lease             | Opaque broker controller lease; provider-private lock                              | Non-blocking workspace-wide broker lease held for the launcher controller lifetime; its provider lock and identity never cross the broker or API boundary.                                                                                                                                                                                                                                                                                    |
...
mode. Both actions also delete migration-audited, digest-matching imported
repository/automation archive copies represented by the confirmed inventory;
skipped or uncounted archives and profile archives remain.
...
is advisory rather than a substitute for automation, primary-ledger, and
composite-ledger fences. The composite fence binds every configured-identity or
authoritative fallback ledger and version, preventing a changed hidden alias
from being deleted by stale confirmation. When inventory cannot be read, both destructive
...
Both destructive actions enter the opaque repository-review logical store as
one typed broker mutation. The broker-side provider serializes that whole
operation with one validated `0600` primary automation intent plus `0600`
repository-identity fence copies; paths and archive identities never cross the
broker boundary. The intents contain only the configured and resolved
normalized targets, operation, expected versions, current phase, and creation
time. Phases are `prepared`,
...
an interrupted reset/removal through one broker reconciliation command before
starting repository-review workers, then removes the marker. Provider ledger rows
and migration-audited, digest-matching imported
repository/summary/automation archives are removed consistently and cannot
resurrect purged content.
...
Profile, repository configuration, summary, and ledger state is addressed only
through the broker-selected opaque repository-review logical `StoreID`; its
physical provider path, pool, journal, locks, and transactions remain private.
Authenticated profile CRUD uses `/api/repository-reviews/profiles*`. Repository
...
Ordinary startup never imports legacy profile, automation, repository-ledger, or
summary JSON. Detecting any legacy source or outdated schema returns
`MigrationRequired` without mutation. Only the exclusively fenced offline
database migrator imports those sources in dependency-safe deterministic order,
archives examined inputs without replacement, and makes the current provider
generation authoritative without dual writes. Recovery or rollback uses the
complete provider-generation backup retained by that migrator while the offline
fence is held; application callers never remove or restore database, journal, or
shared-memory artifacts directly.
...
| Broker | `purge-snapshot`, `purge-automation`, `reconcile-purges` on the opaque repository-review store | Compute detail/eligibility, apply reset/removal, and recover durable intents as whole provider-side commands. Versions, the composite ledger fence, bounded blockers, and safe outcomes cross IPC; paths, target inventories, archive identities, locks, and transactions do not. | `FR-REPOREVIEW-041`, `FR-REPOREVIEW-042` |
...
17. Purge/removal preflight computes one authoritative capability and blocker
    ...
    migration-audited imported legacy copies, or removes both configuration and
    ledger/copies. It
...
| `FR-REPOREVIEW-009` | [web/backend/api/repository_review_automations_test.go](../../web/backend/api/repository_review_automations_test.go), [web/backend/api/repository_review_database_preflight_test.go](../../web/backend/api/repository_review_database_preflight_test.go), [web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-routed-pages.test.tsx), [web/frontend/src/components/repository-reviews/repository-review-runs-page.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-runs-page.test.tsx), [web/frontend/src/components/repository-reviews/repository-review-repositories-page.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-repositories-page.test.tsx)                                                                                                                                   |
...
| `FR-REPOREVIEW-041` | [pkg/repoaudit/purge_test.go](../../pkg/repoaudit/purge_test.go), [pkg/repoaudit/broker_purge_test.go](../../pkg/repoaudit/broker_purge_test.go), [web/backend/api/repository_review_automations_test.go](../../web/backend/api/repository_review_automations_test.go), [web/backend/api/repository_review_coverage_test.go](../../web/backend/api/repository_review_coverage_test.go), [web/frontend/src/components/repository-reviews/repository-review-repositories-page.test.tsx](../../web/frontend/src/components/repository-reviews/repository-review-repositories-page.test.tsx) |
