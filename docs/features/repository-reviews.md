...
association. From the paged Findings surface a user can inspect complete
provenance, discuss findings in a durable
...
| `FR-REPOREVIEW-003` | MUST  | `/repository-reviews` is a standard compact collection of review summaries rendered through the shared `StandardCollectionPage`; opening one review navigates to `/repository-reviews/:id`, whose shared detail shell owns Start, safe Stop, Continue, Run again, live stage/progress/usage, commit selection, and bounded run history. `/repository-reviews/:id/findings/:findingId` independently loads and renders either an immutable review occurrence or repository finding, including commit/blob provenance, match hints, effort, lifecycle, issue, and resolution state. The detail route keeps Findings and Issue previews enabled before, during, and after a run. | Collections stay scannable while every review occurrence and repository finding remains directly addressable with complete provenance and lifecycle authority. |
...
| `FR-REPOREVIEW-017` | MUST  | `/repository-reviews/:id/findings` and its automation-owned API are directly loadable while the review is idle, active, paused, failed, or complete. This review derives occurrence membership only from the selected automation's campaign run IDs and campaign start time; Repository findings shows canonical aggregates. The page displays an empty in-progress state before the first checkpoint, polls only while active, and offers selection, discussion, detail, issue, duplicate, synchronization, and validation actions. Selection spans loaded pages but never implies query-wide selection and is capped by each action's server bound. `/report` redirects to `/findings`. Navigation preserves collection `q`/`view`, findings `scope`/offset, explicit selection, and in-memory scroll; Browser Back restores that state. | Live review findings must be useful from the first checkpoint, distinguish occurrences from repository history, and remain navigable without losing operator context. |
...
| `FR-REPOREVIEW-022` | MUST  | The repository ledger atomically stores immutable review-finding occurrences, stable `rrf_` repository findings, and stable mapping jobs derived from review-finding IDs. Startup creates exactly one job for every unassociated occurrence, returns orphaned `running` jobs to `pending`, and atomically commits association plus job completion. Reprocessing is idempotent. A repository finding stores canonical diagnosis/hints, occurrence and unique commit IDs, path/symbol history, known/new/provisional match state, lifecycle, issue and possible-duplicate associations, resolution history, version, and timestamps. | Existing and future findings must enter one crash-safe lifecycle without a one-off migration mode or duplicate aggregates. |
| `FR-REPOREVIEW-023` | MUST  | Mapping uses exact same-commit fingerprints, deterministic prefilter/conflict checks and thresholds, then an in-memory BM25 index over title and causal identity fields to retrieve at most ten candidates. Remaining ambiguity is adjudicated by an isolated private no-history/no-cache/no-tools reviewer request using opaque candidate IDs and the mapping job's profile/model/account snapshot. Only `same` at confidence at least 0.90 without a hard causal conflict auto-associates; `related` never merges, and `uncertain` creates a provisional finding with visible possible duplicates. Provisional findings cannot draft, link, discover, or post issues until the user chooses Merge or Distinct. | Renames and refactors should match while defects sharing only a file, function, or symptom remain distinct and ambiguous merges stay reversible. |
...
| `FR-REPOREVIEW-028` | MUST  | Repository identity resolution is centralized in `pkg/repoaudit` and shared by controller, API, and gateway for GitHub URL, SSH/SCP, owner/repository shorthand, absolute local path, and run-ID fallback. Automation finding counters are projected from that resolved current-campaign ledger. The canonical UI and API are `/repository-reviews/:id/findings` and `GET /api/repository-reviews/automations/{id}/findings`; `/report` redirects/aliases while preserving query, scope, offset, explicit selection, and scroll state. Findings offers This review and Repository findings views. User-visible repository-review copy uses Findings/review findings/repository findings, never Report, review report, or durable findings. | All entry points must count and display the same ledger, and the route/copy must reflect the occurrence-versus-aggregate model. |

## Data And State Model

| State                        | Shape And Location                                                          | Contract                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Review profile               | `workspace/repository_reviews/profile_rrpf_*.json`                          | Reusable versioned `0600` behavior, one reviewer alias, optional issue-writer alias, editable issue prompt, optional execution account, scope, sizing, and one guard expression without repository or lifecycle state. Blank writer means reviewer.                                                                                                                |
...
| Repository ledger            | `workspace/repository_reviews/repo_*.json` plus summary sidecar             | Exact commit/blob checkpoints, immutable review occurrences, canonical repository findings, mapping and validation jobs, observations, contexts, completed review runs, lifecycle/resolution history, canonical finding-to-draft references, and issue previews/drafts including origins, generation provenance, resolved instructions, safe errors, state, external identity, and version.                                  |
...
Automation-owned detail, findings, finding, issue-generation, preview, and
...
| HTTP         | `GET /api/repository-reviews/automations/{automation_id}/findings?scope={current,all}&offset=...&limit=...`; compatibility alias `.../report`; `GET/PATCH /api/repository-reviews/automations/{automation_id}/findings/{finding_id}` | Page immutable review occurrences and repository findings, load one occurrence or aggregate with its histories, and version-fence allowed lifecycle actions. | `FR-REPOREVIEW-003`, `FR-REPOREVIEW-004`, `FR-REPOREVIEW-017`, `FR-REPOREVIEW-022`–`FR-REPOREVIEW-028` |
...
| UI           | `/repository-reviews`; `/:id`; `/:id/findings`; compatibility redirect `/:id/report`; `/:id/findings/:findingId`; `/:id/findings/:findingId/link-issue`; `/:id/issues`; `/:id/issues/:draftId`; `/repository-reviews/repositories`; `/repository-reviews/profiles`; `/profiles/new`; `/profiles/:profileID`; `/profiles/:profileID/edit` | Standard compact review and profile collections with directly loadable review, findings, occurrence/aggregate detail, issue-link, issue-list, preview, profile-detail, and profile-editor routes. Collection and findings navigation state survives nested routes. Execution account and compatible reviewer/writer selection stay visible; sizing and task-admission controls remain under Advanced. Legacy Results and report URLs redirect to their canonical surfaces. | `FR-REPOREVIEW-003`–`FR-REPOREVIEW-006`, `FR-REPOREVIEW-015`–`FR-REPOREVIEW-028` |
...
10. The This review projection joins occurrences to the selected automation only through
    durable campaign run IDs and campaign start time. All-findings scope reads
...
15. Startup reconciles mapping jobs, then CPU matching performs exact,
    deterministic-threshold, and BM25 stages. Ambiguity snapshots the current
    reviewer/profile/account before isolated opaque-ID adjudication. Association
    and job completion share one write; provisional Merge/Distinct is CAS-fenced.
...
- This review with no checkpointed finding is a valid empty in-progress
  ...
  updated during the campaign. All-findings scope remains bounded and paged.
