...
- Operator logging is independent of hook trust and diagnostic preview policy:
  hook names/stages/actions/event kinds/errors are observed, malformed RPC and
  stderr bytes are bounded observations, and no hook-manager global flag can
  expose invocation or process content.
...
| `FR-HOOKS-009` | MUST | Process hooks launch through the exact immutable subprocess policy paired with their admission generation and receive only policy-captured allowlisted values plus explicit hook `env`; they never append the live ambient environment. Lazy configured initialization acquires or reuses its exact `(config, registry, execution policy, diagnostic policy)` runtime-generation lease before the hook-generation mutex and any factory or process handshake, while reload supplies its own revocable safe-only context for the paused replacement generation; a re-entrant factory therefore cannot deadlock with reload admission pause. Startup and pause owners cannot grant diagnostic authority, lease release revokes that authority before admission is decremented, and every first-party root installs zero so propagation remains dormant. Configured hook initialization/reset is serialized across runtime publication and close, replaces only its own tracked registrations, and closes the retired generation. A manual process hook captures its launch policy and survives later config reload until explicitly replaced/unmounted/closed; configured mounting cannot silently close a same-name manual registration. Timeout mutation/snapshot is race-free and a closed manager rejects later mounts. Manager close elects and marks its primary owner before invoking arbitrary subscription or hook close callbacks outside the election and registry lock; synchronous close reentry returns without deadlock, while the elected caller completes each exact retired cleanup once. An overlapping secondary close may return after closure publication while the elected caller still owns callback cleanup. | Reload and lazy initialization must not deadlock, leak, or switch hook processes, while manual operator-owned hooks retain their stated lifetime and ambient credentials remain absent. |
| `FR-HOOKS-010` | MUST | Every HookManager and ProcessHook operator record uses a fixed safe message and typed projection independent of trust or sensitive-Debug policy. Hook names, stages, actions, sources, and extensible runtime-event kinds use fixed enums or field-specific identity observations; arbitrary errors are never rendered or traversed. Malformed process stdout logs only a bounded hook-message byte observation plus validation error type, and process stderr logs only a bounded stderr byte observation. Initialization, close, observer, interceptor, approval, mutation-discard, timeout, decode, and stderr records never contain raw hook arguments/results, error text, process bytes, dynamic field names, or a sensitive preview. | An extension or process hook controls attacker-influenced text and error objects; diagnostic handling must not become another mutation authority, terminal-forgery path, blocking method call, or secret-bearing stderr channel. |
...
| `FR-HOOKS-009` | [pkg/agent/hook_mount.go](../../pkg/agent/hook_mount.go), [pkg/agent/hook_mount_test.go](../../pkg/agent/hook_mount_test.go), [pkg/agent/hook_process.go](../../pkg/agent/hook_process.go), [pkg/agent/hook_process_test.go](../../pkg/agent/hook_process_test.go), [pkg/agent/hooks.go](../../pkg/agent/hooks.go), [pkg/agent/p015_hook_safe_logging_test.go](../../pkg/agent/p015_hook_safe_logging_test.go), [pkg/agent/runtime_gate_test.go](../../pkg/agent/runtime_gate_test.go), [pkg/agent/runtime_policy_constructors_test.go](../../pkg/agent/runtime_policy_constructors_test.go) |
| `FR-HOOKS-010` | [pkg/agent/hooks.go](../../pkg/agent/hooks.go), [pkg/agent/hook_process.go](../../pkg/agent/hook_process.go), [pkg/agent/p015_hook_safe_logging_test.go](../../pkg/agent/p015_hook_safe_logging_test.go), [pkg/agent/p015_hook_logging_structure_test.go](../../pkg/agent/p015_hook_logging_structure_test.go), [pkg/agent/hooks_test.go](../../pkg/agent/hooks_test.go), [pkg/agent/hook_process_test.go](../../pkg/agent/hook_process_test.go) |
