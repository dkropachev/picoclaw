...
| `FR-WORKFLOW-050` | MUST | A trusted repository model-evaluation workflow may set one exact configured alias through `agent/*.with.model`, use bounded `scope_content: metadata` for no-tool ephemeral metadata decisions without Git-content hydration, or use a one-use `frozen_git` capability for exact source. The runtime validates explicit aliases before provider I/O, preserves account policy, reports the actual concrete model when available, and applies the isolated evaluation system prompt only to the selector, candidate, judge, and analyzer steps of the three built-in evaluation workflow refs. The preflight pins inventory, validates AI-selected opaque candidate IDs, and persists a corpus before candidate calls; batch workflows rebuild the same commit, rotate candidate order, run identical chunks, blind outputs, and judge them; analysis consumes only bounded durable judgments and mapping. Managed candidate execution supports a global concurrency cap plus a per-reviewer cap, preserves declared-plan result order, and emits serialized, payload-free child start/completion observations with exact run, job, and step identity; an observation persistence failure aborts aggregation even when ordinary child failures are configured to continue. | Repository benchmarks need reproducible fair model calls and trustworthy live progress without exposing credentials, mutable checkout authority, ambient agent context, model identity to the blinded judge, or silently judging work whose lifecycle could not be recorded. |
