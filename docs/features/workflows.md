...
An explicitly installed GitHub pull-request review template consumes only an
authenticated, user-targeted review request, checks out the signed head/base
revisions, builds bounded per-file diffs, and runs a no-tool structured review.
Its reserved review draft is captured durably before the event dispatch is
acknowledged; the workflow itself has no GitHub write authority.
Another explicitly installed ordinary template, `github-pr-development`,
matches only authenticated own-PR submitted feedback and performs one declared
read-only current-PR action before emitting a versioned capture marker. The
event dispatcher composes review-draft and development-case capture through an
ordered successful-run sink fanout before acknowledgement. This adds no gate
kind or workflow syntax, and the template contains no agent, checkout, edit,
push, merge, or GitHub write step.
...
Before compilation, a pure policy resolver can combine an ordered global gate
list with one trusted repository override. It preserves global slots for
same-ID replacements, appends new repository gates in order, supports exact
...
- Core types/functions: workflow parser, local ref resolver, validator, run
  ...
  fenced AST renderers define the structured authoring boundary. Built-in
  intake composition additionally uses the template registry,
  `GitHubPRDevelopmentWorkflowYAML`, `SucceededEventRunSink`, and
...
| `FR-WORKFLOW-013` | MUST | Runtime limits enforce configured max concurrent top-level runs, default per-run timeout, max reusable-call depth, and retention pruning for terminal runs. Production launcher and CLI pruning preserve terminal runs with the exact owner-reserved `inline/review-attention-gates/v1` reference, including malformed impostors, because the review case reconstructs its declassified lifecycle and exact response replay from that private run after restart; every ordinary terminal run, including a run merely related to a reserved one, retains configured retention. | Automation must be bounded in resource use and storage without deleting cross-feature authority that still owns a restart-safe replay contract. |
...
| `FR-WORKFLOW-040` | MUST | `picoclaw workflow install github-pr-review` idempotently installs an opt-in `workflows/github-pr-review.yml` that matches only body-authenticated, user-targeted `pull_request.review_requested` events. It acquires the event's signed head repository and exact head/base revisions, rejects revision mismatch, and uses `function/git.diff` to emit production-file unified diffs bounded to 128 KiB per file and 512 KiB in aggregate. The review step has no tools or history, returns the fixed structured schema through reserved `picoclawReviewDraft`, and grants no GitHub write authority. A successful event run captures that draft against the exact event, dispatch, and run before acknowledging the dispatch, so a capture failure is retried instead of losing the review. | Requested reviews need exact-revision evidence, bounded untrusted input, no autonomous publication authority, and durable handoff before event acknowledgement. |
...
`github-pr-development` is another entry in the existing built-in template
registry. It becomes ordinary local YAML only through the existing explicit
template transaction and compatibility revalidation; availability in the
catalog creates no run, configuration, or event state. `SucceededEventRunSink`
and its ordered fanout are Go orchestration around terminal external-event
runs, not YAML-visible nodes and not persisted workflow schema. Event
automation owns each sink's domain state and idempotency record.
...
| CLI | `picoclaw workflow install/list/compatibility/revalidate/validate/reload/run/cancel/retry/status/events/graph` | Install local workflow templates, including `code-review`, `github-issue-triage`, `github-pr-review`, and `github-pr-development`, then manage definitions, compatibility stamps, and runs through the same workflow runtime and file run store used by agent tools. | `FR-WORKFLOW-010`, `FR-WORKFLOW-012`, `FR-WORKFLOW-015`, `FR-WORKFLOW-018`, `FR-WORKFLOW-022`, `FR-WORKFLOW-025`, `FR-WORKFLOW-040`, `FR-WORKFLOW-047` |
...
| Local template | `workflows/github-pr-development.yml` | Explicitly installed authenticated own-PR review-feedback selector, one declared read-only current-PR action, and the exact versioned successful-run capture marker. | `FR-WORKFLOW-047` |
...
16. For the GitHub issue-triage template, route only an explicit
    ...
    prose. For `github-pr-development`, require the exact authenticated own-PR
    review-feedback filter, execute its one declared read-only current-PR call,
    and emit only the exact `v1` capture marker. The separate successful-run
    sink, not the workflow output or MCP step result, owns provider verification
    and durable development-case creation.
...
The installed own-PR development template is a narrower ordinary read-only
composition: workflow code owns its exact trigger/action/output and ordered
successful-run sink orchestration, while event automation independently binds
the signed review to provider state and persists the development case. Neither
the marker nor successful workflow result grants checkout or GitHub-write
authority.
...
Gate policy resolution owns only the deterministic combination of one global
...
different entry. Consumers must attach the existing PR-chat
`ReadOnlySessionRef`, including its exact captured revision, to the
compilation's private root when `RequiresSession` is true and short-circuit an
explicit no-op without creating a workflow run; they must not place it in
`RunRequest.Session` or public inputs. For PR review cases specifically,
durable event automation owns that handoff through its review working-context
requirement: it reloads the SQLite-authoritative review chat, atomically
projects it into one hidden derived agent-owned session whose key is stable
across case versions, verifies exact case/owner identity and complete
post-replacement readback, and supplies the exact snapshot revision plus a
detached JSON-native gate subject while its service-local per-case lock and
exact runtime-generation lease remain held. A competing projection through
...
admission and execution fail closed when that canonical agent, strict snapshot
capability, exact owned session, or expected revision is unavailable or stale.
Agent Conversations owns capture orchestration, the frozen-revision check,
isolated provider-call mechanics, and pseudonymous private
cache/account-affinity identity. Session Memory owns complete structured-locator
enumeration, immutable-reference rewriting, strict `FrozenSet` validation, and
materialization; Tool Execution owns the optional bounded live-media snapshot
reader used only before durable creation. Workflows own persistence and
continuation of the resulting self-contained evidence under workflow engine
v11, run schema v6, and validator v7, and never reacquire those live
capabilities. Security owns the rule that generic observation surfaces cannot
declassify the private workflow context.
...
- Installing `github-pr-development` is likewise idempotent and enables no
  ingress, MCP server, model, checkout, gate, or provider write. Marker absence
  is ignored. An invalid marker or sink error prevents successful dispatch
  acknowledgement; ordered fanout stops on that error, and reconciliation may
  repeat every earlier idempotent sink.
...
| `FR-WORKFLOW-009`, `FR-WORKFLOW-012`, `FR-WORKFLOW-013` | [pkg/workflows/executor_test.go](../../pkg/workflows/executor_test.go), [pkg/workflows/store.go](../../pkg/workflows/store.go), [pkg/workflows/store_test.go](../../pkg/workflows/store_test.go), [pkg/workflows/executor.go](../../pkg/workflows/executor.go), [pkg/config/config_test.go](../../pkg/config/config_test.go), [pkg/reviews/workflow_retention.go](../../pkg/reviews/workflow_retention.go), [cmd/picoclaw/internal/workflow/helpers.go](../../cmd/picoclaw/internal/workflow/helpers.go), [cmd/picoclaw/internal/workflow/retention_test.go](../../cmd/picoclaw/internal/workflow/retention_test.go), [web/backend/api/review_attention_workflow_suppression_test.go](../../web/backend/api/review_attention_workflow_suppression_test.go) |
...
| `FR-WORKFLOW-047` | [pkg/workflows/templates.go](../../pkg/workflows/templates.go), [pkg/workflows/templates_test.go](../../pkg/workflows/templates_test.go), [pkg/workflows/template_catalog_test.go](../../pkg/workflows/template_catalog_test.go), [pkg/workflows/event_dispatcher.go](../../pkg/workflows/event_dispatcher.go), [pkg/workflows/event_dispatcher_test.go](../../pkg/workflows/event_dispatcher_test.go), [pkg/reviews/capture.go](../../pkg/reviews/capture.go), [cmd/picoclaw/internal/workflow/command_test.go](../../cmd/picoclaw/internal/workflow/command_test.go) |
