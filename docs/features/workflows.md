...
The workflow package compiles strict gate workflow v2 specifications into the
same private execution and human-task primitives. Deterministic,
working-context AI, isolated AI, human, and zero stages run as ordered all-of;
the first non-pass result skips later stages, and a durable human response
resumes from the exact cursor without replaying completed work. The PR workspace
selects one lifecycle profile and supplies one frozen subject; workflow code
does not select repositories, persist profiles, or advance product state.
...
- Core types/functions: workflow parser, local ref resolver, validator, run
  ...
  fenced AST renderers define the structured authoring boundary. Gate workflow
  v2 compilation and private continuation define the PR-lifecycle workflow
  boundary.
...
| `FR-WORKFLOW-048` | MUST | Gate workflow v2 defines one strict bounded `GateWorkflowSpec` with canonical ID, name, `attention`, `authorization`, or `classification` purpose, canonical decision point, and one through 64 unique ordered `GateStageSpec` values. Stage kinds are `deterministic`, `ai_working_context`, `ai_isolated_context`, `human`, and `zero`; results use only `pass`, `revise`, `defer`, or `block`. A zero stage is an explicit immediate pass. A deterministic stage evaluates its bounded compiler-owned `inputs` expression as pass or block. AI stages use fixed no-tool working or ephemeral profiles and strict `{outcome,reason,questions}` output. Human stages suspend with a strict object response containing an enum `decision`, optional comment, and optional answer object. `CompileGateWorkflowV2` canonicalizes and digest-pins the normalized spec, freezes subject and stage values in the existing private root, and lowers nonzero model/human stages into one manual inline job in source order. Every later stage has a compiler-generated short-circuiting AND condition over every earlier pass result; false deterministic checks and non-pass AI/human results skip all remaining stages, while human resume continues at the durable cursor without replay. The job retains one private `gate_passed` output for the product owner to consume; private admission accepts that sole compiler-stamped output while default private-run projection still redacts it. All-zero and deterministic-only workflows resolve immediately without a run. Unknown fields, duplicate JSON keys, trailing values, unsafe IDs or agent IDs, unsupported field/kind combinations, missing human questions, missing AI criteria, invalid/missing deterministic subject paths, cyclic/non-JSON content, mixed working-session owners, and every text/count/JSON bound fail before run creation. Missing-workflow purpose defaults, gate-profile selection, mandatory technical invariants, and product state transitions remain outside the compiler. The expression evaluator adds only quote-aware, bounded, left-to-right, short-circuiting `and`; OR and parentheses remain unsupported. Gate workflow v2 is the sole PR-lifecycle gate contract; the older generic gate API does not provide a PR-workspace compatibility path. | PR lifecycle decisions need customizable deterministic, AI, human, and mixed staged gates with typed terminal dispositions, durable suspension, private evidence, and explicit automation without a second executor or accidental continuation after a failed stage. |
...
| Go API | `GatePurpose`, `GateOutcome`, `GateWorkflowSpec`, `GateStageSpec`, `GateWorkflowV2Compilation`, `ValidateGateWorkflowSpecV2`, `CompileGateWorkflowV2`, `ResolveGateWorkflowV2Outcome` | Validate, canonicalize, digest, and compile one explicit staged all-of gate workflow into an immediate typed result or the existing private manual workflow/human-task runtime, then resolve the first non-pass or final pass from the exact private run. | `FR-WORKFLOW-048` |
...
Gate workflow v2 owns validation, canonicalization, private compilation,
ordered all-of execution, and human-task continuation for one already-selected
specification and frozen subject. The PR workspace owns repository/profile
selection, subject construction, stage-result interpretation, and lifecycle
transition. Working-context stages may use only an explicitly captured owner
snapshot; isolated, deterministic, human, and zero stages gain no ambient
session. Generic workflow HTTP, SSE, graph, cancel, retry, and task routes hide
every `inline/pr-lifecycle/` run and its private relationships.
...
| `FR-WORKFLOW-048` | [pkg/workflows/gatetypes/v2.go](../../pkg/workflows/gatetypes/v2.go), [pkg/workflows/gatetypes/v2_test.go](../../pkg/workflows/gatetypes/v2_test.go), [pkg/workflows/gates_v2.go](../../pkg/workflows/gates_v2.go), [pkg/workflows/gates_v2_result.go](../../pkg/workflows/gates_v2_result.go), [pkg/workflows/gates_v2_test.go](../../pkg/workflows/gates_v2_test.go), [pkg/workflows/expression.go](../../pkg/workflows/expression.go), [pkg/workflows/gates.go](../../pkg/workflows/gates.go), [pkg/workflows/validator.go](../../pkg/workflows/validator.go), [pkg/workflows/private_context.go](../../pkg/workflows/private_context.go), [pkg/workflows/executor.go](../../pkg/workflows/executor.go) |
