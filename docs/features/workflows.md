...
independently re-evaluated by the server before any run is created. Run detail
also exposes independent cancellation/completion lifecycle fields and trusted,
payload-free event/dispatch provenance without inferring authority from legacy
run input or event maps.
...
| `FR-WORKFLOW-012` | MUST | Runs can be canceled and retried. Canceling a nonterminal run atomically marks it canceled, records `cancel_requested_at`, `cancel_reason`, and `completed_at`, propagates cancellation to active reusable children, and stops before later jobs or steps; a terminal run remains unchanged. Retry creates a new run linked through `retry_of_run_id`, uses the loaded previous run's authoritative workflow ref, inputs, event, session, and delivery, and retains any trusted origin rather than mutating the original run. | Operators need safe intervention without losing audit history or retargeting a retry through current browser selection. |
...
| `FR-WORKFLOW-033` | MUST | An authenticated dashboard user launches the selected published workflow, retries a selected terminal run, cancels a running run, or inspects its lifecycle and external-event relationships. | Run and Retry are enabled only for the exact current ready published dependency report and submit its opaque revision through optional `expected_dependency_revision`; the server always performs a fresh stable `FR-WORKFLOW-029` evaluation before run creation, compares a supplied revision, and admits only a ready report. Retry first loads the named previous run and uses its `workflow_ref` as authoritative regardless of route selection or request fields. Cancel uses an accessible confirmation dialog that requires an explicitly present, trimmed nonblank valid-UTF-8 reason of at most 1,024 bytes; an HTTP client that omits `reason` retains the legacy reasonless behavior, while an explicitly blank reason is invalid. Run detail separately displays `cancel_requested_at`, `cancel_reason`, and `completed_at`. A server-owned `RunOrigin` contains only `kind`, `event_id`, optional `dispatch_id`, and `root_run_id`: production event dispatch requires `external_event` plus exact `ev_`/`dsp_` 32-lowercase-hex IDs; event-parity testing requires `external_event_draft_test`, the exact event ID, and no dispatch; root IDs are `wr_` plus one or more ASCII letters, digits, `_`, or `-`, at most 1,024 bytes. The first root receives its own run ID and reusable children and retries inherit the unchanged origin/root identity. The UI uses only that validated projection for ID-only event, dispatch, and family-root links. | The fresh readiness check and optional revision comparison complete before any run record is created. Cancel persists its reason and independent request/completion timestamps through the ordinary atomic run update. Origin is created only by trusted dispatcher or server-resolved draft-test paths, persisted with normal run state, and propagated without copying an event payload. Older runs with no origin remain readable and simply expose no external relationship; malformed persisted origin is omitted from the browser projection and is never reconstructed from `event`, `inputs`, delivery, session, error, or route state. | A supplied revision mismatch returns `409 dependency_revision_mismatch`; a current missing, invalid, disabled, structurally blocked, or runtime-blocked published closure returns `409 workflow_dependencies_not_ready`; unavailable stable evaluation returns `503 dependency_check_unavailable`; and a missing retry source returns `404`. None creates a run. Browser Run, Retry, and draft-test request DTOs reject an `origin` field, and manual payload lookalikes never become trusted provenance. Explicit blank, invalid-UTF-8, or over-1,024-byte cancel reasons return `400 invalid_cancel_reason`; null, malformed, trailing, or unknown request fields return `400 invalid_cancel_request`; a request over 16 KiB returns `413 cancel_request_too_large`; none cancels the run. Relationship targets may independently be pruned and return not found. URLs and labels contain only validated refs/IDs: payloads, inputs, secrets, delivery data, cancellation reasons, errors, dependency details, and cursor/lease tokens never enter them. | Action-time readiness prevents a stale green dashboard or changed reusable dependency from launching work; explicit cancellation context and typed provenance make incident navigation auditable without turning untrusted workflow data into authority or browser history. |
...
drops a malformed persisted origin and never synthesizes one from the larger
event or input snapshots.
...
| Go API | `pkg/workflows.Parse`, `Resolver.ResolveLocal`, `Validate`, `LoadRunnableLocalSnapshot`, `Executor.Run`, `Executor.Retry`, `FileRunStore`, `RunOrigin`, `RunRequest.Origin`, `ProjectWorkflowRunForBrowser`, `MatchChannelMessage`, `MatchCommandMessage`, `MatchRuntimeEvent`, `EvaluateEventTrigger`, `MatchEventTrigger`, `EventWorkflowRouter`, `EventWorkflowDispatcher`, `BuildRunGraph`, `ReloadLocal`, `ListWorkflowTemplates`, `InstallWorkflowTemplateWithCompatibility`, `CheckWorkflowDependencyClosure`, `ResolveWorkflowDependencyReadiness`, `PublishWorkflowDevelopmentFenced` | Parse GitHub-shaped YAML, normalize local reusable refs, reject unsafe refs, validate an exact compatibility-stamped byte snapshot, inspect and transactionally install local workflow templates, project and revise typed event triggers without replacing unrelated YAML, inspect structural/runtime dependency readiness, evaluate process-local and durable triggers, construct and propagate trusted payload-free external-event run origin, durably route/reconcile external-event runs, run/retry/cancel workflows, project safe browser run state, build run graphs, recover and atomically publish fenced development state, reload definitions, and persist run state through guarded internal roots. | `FR-WORKFLOW-001` through `FR-WORKFLOW-016`, `FR-WORKFLOW-018`, `FR-WORKFLOW-022` through `FR-WORKFLOW-031`, `FR-WORKFLOW-033` |
...
21. Before dashboard Run, require the selected ref's exact current ready report
    ...
    named previous run, ignore route/current-selection ref as authority, check
    that previous run's published ref, and send that report's revision. For
    either request, the server loads a stable current configuration and exact
    published bytes, recomputes the complete dependency report, compares a
    supplied revision, checks readiness, then performs existing compatibility
    admission and creates the run. Omission of the optional revision supports
    older HTTP clients but never skips the fresh server gate. Return fixed
    conflict codes for a stale/not-ready report and service unavailable for a
    report that cannot be completed, with no run record in every rejection.
22. Before an externally originated run is created, accept origin only from the
    ...
    unchanged into reusable children and retries. Browser Run/Retry/draft-test
    DTOs cannot set it; project only a structurally valid persisted origin and
    leave absent or malformed legacy provenance unlinked. For dashboard
    cancellation, collect a labelled explicit reason, trim and UTF-8-byte-check
    it in both client and server, then atomically persist reason,
    `cancel_requested_at`, and `completed_at`; an omitted HTTP reason follows
    the pre-existing reasonless behavior.
...
- Dashboard cancellation cannot be submitted until its labelled reason is
  ...
  mutation. Malformed/null/unknown/trailing request data returns
  `400 invalid_cancel_request`, and a request body over 16 KiB returns
  `413 cancel_request_too_large`. An HTTP request that omits `reason` preserves
  the legacy reasonless behavior for older clients.
...
- Retry always resolves its authoritative workflow ref from the persisted
  previous run. A different selected workflow, route ref, or body field cannot
  retarget it; the retry link and inherited trusted origin remain auditable.
...
- A trusted production event origin requires matching exact event/dispatch
  ...
  retries must retain the same valid root. An invalid supplied internal origin
  fails before run creation; an absent or malformed persisted legacy origin
  remains readable but produces no browser relationship and is never inferred
  from lookalike `event` or `inputs` data.
...
- The dashboard's ready report is advisory until the Run/Retry handler completes
  a fresh stable published-ref evaluation. A supplied revision mismatch and a
  currently not-ready closure return `409 dependency_revision_mismatch` and
  ...
  optional revision remains compatible but does not bypass this check, and
  every rejection occurs before run creation.
