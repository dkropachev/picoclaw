...
| HTTP | `/api/workflows/development/event-trigger/*`, `/api/workflows/development/test` | Stateless server-parsed event-trigger projection/revision, deterministic metadata-only match preview, and event-ID-only draft-test context resolved through the protected live gateway. | `FR-EVENT-AUTOMATION-036` |
...
| Frontend | authenticated `/events` dashboard route | Responsive URL-bound event and global dispatch master/detail inspection, exact event/dispatch/workflow/run relationship links, selected-event dispatch history, explicit exact-text payload reveal, and warned non-retried replay through launcher-owned authenticated endpoints. | `FR-EVENT-AUTOMATION-034`, `FR-EVENT-AUTOMATION-037` |
...
| Frontend | authenticated `/agent/workflows` event-trigger builder and captured-event test context | Server-projected deterministic filter controls, raw-YAML fallback, payload-free event selection and field checks, explicit ephemeral payload reveal, and event-ID-only production-parity draft testing. | `FR-EVENT-AUTOMATION-036` |
...
| Go API | `EvaluateEventTrigger`, `EventWorkflowRouter`, `EventWorkflowDispatcher`, `RunRequest.OnRunPersisted`, `LoadRunnableLocalSnapshotWithRevision`, `EventContextFromEnvelope` | Produce deterministic field-level match diagnostics through the same evaluator used by routing, claim one item, compatibility-check and revision-bind the exact loaded workflow bytes, durably fan out deterministic dispatches, reject content drift or a no-longer-matching persisted event, pass the same accepted snapshot into execution, reconcile deterministic runs, link a newly persisted run before effects, renew long leases, and build the detached redacted workflow context shared by dispatch and event-parity draft testing. | `FR-EVENT-AUTOMATION-014` through `FR-EVENT-AUTOMATION-018`, `FR-EVENT-AUTOMATION-020`, `FR-EVENT-AUTOMATION-021`, `FR-EVENT-AUTOMATION-036` |
...
16. If no run exists, load one exact current runnable workflow snapshot, reject
    ...
    context, renew the dispatch lease, and call the shared executor with that
    exact workflow object, run ID, and an `OnRunPersisted` callback. The executor
    exclusively creates the run, invokes the callback to link and renew the
    dispatch, and only then starts lifecycle callbacks or workflow steps.
...
30. The workflow editor sends draft YAML to a stateless server projector,
    ...
    draft run with empty delivery and no dispatch ID. Persist the selected
    event ID beside the draft-key-fenced test snapshot. AI authoring uses the
    event schema as instructions but runs with no tools/history and receives
    only bounded structural failure context, never captured payload values.
