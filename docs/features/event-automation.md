...
narrow uncomposed prerequisites; active mixed-gate execution, push runtime,
...
60. For the initial publication-gate processor, require one caller-supplied
    ...
    live claim after transient failure until the separate safe-requeue slice;
    terminalize only an existing store-proven provider/local conflict,
...
The remaining `#126b2` composition must own active gate launch/response, safe
transient requeue, the generation-owned publisher, exact `PushPinnedLine`
adapter, worker-level observer wiring, and unknown-outcome reconciliation.
...
| Active mixed-gate execution/PR-chat bridge, generation-owned push worker and adapter, provider-observer worker wiring, safe transient requeue, and unknown-outcome reconciliation scheduling | Remaining `#126b2` / later requirements | Not implemented. It must consume the schema-v18 store and provider observers through distinct least-authority interfaces and receives no authority merely from a schema-v15/16 decision, private workflow run, completed attempt, ledger row, gate answer, review outcome, primitive push result, provider observation, processor result, or browser-visible `local_ready`. |
