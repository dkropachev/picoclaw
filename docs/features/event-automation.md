...
effective repository preview, without launching a decision. Automatic
review-event triggering remains a separate later stage.
...
- Core types/functions: `Envelope`, `Actor`, `Subject`, envelope normalization,
  ...
  transport, and the review-attention policy draft/model helpers.
...
The editor intentionally stops at configuration: automatic submitted-review
triggering, attention navigation into a PR chat, and user-driven launch remain
future stages.
...
- Opening, editing, validating, previewing, refreshing, or discarding the policy
  ...
  explicitly reports that policies are not yet triggered automatically.
