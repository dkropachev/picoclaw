...
started state remains inert for the later expiry transition.
Production generation-owned claiming/renewal and publisher cleanup,
unknown-outcome reconciliation, acknowledgement, and merge remain later
...
the queue loop. PRs `#137` and `#138` must add read-only unknown-outcome
reconciliation, then generation-scoped claiming/renewal, publisher cleanup,
and complete worker-level observer wiring.
