...
- Core types/functions: `Envelope`, `Actor`, `Subject`, envelope normalization,
  ...
  Workspaces' exact disposable candidate-materialization callback.
...
Schema v14 adds controller-private
`pr_development_repair_orchestrations`, keyed one-to-one by repair attempt. Its
phase, claim, immutable provider/workspace pin, source tree, controller binding,
model markers, exact local-CI receipt, Park operation, ledger result, timestamps,
and hashes form the restart checkpoint for the composed worker. Raw scheduling
tokens and failure replay proofs remain private; read-only lookup redacts the
live claim token. Only safe pre-effect phases are reclaimable. Receipt fields
cover every terminal CI status and bind the exact controller revision, line,
...
fence, orchestration, and reservation retirement. Migration creates no
orchestration, receipt, claim, ledger entry, attempt outcome, or controller.
...
54. When opening schema v12, create and structurally validate empty schema-v13
    ...
    Preserve an already completed legacy owner attempt exactly. If the mutation
    ...
    the first three kinds. Exact complete replay is no-write; changed or
    non-immediate replay fails closed. Store methods execute no Git, model, CI,
...
56. When opening schema v13, create and structurally validate the empty
    schema-v14 repair-orchestration table before advancing `user_version` and
    synthesize no checkpoint, receipt, attempt outcome, or ledger entry. A
    trusted worker claims only a provider-thread latest queued attempt, or an
    expired exact `bootstrap`, `edited`, or `validated` checkpoint; the claim
    suppresses the legacy repair scanner. Expire `editing` to
    ...
    Stop future mutation heartbeats immediately before the bounded prepared
    Park while continuing the orchestration heartbeat. Finalize Park only when
    the post-Park snapshot exactly equals its preview, and in that same SQLite
    transaction finalize the operation, append the fence and next attempt
    ledger row from the receipt, complete the public attempt, advance the
    session once, retire mutation authority and the scheduling claim, and enter
    ...
    later gates decide attention and publication. Before controller or model
    authority exists, a bounded bootstrap failure may atomically fail the
    attempt, clear suppression, and release a newly acquired ordinary pin.
    After model or Git ambiguity, never guess or silently repeat an effect;
    leave the durable checkpoint for its exact recovery path.
...
local commit effect, but deliberately leaves it unwired until the controller
consumes exact `FR-EVENT-AUTOMATION-062` validation evidence and prepares the
write-ahead commit intent. Git Workspaces owns the candidate, reservation lock,
commit-object verification, compare-and-swap, and index reconciliation; Event
Automation owns the later attempt ledger and recovery state that decide when
this primitive may be called.
...
projector is not yet called by chat or repair execution: the controller-aware
worker in `#120` will load it only after consuming the now-defined local-CI,
commit, and park contracts, and a later review worker will supply the structured
review result.
...
eventing; neither store layer calls Git. These primitives close effect
ambiguity but do not yet wire a recovery worker, edit loop, validator consumer,
reviewer, gate, or publisher.
...
gate, private context, event, or dispatch. `#120` must explicitly consume this
evidence; a `passed` result does not mutate a controller or authorize Commit,
Park, review, provider access, or publication.
...
| Repair-attempt orchestration, model edit loop, operational schema-v11 ledger append/checkpoint use, and recovery-worker scheduling | `#120` | Must consume the v13 prepare/finalize protocol; it cannot replace it with an in-memory sequence. |
...
| `FR-EVENT-AUTOMATION-063` | [pkg/eventing/pr_development_orchestration_schema_sqlite.go](../../pkg/eventing/pr_development_orchestration_schema_sqlite.go), [pkg/eventing/pr_development_orchestration_store_sqlite.go](../../pkg/eventing/pr_development_orchestration_store_sqlite.go), [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_operation_store_sqlite.go](../../pkg/eventing/pr_development_operation_store_sqlite.go), [pkg/prdevelopment/controller_identity.go](../../pkg/prdevelopment/controller_identity.go), [pkg/prdevelopment/controller_evidence.go](../../pkg/prdevelopment/controller_evidence.go), [pkg/prdevelopment/controller_heartbeat.go](../../pkg/prdevelopment/controller_heartbeat.go), [pkg/prdevelopment/controller_effects.go](../../pkg/prdevelopment/controller_effects.go), [pkg/prdevelopment/thread_context.go](../../pkg/prdevelopment/thread_context.go), [pkg/prdevelopment/thread_context_loader.go](../../pkg/prdevelopment/thread_context_loader.go), [pkg/prdevelopment/localci](../../pkg/prdevelopment/localci), [pkg/gateway/pr_development_local_ci.go](../../pkg/gateway/pr_development_local_ci.go) |
