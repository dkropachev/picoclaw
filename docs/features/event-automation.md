...
provider observation, gate/push runtime composition, acknowledgement, and merge
...
call it, acquire transport authority, refresh provider state, alter
...
but no runtime component calls it or invokes an effect. The remaining `#126b2`
composition must own the gate launcher/response bridge, generation-owned
publisher, exact `PushPinnedLine` adapter, observer wiring, and unknown-outcome
reconciliation worker. Review acknowledgement and merge remain further separate
features whose concrete user-facing actions and policy have not been chosen;
the journal deliberately does not guess them.
...
| Mixed-gate execution/PR-chat bridge, generation-owned push worker and adapter, provider-observer wiring, and unknown-outcome reconciliation scheduling | Remaining `#126b2` / later requirements | Not implemented. It must consume the schema-v18 store and provider observers through distinct least-authority interfaces and receives no authority merely from a schema-v15/16 decision, private workflow run, completed attempt, ledger row, gate answer, review outcome, primitive push result, provider observation, or browser-visible `local_ready`. |
