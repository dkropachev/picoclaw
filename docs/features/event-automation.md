...
| HTTP | `/api/workflows/development/event-trigger/*`, `/api/workflows/development/test`, `/api/workflows/runs*` | Stateless server-parsed event-trigger projection/revision, deterministic metadata-only match preview, event-ID-only draft-test context resolved through the protected live gateway, and safe workflow run projection whose origin is validated against every retained parent/retry ancestor while respecting independent pruning boundaries. Browser request DTOs cannot supply origin. | `FR-EVENT-AUTOMATION-036`, `FR-EVENT-AUTOMATION-038` |
