...
The default authenticated `/reviews` surface is a repository-first portfolio
over captured outgoing-review and own-PR-development cases. It preserves each
case and keeps review and development roles distinct while separating finished
PicoClaw work from provider-verified or explicitly captured pull-request
closure. Bounded live provider status is loaded only for captured review-role
items, and a selected review case may load the bounded live snapshot defined by
`FR-EVENT-AUTOMATION-079`; own-PR development detail remains capture-only.
Canonical repository, pull request, filter, role, and case navigation is
shareable, while cursors, loaded projections, drafts, and provider capabilities
remain memory-only.

For a selected case, the same authoritative detail read may additionally
declassify one bounded `local_development` snapshot for its latest repair
attempt. It reports only the attempt lifecycle, exact local candidate
commit/no-change fact, terminal local-CI status and opaque plan/result
fingerprints, reservation-free local-review progress/result summary, and a
derived local-readiness boolean. That boolean means only that the exact local
candidate has green persisted CI and a passed paired local review; it is never
provider refresh, push, publication, acknowledgement, or merge authority.
...
gates, checkout, editing, validation, and the effect-free publication journal
remain separate explicit stages; provider publication execution,
acknowledgement, and merge remain later.
...
The private PR-development controller now has a write-ahead boundary for every
retained-line `Adopt`, `Resume`, `Commit`, and `Park` effect. One schema-v13
operation row owns the exact request, result, and any reclaimable recovery
claim, so an expired in-flight effect never becomes an untraceable Git
ambiguity or a second pending schema-v12 recovery. Adopt and Resume recovery
use the Git-workspace composite old-to-fresh reservation transitions, Commit
rotates before exact deterministic replay, and Park exactly replays the old
...
`.git`-free disposable trees, discovers a small deterministic workflow-like
lint/build/test plan from both snapshots, and runs the candidate plan only in a
mandatory local sandbox. `.picoclaw/ci.yml` or `.picoclaw/ci.yaml` is
authoritative when present; otherwise discovery prefers one bounded
repository-native quick profile and consults supported pull-request GitHub
workflows only as a fallback. Because every accepted command runs in a fresh
filesystem and process sandbox, a GitHub job with multiple executable steps is
rejected instead of silently losing shared job state. A definition change
between parent and candidate is reported as `plan_changed` and can never become
green; dependency-only changes invalidate environment/result identity without
inventing a plan change. Exact-manifest discovery plans and execution evidence
persist owner-locally. Production exact-success result reuse is currently
disabled while host toolchains and controller-provided dependency mounts remain
mutable and lack complete immutable manifests. This primitive neither wires
dependency provisioning or the repair worker nor advances controller, ledger,
commit, park, review, workflow, provider, or browser state.

Schema v14 composes those private primitives in one generation-owned trusted
repair controller. It atomically marks only new provider sessions eligible;
unmarked migrated controllerless sessions and their later attempts remain
legacy-owned, while an exact migrated `ready` controller owner is independently
eligible from its retained controller evidence. The controller claims only
those exact owners, heartbeats the
scheduling checkpoint independently from mutation ownership, reconstructs a
bounded ordered thread context with logical ledger compaction, and re-verifies
the selected provider case before touching its retained checkout. It binds the
exact pin, controller, model prompt/result, candidate, and terminal local-CI
attestation in durable phases; every valid status, including non-green, then
...
schema-v18 supplies only durable publication admission and journaling. Fresh
...
already-claimed phase dispatch now exist as internal prerequisites. A fenced
push-ready handler now authenticates one exact live ready-origin claim, repeats
the pinned provider observation, durably records the exact parked-line request
before invoking Git at most once, classifies the outcome deterministically, and
finalizes it while exact journal authority remains live. Definitively stale
started state is now expired and inspected only by the restart-safe head-only
reconciliation worker. The production gateway now composes the complete
publication and reconciliation graph only after every policy, exact run-store,
CI-evidence, parked-workspace, owner-runtime, full/head provider, narrow Git,
store, and outer-generation dependency exists. It then runs separate
generation-leased claim and reconciliation loops and drains both before their
service generation closes. Provider review acknowledgement and pull-request
merge remain later independently approved and fenced stages.
...
work under the active gateway generation, performs only the exact kind-specific
Git reconciliation, and, except for direct Park, durably checkpoints that
result before retiring the fresh bearer through retained-line suspension. The
controller is explicitly `suspension_pending` during that replayable handoff
and `suspended` after every raw recovery bearer is erased. A later queued repair
must persist and exactly complete a fresh suspended resume before any model can
continue the preserved candidate. Both phases are locally not ready and grant
no review, attention, provider, push, publication, acknowledgement, or merge
authority. Legacy unbound v12 recovery stays untouched until a separately
approved idempotent retirement policy exists.

Schema v18 adds the all-private publication journal without enabling a
publisher. The same transaction that accepts an exact green `passed` local
review appends one `pending` publication occurrence containing the immutable
controller, orchestration, CI, ledger, and parked-line evidence that earned it;
migration never backfills older reviews. The store can lease pre-effect work,
pin one exact operator policy, gate subject, provider observation, and private
decision-run link, release scheduling ownership while a human gate waits, and
write-ahead one complete `PushPinnedLine` request immediately before a future
caller could perform that effect. Only `push_started` briefly excludes a new
controller mutation; every earlier phase keeps the retained branch parked and
reservation-free, and a later repair may supersede stale publication work. An
expired `push_started` record becomes `outcome_unknown` and can be resolved only
from independently proven read-only remote evidence, never by automatically
repeating the push. This slice runs no gate, model, provider read/write, Git,
gateway, or worker and defines no review-acknowledgement or merge behavior.

The publication store now also exposes one narrow gate-context snapshot for a
trusted coordinator holding an exact live claim that originated from
`pending`. One SQLite read transaction authenticates that lease and returns the
claim-redacted publication together with the selected case ordinal, complete
integrity-checked conversation and transcript digest, exact passed
attempt/review tail, case, provider thread, authority-redacted repair session,
controller, fence, orchestration, and full ledger. The first subject pin can use that snapshot's
conversation version and digest as a transaction-time compare-and-swap; an
exact already-pinned replay remains idempotent after later chat. This storage
slice neither duplicates the conversation schema nor runs a provider,
workflow, gate, model, Git, filesystem, gateway, or worker effect.

The first uncomposed publication-gate processor now advances an already
claimed-from-`pending` occurrence through only the zero-gate path. A narrow
store authenticator first proves the exact live claim and current high-water
and returns only the redacted publication plus its exact repository policy
selector, without conversation or other rich gate context. The processor captures
and pins the selected reusable rule set, strictly decodes that pin,
uses the ordinary gate compiler to prove an empty, disabled, or all-zero
composition, pins one small versioned conversation-fenced audit subject, then
...
`push_ready` handlers. The push-ready handler now supplies that third internal
phase: it renews and authenticates the exact ready-origin claim, repeats the
full provider observation against its immutable pin, derives only the canonical
parked-line request, and commits `StartPRDevelopmentPublicationPush` before
calling `PushPinnedLine`. Only a start that returns `newlyStarted=true` may call
Git; a historical or lost-response replay never does. Renewal ownership is
drained from the pre-effect queue heartbeat before the durable start and then
continued only through the push journal. The retained branch remains parked and
reservation-free while the exact Git result or bounded uncertainty is finalized.
This internal handler still does not install the later generation-owned claim
loop or gateway composition.
...
A trusted internal launcher can now resolve one frozen reusable attention rule
set for an exact repository, case version, and decision point, compile any
...
operator configuration now persists a built-in, fixed-name `Default` set plus
independently named reusable sets. One set is selected as the fallback, and
exact repositories may be assigned to any set; a set can serve many
repositories. The active gateway generation resolves an immutable
case-insensitive repository assignment catalog,
...
visual editor for that complete catalog. Set names are fixed after creation;
duplicating a set creates an independent copy with a new permanent name. The
editor presents `review.submitted`,
`pr_development.review_attention_required`, and
`pr_development.before_push` as known decisions alongside custom canonical
decision points, with explicit default and repository assignments and ordered
mixtures of all four gate kinds, without
launching a decision or creating a second policy tree. When this
...
- Core types/functions: `Envelope`, `Actor`, `Subject`, envelope normalization,
  ...
  `/reviews`.
  Retained-line effect recovery additionally uses the narrow
  `eventing.PRDevelopmentControllerOperationStore`, private schema-v13
  `pr_development_controller_operation_intents`, and Git-workspace
  `RecoverPinnedLineAdoptReservation` /
  `RecoverPinnedLineResumeReservation` composites; it introduces no generic
  operation executor. Local validation additionally uses the controller-only
  `pkg/prdevelopment/localci` plan/discovery/runner/cache boundary and Git
  Workspaces' exact disposable candidate-materialization callback. Composed
  repair execution additionally uses
  `eventing.PRDevelopmentRepairOrchestrationStore`, schema-v14
  ...
  review iteration. Recovery execution additionally uses a narrow unified
  `eventing.PRDevelopmentControllerRecoveryScanner`,
  `eventing.PRDevelopmentControllerSuspensionExecutionStore`, and
  `eventing.PRDevelopmentControllerSuspendedResumeRecoveryStore` with tagged
  v12/v13 claims, append-once suspension checkpoints, suspension finalization,
  and suspended-resume prepare/finalize operations;
  `prdevelopment.ControllerRecoveryWorker`;
  and the exact Git-workspace rotation, composite recovery, Commit, Park,
  suspension, and suspended-resume methods. These names describe the required
  capability split rather than a generic operation executor.
  Publication durability additionally uses the private
  `eventing.PRDevelopmentPublicationReader`,
  `eventing.PRDevelopmentPublicationQueue`,
  `eventing.PRDevelopmentPublicationPushJournal`,
  `eventing.PRDevelopmentPublicationOutcomeReconciler`, and
  `eventing.PRDevelopmentPublicationDecisionRunStore`; schema-v18
...
- Runtime ordering: resolve disabled-safe config, normalize and validate an
  ...
  Before any controller-owned retained-line Adopt, Resume, Commit, or Park,
  prepare its exact schema-v13 operation; after the external Git result,
  finalize only that result. If its mutation lease expires, claim recovery on
  that same operation, reconcile its kind-specific Git effect, and finalize
  through the claim. Park finalization atomically performs every local database
  terminal transition before a reservation-free review can be claimed. For
  local CI, first materialize and hash the exact parent and candidate under the
  reservation operation lock, discover both definition snapshots without
  executing them, apply explicit-plan authority then native-quick-profile and
  supported GitHub fallback precedence, and reject any definition drift. Reuse
  the persistent discovery record only for the exact two manifests and
  discovery/plan versions, resolve the exact environment, and run every
  required step in the disposable sandbox. Exact-success result reuse remains
  conditional on an immutable complete environment identity and is disabled
  for the current mutable-host backend. Return only canonical plan/result
  evidence. For an eligible provider repair, claim and heartbeat its durable
  orchestration, reconstruct or compact only the exact reviewed context prefix,
  reverify the provider case, establish the exact workspace/controller line,
  and persist model start/result markers around the edit-only invocation. Under
  the same live mutation owner, snapshot and attest the candidate, record every
  terminal CI status without relabeling, deterministically Commit or prove
  no-change, preview the immutable review handoff, and keep both leases renewed
  ...
  Independently, claim the oldest eligible bound recovery under one exact
  runtime generation and renewable recovery token; execute only its exact
  Adopt/Resume/Rotate-plus-Commit/Park effect. For non-Park, transactionally
  checkpoint that result and enter `suspension_pending` before Git suspension,
  then finalize the exact suspension into bearer-free `suspended`. Park instead
  uses its existing direct terminal transaction. Before any later repair model,
  prepare an exact fresh suspended-resume intent, replay Git resume, and
  atomically install the sole new mutation owner.
  On an exact green passed-review completion, append its review ledger row,
  finish the fence/controller, and create one publication occurrence in the
  same immediate transaction. Pre-effect claims may pin immutable policy,
  subject, provider, and decision-run facts and may release ownership into
  `gate_waiting` or `push_ready`. Immediately before any later caller could push,
  atomically revalidate the complete local high-water and fresh provider pin,
  persist the canonical push request, and enter `push_started`; serialize that
  transition against controller mutation. Journal an exact proven terminal
  result, or convert expired started work to `outcome_unknown`; accept only a
  distinct minimal fresh read-only remote-head proof of the desired tip for
  later reconciliation, without rechecking or replacing the original review
  pin, and never
  schedule another push from unknown state.
- Non-obvious constraints: deduplication is scoped by source and connector;
  ...
  An active schema-v13 operation is the sole live recovery claim for its
  expired effect: it never stages a pending schema-v12 intent, although
  Adopt/Resume/Commit recovery finalization appends linked already-finalized
  v12 evidence for audit continuity. Recovery claims authorize only exact
  reconciliation, never general mutation, and Park never receives a
  replacement bearer. Persistent local-CI discovery is an optimization keyed by
  the exact parent/candidate manifests and implementation versions. Any future
  exact-success result reuse is separately conditional on a complete immutable
  toolchain/dependency identity; the current production backend deliberately
  does not reuse passing results from mutable host toolchains or dependency
  mounts. Definition drift, zero or incomplete plans, changed candidate or
  environment identity, corrupt evidence, sandbox unavailability, or any
  non-success can never be normalized into a pass. CI discovery/execution is
  not a workflow run or an attention gate and grants no later lifecycle or
  publication authority. Orchestration scheduling and controller mutation use
  independent renewable leases; both must remain live during model, snapshot,
  CI, and Git work. A scheduling phase is a restart checkpoint, not proof that
  no operation intent exists. Every terminal CI status is truthful evidence,
  ...
  `attention_required` rather than a false pass or retry loop. Recovery claims
  are scheduling-only and provider-independent; non-Park reconciliation cannot
  expose fresh mutation authority to a model between recovery and suspension.
  A schema-v12 row lacking an exact bound-line fence is never selected by this
  worker because release of a successfully rotated unbound bearer is not an
  idempotent lost-response protocol. `suspension_pending` and `suspended` are
  always locally not ready. Publication claims are likewise scheduling-only:
  pending, claimed, `gate_waiting`, and `push_ready` never reserve the checkout
  or freeze later repair. Only `push_started` is a short mutation-exclusion
  fence, and its expiry is terminal uncertainty rather than retry authority.
  Policy, subject, provider, request, result, decision, claim, and diagnostic
  bytes remain private. A publication row, gate result, passed review, or
  browser-visible `local_ready` never supplies a provider/Git capability, and
  this foundation deliberately leaves acknowledgement and merge semantics
  undefined.
...
| `FR-EVENT-AUTOMATION-046` | MUST | An authenticated operator reads or replaces the complete review-attention catalog, or an enabled event/workflow gateway generation starts or reloads with that catalog. | Current configuration stores bounded `reviews.attention.rule_sets`, one `default_rule_set_id`, and case-insensitive exact `repository_assignments`. The fixed ID `default` and name `Default` always exist; its empty initial rule map makes every decision an effect-free no-op, but its rules remain editable. Other set IDs and names are fixed after creation, names are unique ignoring case, any live set may be the current fallback, and any live set—including the built-in set—may be assigned to many repositories. Each set contains at most 128 canonical decision points and the complete catalog contains at most 8,192 rules and 8,192 gates in any validated ordered mix of working-context AI, isolated AI, deterministic, and zero gates. A trusted source selects the exact assignment or current fallback, case-folds repository lookup, deep-detaches snapshots, hashes the complete named catalog, and derives a selected revision only from canonical repository, decision, selected set ID, and selected gates so unrelated set or assignment edits do not retarget an admitted decision. Legacy global/repository catalogs remain valid on the legacy runtime path and are deterministically materialized without losing behavior when the result fits named bounds; repositories with byte-identical effective rules share one imported set. If distinct expanded results exceed named bounds, runtime behavior remains active and the management API returns an explicit simplify-before-migration conflict instead of preventing gateway startup or partially converting the catalog. Every configured AI gate names an available exact agent, and every working-context agent has a session store, before the active gateway generation opens event storage. Exact authenticated `GET` and same-origin `PUT /api/reviews/attention-policies` expose only the normalized named catalog, revisions, and gateway effect. `PUT` is a strict full replacement fenced by `expected_config_revision`; it rejects renaming an existing stable ID, missing references, deletion of a used/default set, or a changed built-in identity before atomically raw-patching only `reviews.attention`. | A successful replacement mutates only operator-owned PicoClaw configuration: it runs no model, edits no code, executes no CI, invokes no Git or push, acknowledges no review, resolves no review thread, merges no pull request, and writes no checked-out repository, review case, transcript, decision link, workflow run, event, provider, or GitHub resource. Editing a shared set affects future unpinned decisions for every repository selecting it; changing the fallback affects only unassigned repositories, and explicit assignments remain selected. Existing immutable pins remain authoritative under `FR-EVENT-AUTOMATION-072`, `FR-EVENT-AUTOMATION-075`, and `FR-EVENT-AUTOMATION-078`. Policy changes participate in the gateway restart signature only while event ingress and workflows are active. A save preserves every unrelated public value and numeric token from the exact loaded revision, never serializes environment overrides or materialized defaults, and leaves the security sidecar byte-identical. | Null or malformed collections, an absent or renamed built-in set, duplicate/case-colliding names or repositories, missing fallback/assignment references, renamed existing IDs, noncanonical set/decision/gate/agent IDs, unknown gate kinds, duplicate gates, incompatible working-context agents, non-JSON questions, missing configured agents, excessive count/depth/size, unsupported transport content, unsafe Unicode, duplicate/unknown/trailing JSON, oversized bodies, missing/stale revisions, cross-site mutation, load/save uncertainty, or runtime drift fails closed. Exact before-push owner equality remains a per-publication runtime condition under `FR-EVENT-AUTOMATION-075`. A stale revision conflicts and never retries or writes. Repository policy authority can never be loaded from the PR head. | Reusable named behavior must stay consistent across many repositories without allowing reviewed code, stale browser state, an unrelated set edit, or a partially initialized runtime to change who can stop or steer development. |
| `FR-EVENT-AUTOMATION-047` | MUST | An authenticated operator opens `/reviews?view=policies`, edits a set's rules, duplicates a set, changes the current fallback, assigns or unassigns exact repositories, saves the complete catalog, reloads authoritative state, or navigates away with unsaved changes. | The responsive accessible Rule sets editor reads only `GET /api/reviews/attention-policies` plus the bounded agent catalog and keeps one memory-only draft. The built-in `Default` set is visible with a locked name and all known moments Off when empty. Every set name is read-only after creation; Duplicate asks for one trimmed case-insensitively unique permanent name, generates a fresh stable ID, deep-copies exact rules/questions, and copies neither assignments nor fallback status. A set displays whether it is built-in, currently used by default, and how many repositories select it. Any set can become the fallback. Repository assignments accept exact `owner/repository`, choose one live set, allow many repositories to share it, and return to the fallback when removed. Known `review.submitted`, `pr_development.review_attention_required`, and `pr_development.before_push` choices coexist with custom canonical moments and ordered working-context AI, isolated AI, deterministic, and zero checks. Strict local validation mirrors server identity, name, reference, collection, gate, text, JSON, and complete-catalog bounds. Lossless parsing retains every accepted number token and case-distinct/special key across GET, duplication, editing, and PUT. | The inbox URL omits `view`; the editor uses only `view=policies` and places no catalog data in route state, browser storage, logs, or toast text. Background refetch never overwrites a dirty draft. Save sends one non-retried full replacement with the captured config revision; success rehydrates authoritative state and reports applied versus restart-required effect. A conflict preserves the draft and requires explicit reload/discard. Navigation and before-unload remain blocked while dirty or saving. The editor explains shared-set blast radius and future-unpinned scope and warns that a before-push working-context agent must match the PR-development owner. Opening, editing, duplicating, assigning, refreshing, discarding, or saving creates no review case, decision, run, model/tool call, code edit, CI, Git, push, provider acknowledgement, thread resolution, or merge. | Malformed or numerically lossy responses fail unavailable rather than partially populating the form. Duplicate names/IDs/repositories, missing references, deletion of a used/current set, invalid decisions or gate IDs, incompatible working-context agents, missing agents, malformed questions, unsupported expressions, and over-bound state produce actionable errors and disable save. A failed/stale save retains all text and never rebases or retries; delayed reads/saves cannot replace a newer hydrated generation. Empty/error/loading states, long identities, many sets/rules, keyboard navigation, open dialogs, and narrow widths remain operable. | Operators need one comprehensible place to reuse and assign safe attention behavior without raw configuration, overlay modes, numeric corruption, stale-tab overwrite, accidental execution, or leaked policy authority. |
...
| `FR-EVENT-AUTOMATION-058` | MUST | Schema-v10 code opens a schema-v9 eventing database, or a trusted future PR-development controller calls `eventing.PRDevelopmentControllerStore` for the latest queued or completed repair attempt in one provider-verified thread with an existing pinned retained-workspace baseline. | Schema v10 adds at most one private `pctl_` controller and stable `pdln_` retained-line identity per verified `pdt_` thread, immutably bound to one owner repair session, canonical agent, and exact existing workspace/clone/ref/commit pin. First creation atomically transfers that session out of the legacy claim queue and, after store-wide collision and unique-owner checks, inherits the exact `pdrk_` reservation already locking its pinned workspace; every later mutation receives a globally fresh `pdck_` reservation. Expected revision and reserved headroom fence every material transition; exact live token, deadline, monotonic epoch, and non-regressing time fence every state-changing lease write. First Adopt or later Resume must bind the complete exact line result, and Resume durably advances mutation epoch before Park evidence is accepted. Only the completed latest owner attempt may append its next exact park/version/epoch/intent/base/tip/tree/no-change/review-digest fence. That transaction globally retires the reservation digest, hash-chains the parked and authenticated retired-mutation proof, removes usable mutation authority, and enters `review_pending`. A distinct reservation-free review lease may then claim only that fence; Finish folds authenticated review-completion proof into the final tail hash and enters `ready`, Release returns the unreviewed fence to `review_pending`, and only an expired review lease may rotate safely. Complete reads validate owner/pin and initial-reservation equality, phase/lease shape, exact reachable revision/epoch relations, source/line state, completed fence ownership and causal order, two-stage contiguous hashes/versions, no-change tree preservation, and store-wide active/retired reservation non-reuse. | Migration creates only empty tables and validated indexes. The APIs perform no filesystem, Git, model, AI-review, CI, workflow, commit, push, merge, HTTP/UI, or provider effect. Creation changes private queue eligibility by suppressing legacy claims for the owner; this is durable storage ownership transfer, not worker execution. This slice has no controller-aware transition that completes a newly admitted queued attempt, so it does not claim that legacy dirty work can be adopted or that a new queued attempt can complete; the later worker must adopt a clean pinned line before mutation. Reader snapshots validate the complete private aggregate but redact both the lease token and raw mutation reservation. The parked Git-workspace line remains retained after mutation authority is retired, but this store neither creates nor inspects it. | Legacy/malformed identity, duplicate reservation ownership, a sibling or unpinned owner, disallowed/non-latest/cross-session attempt, stale revision, insufficient exit headroom, foreign/expired token, skipped or changed binding, unfinished attempt, duplicate/gapped/changed fence, reservation reuse, impossible no-change tree, noncausal or unreachable proof, or corrupt high-water/hash state fails closed without another lease or partial evidence. An exact current operation that encounters an eventing-recoverable mutation expiration—unbound in eventing, or bound at its active mutation epoch—durably enters `recovery_required` and preserves its bearer in private storage; stale callers and read-only Get neither transition state nor receive that bearer. An expired review lease alone may be reclaimed for the same immutable fence. RecordFence and FinishReview authenticate exact retries with hash-bound retired token proofs; an exact committed Bind retry is no-write only after monotonic-time and live-deadline validation, while an expired retry enters recovery, while Acquire and ReviewRelease do not replay retired authority. No storage phase, including `ready`, proves an AI review, deterministic validation, CI, commit, or publication ran. | A later attempt ledger needs one crash-fenced owner and immutable handoff from exclusive mutation to separate review without holding edit authority across model context, silently rerunning ambiguous filesystem work, or prematurely wiring any worker or product surface. |
| `FR-EVENT-AUTOMATION-059` | MUST | Schema-v11 code opens a schema-v10 eventing database, or a trusted future controller records the post-validation account or completes a reservation-free local review for one exact parked attempt fence in a provider-verified thread. | Schema v11 adds private append-only attempt/review entries and structured review findings. Absolute sequence `2*n` is fence `n`'s attempt account and `2*n+1` its review; the first post-upgrade attempt may anchor at an existing unreviewed parked fence without fabricating earlier logs. Attempt append derives commit, tree, no-change, owner case ordinal, and pre-review fence hash from validated controller state and accepts only a bounded concise description plus exact CI plan/result SHA-256 digests. Review append requires the immediately preceding attempt account and exact live reservation-free review lease, then atomically finalizes that fence and controller into `ready` while storing one bounded `passed`, `changes_required`, or `attention_required` result and at most 128 structured findings bound to the final fence hash. Both chains use domain-separated length-prefixed hashes, exact authenticated replay is no-write, changed replay conflicts, and complete reads revalidate thread/session/controller/fence ownership, order, timestamps, evidence, findings, and every hash. A checkpoint may advance only over a later review entry whose exact chain digest matches; it records a bounded derived summary and compactor/prompt revision proof while retaining every raw entry, finding, and older checkpoint. One atomic context snapshot binds the selected case ordinal, complete thread high-water, and ledger. The pure v2 projection includes the selected plus newest provider reviews in thread-ordinal order, the latest checkpoint plus every raw suffix entry, and newest conversation messages; it labels all content untrusted, reports exact omissions, never substrings feedback or drops a raw suffix entry, and stays within independent provider, ledger, count, and 512-KiB total limits. | Migration creates only empty validated tables and does not infer evidence for old fences. Older already-reviewed fences may be skipped by anchoring at a later newly parked fence, but neither a new attempt account nor a review result can be backfilled onto an already-reviewed fence. Appends and checkpoints mutate only owner-local SQLite; the projection is side-effect free. This slice does not discover or run CI, commit, park, review with a model, compact with a model, alter the legacy worker/chat path, invoke a gate, touch Git/filesystem, expose HTTP/UI, publish, push, merge, acknowledge feedback, or call a provider. CI digests authenticate later structured evidence but do not themselves run or prove a command outside the future controller transaction. | Legacy threads, cross-thread/case/session attempts, missing fences, review-before-attempt, foreign/stale/expired review authority, independently finished fences without their atomic review account, gaps, changed replay, nonadvancing/even checkpoint boundaries, wrong source digest, malformed outcomes/findings/OIDs/digests/timestamps, oversized uncompacted suffixes, and SQL-valid cross-table/hash corruption fail closed. Optional old provider reviews and oldest chat messages may be omitted deterministically with exact ranges/counts; mandatory selected review or ledger evidence that cannot fit returns a typed compaction-required error. All storage/controller identifiers, hashes, CI digests, compactor identity, provider provenance, lease/reservation/workspace/source data, and raw diagnostics remain outside the model JSON and every browser/public surface. | Each completed attempt must carry a durable concise account, commit and validation binding, followed in causal order by its immutable AI review; when that history grows, model context must use verifiable logical compaction without erasing audit evidence or relying on provider prompt-cache retention. |
| `FR-EVENT-AUTOMATION-060` | MUST | Schema-v12 code opens a schema-v11 database, or an exact controller operation discovers that an eventing-recoverable mutation lease expired (unbound in eventing, or bound at its active mutation epoch). | In the same immediate transaction as `recovery_required`, schema v12 appends one bounded hash-chained `pdri_` recovery intent that binds the old bearer and its digest, one globally fresh replacement bearer and digest, expired revision/lease-token proof, source/workspace and optional exact bound-line fence, and a caller-durable rotation identity. A separate recovery claim may be acquired, renewed, exactly replayed, or safely reclaimed after expiry because its sole permitted external effect is the exact idempotent reservation transfer. After `RotatePinnedReservation` returns the matching fence and proof, exact finalization atomically changes the controller old-to-fresh bearer, restores `mutation`, advances revision and lease epoch once, issues a fresh mutation token/deadline, records hash-bound final proof including that issued deadline, and erases both raw keys from the finalized intent. Complete reads validate the intent/final hash chain, at most one active intent, causal digest and issued-deadline transitions, exact controller revision/epoch accounting, and global active/created/revoked/park-retired reservation nonreuse. | Migration creates only empty validated recovery storage. A pre-v12 `recovery_required` controller has no trustworthy expired-token or fresh-bearer evidence and remains visible but deliberately unclaimable; no migration guesses it. Ordinary controller acquisition still rejects every recovery state. Claim writes never grant mutation authority, and finalization does not change line version, mutation epoch, tip, tree, refs, index, worktree, or ordinary files. The APIs run no Git themselves and expose no model, CI, workflow/gate, provider, HTTP/UI, publication, push, merge, or acknowledgement surface. | Missing legacy evidence, stale revision, live foreign claim, expired claim/token, mismatched rotation result, changed controller/source/workspace/line fence, reused reservation, gapped/corrupt intent chain, insufficient exit headroom, or an already re-expired finalized lease fails closed. Pending/already-applied commit or park ambiguity is not reconciled by this slice and remains `recovery_required` for the durable intent protocol in the next requirement. Exact finalization replay is no-write only while its immediate resulting mutation state, including the originally issued deadline, is unchanged. | Recovery must preserve unknown local edits while permanently fencing the crashed worker's known bearer; a database-only lease replacement would let two controllers mutate the retained checkout. |
| `FR-EVENT-AUTOMATION-061` | MUST | Schema-v13 code opens a schema-v12 database; a trusted controller prepares one exact retained-line `Adopt`, `Resume`, `Commit`, or `Park` effect under the live mutation lease for the exact owner attempt; or that prepared operation's mutation lease expires before its result is finalized. | Schema v13 adds private hash-chained `pr_development_controller_operation_intents`; operation identity, order, and request are immutable after prepare, while only fenced lifecycle and append-once result/proof fields may advance. At most one operation per controller is unfinished; each caller-durable operation identity hash-binds its kind, controller/attempt/revision/lease, source/workspace/line snapshot, exact Git request bytes, mutation-reservation digest, previous operation hash, and later exact result bytes and final proof. Prepare commits before the external effect. An exact Prepare replay may no-write return an already-finalized current-tail Commit only when its operation identity, request, and authority are unchanged, allowing the trusted runner to reverify the idempotent `CommitPinned` result before no-write Finalize replay and Park; a non-finalized prepared effect with an ambiguous outcome remains recovery-owned, and changed replay conflicts. An expired operation enters its own renewable/reclaimable recovery state and is the sole live recovery claim: no pending schema-v12 recovery row is created. Adopt recovery calls `RecoverPinnedLineAdoptReservation` and Resume recovery calls `RecoverPinnedLineResumeReservation`; both retain canonical old/fresh locks throughout convergence and replacement. A pre-effect Adopt durably revokes old and installs fresh before the second durable save creates or reconciles the line, so replay can complete the recorded intermediate state; a post-effect Adopt first verifies the exact line and Git state before replacing authority. Commit recovery rotates old to fresh and then calls deterministic `CommitPinned` with fresh under the same recovery claim. Park recovery exactly replays `ParkPinnedLine` with old, which retires it, and never stages fresh mutation authority. Recovery finalization for Adopt, Resume, or Commit atomically appends linked already-finalized schema-v12 evidence for audit continuity, but only the schema-v13 row owns the live claim. Complete reads validate operation ordering and hashes, controller and attempt causality, exact request/result shape, claim fencing, raw-key erasure, and store-wide reservation nonreuse. | Migration creates only empty validated operation storage. A migrated, already-bound live v12 mutation may establish the sole initial v13 high-water with Commit then Park or with a no-change Park; every later attempt still begins with Resume, and legacy Bind/Fence cannot bypass an established operation history. Normal Adopt/Resume finalization binds the exact line result; Commit finalization records only its exact verified commit result while the controller line remains mutating until Park; their recovery finalization installs the fresh bearer under a new mutation lease and erases staged raw keys. Normal or recovered Park finalization is one immediate transaction that finalizes the operation, appends the exact attempt-review fence, retires and removes mutation authority, enters `review_pending`, and, for a queued owner attempt, records its bounded completion summary/iterations/workspace, clears active attempt ownership, and increments the repair-session version once. An owner attempt that was already completed before controller adoption remains byte-for-byte unchanged and does not receive another session bump. These store methods execute no Git themselves. | A missing prepare, wrong operation order or kind, concurrent unfinished operation, legacy transition after operation ownership, legacy recovery row without operation evidence, stale controller/revision/lease/claim, request or result drift, cross-controller/attempt/workspace evidence, dirty or drifted Git result, reused bearer, corrupt/gapped hashes, insufficient capacity/headroom, or partial Park terminal tuple fails closed without guessing or granting mutation authority. Exact replay of a normal finalized Commit is no-write when its causal result, operation-chain tail, controller revision/phase/current attempt, and live bearer token/epoch/reservation remain exact; only a monotonic heartbeat advance of that bearer's lease deadline and aggregate update time may differ. Adopt, Resume, every recovered operation, and Park still require their exact immediate resulting state. A changed result or authority, an expired normal Commit bearer, or any non-immediate replay outside the Commit-only heartbeat allowance fails closed; a stale crashed worker cannot finalize after recovery rotates or retires its bearer. This slice does not discover/run/cache CI, execute a model or edit loop, append the schema-v11 attempt/review ledger, perform AI review, evaluate attention gates, expose HTTP/UI, contact a provider, publish, push, merge, or acknowledge review feedback. | Write-ahead operation identity plus effect-specific exact replay closes every crash window between SQLite and retained-line Git while keeping the branch after mutation authority is released and making the queued-attempt-to-review handoff indivisible. |
...
| `FR-EVENT-AUTOMATION-065` | MUST | A trusted local caller explicitly launches one configured attention decision for the exact current `attention_required` PR-development review tail, conversation prefix, and decision point. | Schema v15 exposes one integrity-checked atomic attention snapshot containing the provider thread high-water, complete fenced conversation, immutable owner/session/controller/fence state, and full ledger. The launcher resolves the authoritative repository through the selected case, captures the existing global plus repository-local gate policy, preserves its ordered repeated `ai_working_context`, isolated `ai`, deterministic, and zero workflow primitives, reloads and cross-validates the exact persisted local-CI evidence and retained-line review snapshot, and builds one canonical bounded subject that always carries the raw target attempt/review pair. A working-context gate must use the immutable repair owner agent and receives the complete conversation only through an exact-revision protected review-scoped session; isolated and deterministic gates receive only the frozen subject. | A domain-separated subject revision binds the canonical subject and non-model-visible thread, transcript, ledger/checkpoint, CI, fence, and projection high-waters. Together with case, review entry ID/hash, conversation version, decision point, and effective policy revision it determines one workflow-run ID. One immediate transaction revalidates the exact ready, unleased, reservation-free, no-later-attempt state, inserts the immutable decision link, and invokes durable private workflow-run creation exactly once. Historical exact replay resolves the same link after later state advances; a policy containing only zero gates creates no session, link, or run. The gateway may compose this launcher but starts no trigger worker in this slice. | Changed policy, conversation, provider thread, checkpoint, CI, diff, review tail, controller/session ownership, active lease/reservation, queued later attempt, wrong working agent, oversized mandatory evidence, protected-session collision/rollback, malformed run, or ambiguous storage failure fails closed without a second model/run. Launch and any waiting human task retain the local branch without mutation authority and cannot append PR chat, schedule repair, call a provider, push, publish, acknowledge, merge, or navigate UI. Automatic triggering, response application, and browser integration are later requirements. | All four gate choices and their ordered mixtures should reuse one workflow engine and one global/repository policy catalog, while exact local evidence—not a mutable controller counter, session CAS token, or model claim—defines durable decision identity and prevents duplicate attention side effects. |
| `FR-EVENT-AUTOMATION-066` | MUST | Schema-v16 code completes an exact reservation-free local review as `attention_required`, a generation-owned attention worker observes its durable occurrence, or an authenticated user opens or answers the selected case's attention conversation. | The same immediate transaction that appends the review entry and makes its controller ready inserts exactly one occurrence bound to the immutable review ID/hash and transaction-current conversation version/digest. Migration creates no occurrence for historical reviews. The worker claims one due occurrence under a renewable scheduling-only lease, captures the assigned or fallback reusable set for fixed decision point `pr_development.review_attention_required`, canonically pins that policy and the active subject revision before session, model, workflow-run, or human-task effects, and launches through the ordinary mixed workflow-gate engine. Exact retry reuses the immutable pin and resolves an already-linked deterministic run before consulting later mutable state. An empty or all-zero rule completes `noop` without a run; a later causally superseding development state, irrecoverable admission uncertainty, or immutable bounded-subject failure completes a distinct terminal state rather than retrying forever. The claim and runtime generation end after delivery or terminalization, while the retained branch remains reservation-free. | The protected runtime and authenticated launcher expose only exact GET `/pr-development/{case}/attention` and POST `/pr-development/{case}/attention/respond` operations. For this local-review source they validate the authoritative case, current review occurrence, canonical policy pin, subject identity, decision link, private run, and stable human-task chain before returning only case version, bounded public lifecycle, response availability, configured titles/questions, accepted answers, and one opaque fence for the sole actionable turn. POST accepts only that fence, the exact case version, and one normalized bounded answer; server-loaded private identities derive an idempotent response ID and resume the ordinary workflow task. The unchanged authenticated case-list `attention_required` boolean is shared with requirement 074: this source contributes true exactly while the authoritative current review still requires attention and its current occurrence is pending, claimed, or delivered, and contributes false for absent, superseded, no-op, recovery, or failed occurrences. The browser treats the final safe union only as generic `Needs input`, selects it through canonical `view=development&case={pdc_...}&focus=chat`, and renders the selected case-owned conversation beside existing chat without exposing which private source supplied it. This is in-app discovery only and makes no out-of-band, provider, email, or channel-notification claim. The rule-set editor continues to own arbitrary decision points and ordered repeated-gate configuration inside reusable sets, one current fallback, and exact repository assignments; it offers this decision point as a known preset without creating another policy tree. | Missing or corrupt occurrence/pin/link/run/task state, stale or cross-case fences, later-attempt ambiguity, policy or subject drift, oversized evidence/answer, malformed routes or JSON, runtime reload, cancellation, uncertain admission, or simultaneous current local-review and publication sources fails closed through bounded statuses and fixed errors. GET, list or conversation polling, navigation, zero/no-op, and failed validation mutate no case, chat, repair, controller, branch, repository, provider, workflow, or notification state. A valid response changes only the exact private human task and continuation; it does not itself append PR chat, schedule a repair, acquire mutation authority, call a provider, push, publish, acknowledge, merge, or delete the retained branch. | A configured gate must reliably surface the exact local PR discussion in the workbench, preserve every ordered question and answer across restart, and compose safely with the later publication-gate source without turning attention delivery or its browser token into code or provider authority. |
| `FR-EVENT-AUTOMATION-067` | MUST | An authenticated user reads the exact detail of a selected PR-development case, whether or not its atomic workbench contains a repair session or attempt. | The existing one-snapshot workbench read integrity-loads the selected case, bounded conversation, optional repair session, verified provider thread, complete controller aggregate, and ledger before constructing a dedicated optional `local_development` DTO field by field. Live or otherwise incomplete lifecycle comes only from the latest public repair attempt; only when that attempt is completed does the snapshot load its exact orchestration for completed-evidence validation, and commit/CI/review declassification requires that orchestration itself to be completed and receipt-bound. The object is omitted when there is no repair session or attempt. Otherwise it exposes only the latest attempt's already-public opaque `attempt_id`, `attempt_ordinal`, fixed `attempt_status`, bounded `summary`, and `updated_at`. Before an exact completed orchestration, validation receipt, and bound immutable attempt-ledger entry exist, `commit_sha`, `ci_status`, `ci_plan_digest`, and `ci_result_digest` are absent, `no_changes` is false, `review_status` is `not_started`, `review_outcome` and `review_summary` are absent, and `review_finding_count` is zero. Once that complete exact linkage exists, its candidate commit and no-change fact, terminal local-CI status, and canonical opaque CI plan/result fingerprints are present; `review_status` is `pending` with no outcome/summary and zero findings until the immediately paired reservation-free review entry exists, then `completed` with its fixed outcome, bounded summary, and exact bounded finding count. | This projection adds no schema version, table, index, migration, backfill, or mutation. `local_ready` is derived as true only when the latest public attempt is `completed`, its exact completed orchestration and receipt bind the attempt entry whose `ci_status` is `passed`, its immediately paired review records `passed`, and the loaded controller is owned by that repair session, points at that attempt, and is in reservation-free `ready`; every other representable state is false. Reading performs no repair admission, model, local-CI, Git, workspace, branch, controller, ledger, workflow/gate, provider, publication, acknowledgement, or merge effect. | Ordinary progressive absence is represented only by the omission and status rules above. In particular, a live incomplete orchestration is neither required nor exposed, and a legacy or otherwise unbound ledger entry whose compatibility read defaults CI to passed exposes no commit, CI, or review evidence, remains `not_started`, and can never make `local_ready` true. A missing or future conversation binding, invalid thread/session/controller/orchestration/receipt/attempt/ledger relation, noncontiguous or corrupt evidence, cross-case or nonlatest association, invalid lifecycle/CI/review value, malformed commit/fingerprint/timestamp, or over-bound text/count fails the entire authoritative detail read through its existing fixed unavailable boundary rather than returning a partial or stale local snapshot. The DTO cannot represent a workspace or checkout, internal Git ref, thread/controller/ledger/orchestration identity or hash, lease/reservation/claim/worker field, credential/provider-write capability, raw finding, raw CI plan/step/output/error/attestation, or internal diagnostic. The commit is local candidate identity only; the two fingerprints are equality-only evidence labels; `local_ready` is never repair, provider-refresh, push, publication, acknowledgement, or merge authority. | Users need an honest current view of what happened locally and whether the exact candidate cleared CI and review, without exposing the private evidence graph or mistaking local readiness for permission to change code or GitHub. |
| `FR-EVENT-AUTOMATION-068` | MUST | Schema-v17 code opens a schema-v16 event store; the generation-owned PR-development recovery worker observes the oldest due bound schema-v12 recovery or schema-v13 Adopt/Resume/Commit/Park operation recovery, including one whose recovery claim expired; or a later repair orchestration owns a queued attempt for a controller whose retained line is `suspended`. | Migration adds the private schema-v17 suspension handoff and suspended-resume records and extends the controller with `suspension_pending` and `suspended` phases without synthesizing a row or Git result. The provider-independent worker claims or expired-claim-reclaims one eligible recovery under a fresh renewable scheduling token while holding the exact active event-runtime generation. A bound schema-v12 intent and a schema-v13 Commit first call exact `RotatePinnedReservation`; Commit then calls the prepared exact deterministic `CommitPinned` with the fresh bearer. Schema-v13 Adopt and Resume call their exact old-to-fresh composite recovery methods. Before any of those non-Park paths may call Git suspension, one immediate transaction authenticates the live recovery claim and append-once persists the canonical recovery result, rotation proof, exact line fence, fresh-bearer digest and raw fresh bearer, a distinct caller-durable suspension intent, and, for Commit, the complete prepared request plus exact `CommitPinned` result; it erases obsolete raw old-bearer copies, issues no mutation lease, and enters `suspension_pending`. Exact replay returns that same checkpoint without another Git recovery effect. The worker then calls `SuspendPinnedLine` for bound-v12, Adopt, or Resume, or `SuspendPinnedLineCommitRecovery` for Commit, and atomically finalizes only the matching suspension evidence into `suspended`. Park instead exactly calls `ParkPinnedLine` with old, snapshots the resulting parked line, and uses the existing atomic schema-v13 Park finalization directly; it creates no replacement bearer, suspension checkpoint, or suspended phase. | A completed non-Park suspension retains the exact workspace/source/line/version/epoch/tip/tree and candidate/optional prepared-Commit evidence, terminalizes its source recovery through the schema-v17 handoff, clears every mutation/review/recovery lease, and erases every raw old/fresh/claim copy from the recovery and handoff rows; only non-authorizing digests and hash-linked exact results remain. The controller has no usable reservation and cannot be selected for model, CI, review, attention, or publication work. Before any later repair model or compactor call, the repair orchestration must atomically prepare one exact suspended-resume record under its live scheduling claim with the latest suspension fence, one globally fresh raw bearer, its digest, and a caller-durable resume intent; call exact `ResumeSuspendedPinnedLine`; then atomically authenticate its exact result, install that bearer as the controller's sole live mutation reservation under a fresh mutation lease, consume and erase the staged raw resume copy, and continue the same queued attempt. A crash before or after the Git resume replays only that durable intent and cannot start a model until eventing finalization proves the exact mutation owner. If that resume claim expires before handoff, the recovery worker exact-replays the same resume, atomically finalizes its intermediate proof while appending a new `suspended_resume_recovery` suspension checkpoint that alone receives the fresh bearer, and runs ordinary exact suspension; the controller therefore returns to bearer-free `suspended` instead of leaving idle mutation ownership or starting a model. Workbench projection treats `suspension_pending` and `suspended` as `local_ready=false`. | Selection is deterministic and bounded; stale generation, lost/expired/replaced claim, changed result, checkpoint, suspension, resume, controller revision, attempt, line, source, candidate, prepared Commit, bearer, or intent fails without another effect or authority grant. Recovery preflights enough rotation and suspension/resume history capacity before old-to-fresh mutation so it cannot create a fresh owner that cannot be suspended and later resumed. Schema-v12 intents without a complete bound-line fence are deliberately excluded from automatic claim, reclaim, rotation, release, and migration; they remain unchanged in `recovery_required` and locally not ready until a separately approved idempotent unbound-retirement policy safely defines response-loss replay. Reload and shutdown cancel, drain, and join the worker before closing its generation's manager/store; it requires neither GitHub/provider-object readiness nor any model, workflow, gate, browser, push, publication, acknowledgement, or merge capability. | Ambiguous local effects need an always-on, crash-replayable path that preserves unknown work yet releases edit ownership, without converting a recovery claimant, transient replacement bearer, suspended branch, green historical evidence, or later resume into model or provider authority. |
| `FR-EVENT-AUTOMATION-069` | MUST | Schema-v18 code opens a schema-v17 event store; one exact reservation-free local review completes as `passed`; or a trusted future publisher invokes a private publication-store transition with its exact current claim and immutable inputs. | Migration creates empty private publication storage and supporting exact-evidence indexes without backfill or inferred work. In the same immediate transaction that appends the passed review, finalizes its fence, and enters controller `ready`, the store creates exactly one unique, idempotently replayable exact `pending` publication occurrence with a fresh opaque ID only when the exact completed orchestration receipt binds green persisted CI, the attempt-ledger entry immediately precedes the passed review entry, no later attempt exists, and the controller/source/workspace/retained-line version, mutation epoch, Park intent, base, tip, tree, and no-change facts all agree. The returned review completion includes that record. The queue can claim or safely expired-claim-reclaim only due pre-effect `pending`, `gate_waiting`, or `push_ready` work with fresh owner/token/deadline/epoch fencing; renew an exact live pre-effect claim; return an exact still-live claim to its authenticated `claim_from` origin at an availability that is non-past at the live transition without discarding progressive evidence; and create-once pin canonical policy, private gate subject, and caller-supplied provider observation. The separate decision-run store atomically binds one semantic decision key to one deterministic private workflow run through a callback whose deterministic RunID creation is idempotent and replayable. A non-nil callback error guarantees that no external run was created; a nil callback return followed by SQLite commit uncertainty returns `ErrPRDevelopmentPublicationAdmissionUncertain`, so an automatic caller must not invoke create again and may only use an explicit exact pre-start terminalization or recovery path. It can release the claim into reservation-free `gate_waiting`, advance completed/no-op gating into `push_ready`, or terminalize pre-start work as `conflict`, `superseded`, `failed`, or `recovery_required`. Immediately before a later caller could invoke Git, `StartPRDevelopmentPublicationPush` authenticates the exact claim, revalidates the complete transaction-current local evidence and absence of later work, requires a provider observation obtained strictly after the current claim and immutable pin and no more than five minutes old whose canonical facts are byte-identical to that pin, refreshes only the observed instant, persists and hashes the full canonical `PushPinnedLine` request, and enters `push_started` as the sole publication state that excludes controller mutation. The push journal alone can renew that exact live started claim while the effect remains in flight. Exact proven `applied`, `already_current`, or `reconciled` results may finalize `published`, preserving a separately flagged local postflight drift; an exact compare-and-swap loss becomes `conflict`; and an unproven post-start result or expired started claim becomes execution-terminal and non-reclaimable `outcome_unknown`. A later trusted caller may resolve unknown to published only by supplying a distinct, independently obtained, minimal read-only remote-head observation whose instant is strictly after unknown completion and no more than five minutes old and whose current tip equals the immutable desired tip; reconciliation neither overwrites the original provider/review pin nor requires that review to remain unchanged. Observing expected, another tip, absence, or unavailability is inconclusive and never calls or schedules push. | The scheduling graph is `pending | gate_waiting | push_ready -> claimed(claim_from=origin) -> exact origin` for safe pre-effect requeue; otherwise a pending claim may enter `gate_waiting`, `push_ready`, or a pre-effect terminal state; a gate-waiting claim may return to waiting, enter push-ready, or terminalize; and only a push-ready claim may enter `push_started`. The requeue edge requires the same live token/epoch and expected origin, changes only scheduling ownership and availability, and exactly replays only for the retained epoch, restored origin, and availability. Started work may become `published`, `conflict`, `failed`, `recovery_required`, or execution-terminal `outcome_unknown`; unknown has only the separately evidenced `published` reconciliation edge. The policy tuple and subject tuple are each internally all-or-none, allowing a progressive policy-only pin before the subject; their canonical bytes and hashes and the provider facts are immutable, while only the provider observed instant may refresh at Start; expected remote tip and push request are immutable before effect; result and disposition exist only for proven publication. Ordinary `pending`, `claimed`, `gate_waiting`, and `push_ready` states hold no controller mutation lease or Git reservation and do not block repair. Mutation acquisition and `push_started` admission serialize in SQLite so exactly one wins; a later repair makes stale publication evidence supersedable rather than freezing the branch. All DTO fields and canonical policy/subject/provider/request/result bytes, hashes, claim values, decision IDs, and diagnostics remain private. The store executes no workflow, gate, model, provider, Git, gateway, HTTP/UI, acknowledgement, merge, or worker effect. | Any non-passed review, non-green or compatibility-default CI, absent or nonadjacent ledger pair, unfinished orchestration, stale controller/fence/source/line, queued or later attempt, malformed/cross-record/over-bound input, changed immutable pin, stale claim, decision mismatch, or current-state drift fails closed without a partial review/publication transition or effect. Exact committed replay returns the existing occurrence or transition; opening v17 creates no occurrence for historical passed reviews. Expired pre-effect work may receive a fresh claim, but expired `push_started` is never reclaimed or reinvoked. A failed atomic occurrence insert rolls back review, fence, controller, and ledger completion together. Outcome reconciliation treats desired-tip equality only as proof of the current result, not proof that a prior call was or was not applied across remote OID ABA. No publication state implies review acknowledgement or merge, whose user-facing actions and policies remain explicitly undefined and unimplemented. | Publication needs one crash-reconstructible authority and effect journal built from exact local evidence, while human waits, retries, later repairs, process loss, and uncertain remote outcomes must not create a stale push, duplicate effect, long-lived checkout lock, or accidental acknowledgement/merge policy. |
| `FR-EVENT-AUTOMATION-070` | MUST | A trusted publication controller needs either the exact current provider/review fence for a schema-v18 publication or a review-independent current remote-head observation for an already `outcome_unknown` push. | Two distinct narrow GitHub publication observers each accept only one store-validated immutable development case plus its non-legacy provider thread identity. The full path reuses the generation-fenced read-only `github/pull_request_read` verifier, exact bounded PR/review decoding, canonical enterprise origin and provider-object cross-binding, and projects only repository, pull number, head repository/ref/SHA/clone URL, current review state, and review digest. The reconciliation path performs exactly one bounded pull-request `get`, validates the same immutable provider, author, URL, target-repository, and pull identity, performs no review scan, and projects only repository, pull number, head repository/ref/SHA so a later review edit or dismissal and a closed or merged pull do not make remote-tip evidence unknowable. Each successful adapter samples a canonical UTC observation time only after provider decoding and validation; the provider cannot supply that time. | Both concrete adapters and their verifier/clock fields remain package-private; separate constructors return only the full or head-only interface, so a caller cannot type-assert one capability into the other or recover the generic runner. Both result types and every member are structurally private and grant no eventing store transition, claim, workflow/gate, model, conversation, filesystem, checkout, Git, push, acknowledgement, merge, HTTP, UI, credential, transport, retry, or publication authority. Each observer invokes only the configured read tool and neither receives nor wraps a provider-write client. The schema-v18 store remains the authority that compares a full observation to the immutable local high-water and pin, requires a newer fresh repetition before push start, or decides whether a later head-only observation proves the exact desired tip. | Nil or canceled context, including cancellation during provider return or timestamp sampling, legacy or malformed thread identity, invalid durable case, provider-origin mismatch, missing or changed provider object identity, malformed/oversized/duplicate JSON, invalid ref/OID, invalid full-path clone endpoint, changed captured review on the full path, or any provider/tool failure returns no observation. Head-only reconciliation deliberately ignores and does not validate the unprojected clone endpoint; it does not infer whether the earlier push ran, retry it, require the review to remain actionable, or treat expected/other/missing/unavailable tips as success. | Provider preflight and crash reconciliation need causally timed least-authority observations, while provider reads themselves must never become scheduling, mutation, write, or retry authority. |
| `FR-EVENT-AUTOMATION-071` | MUST | A trusted publication-gate coordinator holds a publication ID plus the exact live lease token and epoch for a schema-v18 claim whose durable `claim_from` is `pending`, and needs one coherent subject-construction context before the first subject pin. | `GetClaimedPRDevelopmentPublicationGateContextSnapshot` opens one read transaction, authenticates that exact unexpired claimed-from-pending lease, loads and integrity-validates the complete publication high-water, and returns one detached private snapshot containing the claim-redacted publication, selected case ordinal, complete conversation with its rolling transcript digest, exact adjacent passed attempt/review tail, immutable case and provider thread, authority-redacted owner repair session, controller, matching fence and completed orchestration, and full uncompacted ledger evidence. The session projection clears the retired reservation key and every attempt idempotency/lease field after validation. It composes the existing canonical conversation and evidence types rather than storing or decoding a second raw-conversation representation. | The read performs no mutation. On the first `PinPRDevelopmentPublicationSubject`, the same immediate transaction integrity-loads the current conversation and requires both its version and transcript digest to equal the snapshot-derived expected values before storing the canonical subject. An exact already-pinned replay is resolved before that current-conversation comparison and remains a no-write success after later chat; a changed replay conflicts. The snapshot, its nested values, expected conversation fence, and subject request remain structurally private. | Missing, stale, expired, replaced, wrong-origin, cross-publication, or malformed claim evidence; a non-pending claim origin; corrupt or inconsistent publication, conversation, thread, session, controller, fence, orchestration, attempt/review tail, or ledger evidence; a changed first-pin conversation; or cancellation fails closed without a partial snapshot or pin. Neither operation invokes or exposes a provider, workflow, gate engine, model invocation, repair-session mutation, Git, filesystem, checkout, push, gateway, worker, HTTP/UI, acknowledgement, or merge capability. | Gate compilation needs one atomic, conversation-fenced local evidence graph so concurrent chat cannot silently change first-pin meaning, while exact retries must not be invalidated by chat that occurred after the immutable subject was already established. |
| `FR-EVENT-AUTOMATION-072` | MUST | A trusted coordinator gives `PublicationGateProcessor` one existing integrity-checked schema-v18 publication whose exact live lease is still `claimed` from `pending`; the processor is constructed only with the existing attention `PolicySource`, narrow exact-claim authenticator, claimed gate-context reader, pre-effect publication queue, full read-only publication observer, and clock-independent canonical helpers. | Before consuming even a caller-supplied durable pin, the processor uses `AuthenticateClaimedPRDevelopmentPublicationGate` to prove the token/epoch, live pending-origin lease, immutable high-water, and authoritative current pin progression while returning only the redacted publication and exact repository policy selector, never conversation or rich context; a stale caller may omit later authoritative pins but may not alter one it supplies. The processor then advances that authenticated private publication progressively. When no policy is pinned, it calls `attention.PreparePolicy` exactly once for that authenticated repository and `pr_development.before_push`, then pins the canonical source revision, resolution, and decision revision. Every later stage strictly reloads that durable policy through `attention.DecodePreparedPolicy`. If any effective AI-working-context, isolated-AI, or deterministic gate remains, including a mixture containing zero gates, processing stops successfully with `requires_execution` after the policy pin; no subject context or provider observation is acquired. For an empty, disabled, or all-zero effective policy, `workflows.CompileGateWorkflow` must independently confirm `Noop`; the processor then pins one small canonical versioned zero-gate subject that binds the publication/case/review identity and snapshot conversation version/digest through the existing first-pin compare-and-swap, obtains and pins one fresh full read-only provider observation, and marks the occurrence `push_ready` with an empty decision-run ID. | Authentication is read-only; policy, subject, and provider pins and the final no-op readiness transition remain the only mutations and occur in that order. Exact progressive replay consumes an existing valid authoritative pin before its producing capability: it never reconsults policy after policy pinning, never reloads rich gate context or rebuilds a pinned subject after later chat, never repeats provider observation after provider pinning, and never changes a prior canonical value. The result, supplied claim, prepared values, subject, observation, and every nested authority field remain `json:"-"`. This slice adds no schema or configuration migration and does not claim, renew, release, or safely requeue work; create/admit/read a decision run; execute or resume a workflow or model; inspect a filesystem or checkout; invoke Git or push; start a gateway or worker; write a provider object; acknowledge a review; or merge a pull request. | Invalid or changed claim/pin/context/policy/subject/provider/local high-water, cancellation, malformed compiler or provider output, or capability failure cannot fabricate readiness. A transient or unclassified failure leaves the occurrence under its still-live claim with every already completed immutable pin retained for a separately authorized caller to submit through the exact safe-requeue seam in requirement 073; the processor itself never invokes that seam. Only a provider/local conflict or causal supersession returned by the exact validating store boundary, or another existing exact pre-start terminal condition, may terminalize it; identical public error sentinels from policy or provider capabilities remain untrusted and transient. Processor-local inconsistency never manufactures a terminal result and no error guesses that an external effect occurred, because this processor has none. | The first executable publication slice needs a restart-safe zero-gate fast path and an explicit handoff for every active composition without yet inheriting run admission, scheduling, code mutation, or remote-effect authority. |
...
| `FR-EVENT-AUTOMATION-076` | MUST | A trusted publication lifecycle owner invokes `PublicationPushReadyHandler` with one exact already-claimed schema-v18 publication whose durable origin is `push_ready`, plus separately injected exact push-claim authentication, queue renewal/requeue, full provider observation, push-journal start/renew/finalize, and pinned-line push capabilities. | The handler immediately renews and authenticates the live publication ID/token/epoch/origin before any provider or Git work, replaces caller state with the authority-redacted durable publication, exact immutable case, and provider thread identity, and requires complete immutable policy, subject, provider, optional decision-run, and parked-line evidence. While the pre-effect queue heartbeat remains authoritative, it obtains one fresh full provider observation and requires its canonical provider facts to equal the immutable pin, then constructs the sole canonical `PushPinnedLine` request from the durable repository/source/workspace/line/Park/base/tip/tree evidence and the freshly repeated expected remote tip. At the effect barrier it drains queue renewal and calls `StartPRDevelopmentPublicationPush` with that exact request before Git. Only a start result with `newlyStarted=true` authorizes exactly one `PushPinnedLine` invocation using the request returned by the journal; an exact historical start replay, including recovery from a lost successful start response, never invokes Git. After start, ownership is renewed only through `PRDevelopmentPublicationPushJournal` until Git returns; that renewal is drained at the immediate finalization barrier before the exact result is recorded. | A complete `applied`, `already_current`, or `reconciled` result records `published`; the same proven remote result joined with local workspace drift records `published` plus local-drift evidence. A typed post-start pinned-line conflict without an unknown outcome records `conflict/push_conflict`; a definite invalid request or remote-unavailable failure records `failed/push_failed`; cancellation, deadline expiry, explicit outcome uncertainty, an invalid/partial result, an unclassified post-start error, or an already-started replay without a reusable exact terminal result records `outcome_unknown`; that uncertainty classification takes precedence over any joined conflict or workspace-drift error. A lost finalization response is resolved only by replaying the identical finalization tuple and never by starting or invoking Git again. The local line remains parked and mutation-reservation-free before, during, and after this handler; only the exact remote branch update and private publication journal may change. | Provider-pin drift becomes exact pre-start `conflict/provider_changed`, a store-proven local high-water conflict becomes `conflict/local_evidence_changed`, and causal replacement becomes `superseded`. A classified transient failure before durable start may release only the authenticated `push_ready` claim with bounded deterministic delay and all pins intact. Missing, stale, expired, replaced, cross-origin, malformed, or incomplete authority; queue-heartbeat loss; request mismatch; start ambiguity that cannot be exactly replayed; push-journal heartbeat loss; or finalization conflict fails closed. Once start may have committed, the handler never requeues or performs a second Git call. This slice adds no schema/configuration/route/DTO/UI, model/workflow/local-CI/repair mutation, production claim loop, unknown-outcome observer, provider review-write, review acknowledgement, thread/comment resolution, or pull-request merge authority. | Publishing a reviewed parked line must preserve the journal-before-effect crash fence and at-most-once Git boundary while preventing scheduling replay, provider drift, local repair, or an uncertain response from becoming another remote mutation. |

| `FR-EVENT-AUTOMATION-077` | MUST | A trusted process-local `PublicationOutcomeReconciliationWorker` is invoked with only the private publication outcome-reconciler store, exact development-case and provider-thread reads, one distinct head-only publication observer, and a local clock; or the same worker is reconstructed after process restart. | Before scanning unknown work, each iteration asks the store to expire one bounded batch of abandoned `push_started` rows into terminal `outcome_unknown`; a full expiry batch is reported as progress and drained before provider observation. The private store enumerates existing unclaimed `outcome_unknown` rows in stable `(available_at, created_at, id)` keyset order with a bounded page and fully integrity-validates and authority-redacts each record, so rows committed by an earlier process remain discoverable and backed-off prefix rows cannot starve later work. For one due row, the worker reloads and cross-binds the exact case and non-legacy provider thread before invoking only `PublicationRemoteHeadObserver`. It records `published/reconciled` only when the independently observed repository, pull number, head repository, head ref, and current head exactly match the immutable provider identity and durable requested tip. The exact reconciled result is derived solely from the write-ahead push request. | The expected pre-push tip when distinct from the requested tip, another tip, missing head, changed identity, provider unavailability, malformed local evidence, or any unproved result remains `outcome_unknown`; none is interpreted as failure or permission to push. When the expected and requested tips are identical, their equality is the same exact desired-tip proof and may reconcile. Inconclusive work receives bounded process-local exponential read backoff, a bounded retry table, and rotating scan progress; restart deliberately forgets that optimization and may safely retry the read immediately. One worker serializes its own iterations. Concurrent workers or a lost reconciliation response converge by rereading and accepting only an already-published row whose request, desired remote identity and tip, reconciled disposition, and derived result are semantically identical; a changed terminal row remains a conflict. Cancellation stops promptly without creating retry state. | Enumeration, expiry, observation, and reconciliation expose no publication claim, pre-effect requeue, push-journal start/renew/finalize, Git/filesystem/checkout, model, workflow/gate, repair/controller mutation, full review-bearing provider observation, provider write, acknowledgement, thread/comment resolution, merge, gateway-loop, HTTP/UI, or configuration authority. No path invokes or schedules another Git push. Corrupt cursors, rows, case/thread bindings, observations, or convergence evidence fail closed, and a process-local retry eviction can cause only another read. Production generation-owned scheduling of this worker remains outside this requirement. | An ambiguous post-push result must survive restart and become published only from current exact desired-tip proof, without a duplicate remote effect, starvation, permanent process memory, or a broader provider/Git capability. |

| `FR-EVENT-AUTOMATION-078` | MUST | One event-automation service generation has durable publication work and may run production publication only when event workflows, the generation-specific workflow executor, its exact selected `RunStore`, the resolved global/repository attention policy source, persisted local-CI evidence, the parked-line read projection, owner-agent attention-runtime acquisition, distinct full and head-only provider observers, the parked-line pusher factory, the schema-v18 store, and the outer runtime-generation acquire function are all present. | `newPRDevelopmentPublicationRuntime` treats that dependency set as one all-or-none static boundary. A missing or typed-nil capability returns no runtime and starts neither worker loop. Once either loop has acquired the outer service generation, one serialized initialization resolves the parked workspace and a separately wrapped `PublicationPinnedLinePusher` before any durable read, claim, expiry, or reconciliation; it then constructs the policy processor, active executor with the exact supplied run store, pending/gate-waiting/push-ready handlers, complete dispatcher, claim-only `PublicationWorker`, and head-only `PublicationOutcomeReconciliationWorker` together and caches them only for that service generation. `PublicationWorker` owns only the one-method claim capability, requests exactly one due pre-effect publication with a bounded handoff lease that is strictly shorter than every composed handler-owned renewal lease, and transfers the exact returned claim to the dispatcher without renewing, requeuing, finalizing, or reclassifying handler errors. The gateway runs publication and reconciliation as separate polling loops, and each complete iteration remains inside its own outer generation lease. | Durable mutation remains exclusively in the already-defined phase handler, queue, push-journal, and outcome-reconciliation seams. Zero-gate and successful active-gate paths both first record `push_ready` and are later reclaimed through the same one push-ready handler; neither gains an inline push path. A human-task wait returns to reservation-free `gate_waiting` and releases its iteration generation. Reload cancels and joins both loops before closing CI evidence and the event store or swapping provider/agent runtime state; cached workspace and pusher adapters cannot survive that event-service generation. The replacement runtime receives the new policy/runtime generation but every already-pinned policy, subject, provider observation, and decision-run link remains authoritative and is consumed before any current producer. This requirement adds no schema, configuration, route, DTO, UI, provider-write, review acknowledgement, thread/comment resolution, or merge mutation. | Disabled workflows, an incomplete static graph, unavailable Git resolution, cancellation before initialization, or generation mismatch reaches no durable publication operation and leaves queued or unknown work for a later complete generation. Claim errors return no work; an impossible claim result larger than one is rejected without dispatch; a handoff lease equal to or longer than any handler renewal lease is rejected at construction; exact handler errors remain owned by that handler, so the worker never invents a cross-phase cleanup. Initialization constructs neither loop partially, gives reconciliation only the non-widenable head observer, gives push only the narrow pusher, and never substitutes a fallback run store, live policy, generic Git manager, full provider verifier, or process override before dependency validation. Shutdown or reload waits for active gate, provider, Git, or reconciliation work to return before its store/evidence/provider/agent/Git generation can be retired. Publication success, a private run, gate answer, provider observation, Git receipt, retry value, or browser-visible `local_ready` still grants no review acknowledgement or pull-request merge authority. | Production publication must finally drain durable work without allowing partial wiring, reload, fallback storage, or a scheduler lease to widen least-authority phase boundaries, strand one origin, duplicate a remote effect, or reinterpret publication as acknowledgement or merge approval. |
| `FR-EVENT-AUTOMATION-079` | MUST | An authenticated operator requests the current provider status or bounded provider snapshot for one exact stored `prc_` review case, or explicitly asks to resolve or unresolve one projected review thread using its current opaque token. | The active event-service generation constructs a GitHub review provider only when the exact generation-fenced pull-request read tool is ready, and enables thread mutation independently only when the exact review-write tool is also ready. Status performs only the case-bound pull-request read. A full snapshot strictly cross-binds the stored connector, repository, pull number, and canonical pull URL; reads bounded review history and review threads with capped pagination, byte, item, and public-response budgets; reports explicit availability, completeness, capabilities, and limitations; and omits every raw provider thread ID. Each mutable thread instead receives a random-key HMAC token bound to the runtime generation, case, provider identity, thread identity, and observed state. Resolve and unresolve rescan live threads, select solely through that token, invoke only the narrow thread action, reconcile an uncertain tool response by rereading, and return a fresh bounded snapshot. The protected gateway exposes exact case-owned provider routes, while the authenticated launcher replaces browser authority with the managed process bearer on exact bounded numeric-local no-proxy/no-redirect requests. The browser client strictly validates every response field, collection, scalar, URL, token, and aggregate budget before exposing a typed value. | Status and snapshot reads mutate no review case, workflow, model, repository, Git state, review submission, acknowledgement, merge state, or browser persistence. An explicit thread action may change only that one provider thread. When an MCP result references a configured artifact, consumption accepts only a regular euid-owned file beneath a strictly resolved private artifact root, rejects links and unsafe parents, reads through a no-follow handle, and removes only the exact unchanged owned artifact after success or failure; inline results require no filesystem mutation. Tokens and their random signing key remain process-local and expire on generation replacement. | Missing read readiness yields a safe case-bound unavailable projection; missing write readiness disables mutation. Noncanonical routes, queries, methods, bodies, case IDs, actions, tokens, content types, encodings, browser provenance, local gateway authority, redirects, provider identity, JSON, pagination, counts, sizes, paths, permissions, or artifact ownership fail closed through bounded public errors. Canceled or expired contexts propagate. Incomplete but structurally safe provider history remains explicitly partial; incompatible provider output is never presented as live truth. A stale, changed, or foreign token conflicts without guessing a thread or retrying a write. No provider result grants review submission, pull-request merge, arbitrary MCP execution, raw artifact access, or generic GitHub mutation authority. | Operators need current review state and narrowly confirmed thread maintenance without treating captured data as live, exposing provider identifiers or runtime credentials, or widening one UI action into general repository authority. |
| `FR-EVENT-AUTOMATION-080` | MUST | An authenticated dashboard user opens canonical default `/reviews`, follows a repository, pull-request, role, or review-case deep link, filters tracked work, refreshes captured or live state, opens the existing review editor, or uses an explicitly available provider-thread control. | The responsive portfolio independently drains the review-case and own-PR-development cursor streams, rejects repeated cursors, retains partial pages, deduplicates by public case ID, and groups by case-insensitive repository plus pull number while preserving distinct cases and independent review/develop roles. Repository and pull-request summaries distinguish pending work, finished PicoClaw review work, provider-verified or explicitly captured closed pull requests, and actionable input; newer finished work cannot hide older actionable work. After captured pagination settles, review-role items receive bounded four-way live status reads, and a selected review case receives one bounded live snapshot. Current provider state supersedes stale captured closure, while unavailable live state leaves captured workflow state visible and labelled. Clean `/reviews` is the overview; only canonical bounded `repo`, `pr`, `filter`, `role`, and `review_case` values identify portfolio depth. `view=review` plus a canonical `prc_` case opens the existing editor and retains a safe portfolio return context. Filtering and field/operator/value autocomplete support status, role, attention, author, reviewer, number, updated, and text using `=`, `!=`, `~`, quoted values, and `AND`. Review and development remain distinct views; unfinished development is an explicit placeholder. | Portfolio reads, status checks, snapshot refreshes, filtering, selection, pagination, role changes, and navigation create no durable event, case, model or workflow run, checkout, Git/CI action, push, acknowledgement, or merge. Canonical selection/filter state alone enters the URL; repository search, cursors, DTOs, autocomplete state, drafts, provider tokens, errors, and mutation ambiguity stay memory-only. The review editor keeps its existing exact case mutations and dirty-edit fence. Only an explicit resolve or reopen control with a server-projected capability and opaque case-bound token may invoke `FR-EVENT-AUTOMATION-079`; refresh never repeats that mutation. | Initial list failure remains retryable. Later-page failure or repeated cursor stops automatic pagination, retains and labels partial data, and requires explicit retry; a deep link remains loading until both streams settle, then distinguishes unavailable from not found. Invalid route or query values canonicalize away or report a located parse error without applying it. Live-status failures show checked, total, and failure progress while preserving captured counts; partial or incompatible snapshots explain limitations and omit unsupported controls. Older status or snapshot responses cannot regress newer live state, and an in-flight thread mutation stays bound to its original case/query key. Missing roles, cases, unsafe URLs, unavailable history, empty data, long identities, keyboard autocomplete, and narrow widths remain operable. | Operators need one honest portfolio across outgoing review work and own-PR development without collapsing replay/history, confusing finished work with closed provider state, presenting captured state as live, or turning discovery into hidden automation authority. |

For `FR-EVENT-AUTOMATION-075`, adapting a publication gate snapshot into the
shared attention projection preserves both review stages: the attempt ledger
entry must bind the completed orchestration's mutation-stage fence hash, while
the adjacent passed-review entry must bind the final reviewed fence hash.
Neither proof may be substituted for the other.
...
the local high-water: a linked requeued `pending` decision is queued and
claimed-from-`pending` is checking, both without response authority;
`gate_waiting` and claimed-from-wait may expose the current human
task, and `push_ready`, claimed-from-ready, `push_started`, and `published`
expose only a consistently completed run. A post-gate waiting task is corrupt.
A terminal conflict, failure, or recovery publication may expose only a
completed or failed run, never a still-waiting or recovery task.
A newer local attempt makes the earlier publication historical and hides that
history. These lifecycle distinctions remain private; the browser receives the
same aggregate status and turn DTO as the local-review source.
...
Acquire was never called. If Acquire may own a pin, detached release must be
confirmed before a separate SQLite failure transaction; these two stores do
not form one atomic effect. Pin invocation is the commit-ambiguous boundary:
its success or error leaves Bootstrap reclaimable and must not trigger guessed
release or terminal failure. A confirmed pre-Pin terminal failure clears claim
suppression only for an initial controllerless run; a later run preserves the
retained controller's suppression ownership.

An ordinary pin may be held during an actively claimed Bootstrap, and exact
replay or recovery may preserve previously acquired authority. The controller
mutation reservation is otherwise confined to mutation and its explicit
...
`FR-EVENT-AUTOMATION-068` adds a schema-v17 terminal alternative to the
standalone v12/v13 non-Park recovery finalizers described in
`FR-EVENT-AUTOMATION-060` and `061`. When the generation-owned worker owns the
claim, it must not issue the fresh mutation lease those earlier APIs return;
instead it stores the exact recovery result, enters `suspension_pending`, and
retires fresh into `suspended`. Park continues to use the existing v13 atomic
finalizer unchanged. The standalone contracts remain valid for explicitly
authorized callers, but they are not the automatic worker path. Automatic v12
selection is bound-line-only; no migration or scanner turns an unbound v12 row
into release authority.

For `FR-EVENT-AUTOMATION-063`, an eligible cohort may later have a controller in
`suspended` after recovery. Its queued attempt remains orchestration-owned, but
the controller worker must use the schema-v17 suspended-resume prepare/Git/
finalize sequence before compaction or edit-model execution instead of the
ordinary parked-line Resume operation. Only its exact finalized mutation lease
allows the existing edit, candidate, CI, Commit, and Park sequence to continue.
For `FR-EVENT-AUTOMATION-067`, both `suspension_pending` and `suspended` are
explicit non-ready controller phases even when an older attempt has persisted
green CI or review evidence.
...
Every unmarked controllerless pre-v14 provider session remains legacy-owned.

After `FR-EVENT-AUTOMATION-063`, `FR-EVENT-AUTOMATION-055` continues to own
public admission, revision, idempotency, and safe projection. Schema v14 records
immutable controller-orchestration eligibility only when a new provider-thread
repair session is first created under v14. The v14 scanner owns queued attempts
only for that marked cohort or an exact migrated `ready` retained-controller
owner, both of which the legacy scanner excludes. Every unmarked controllerless
pre-v14 provider session—including a legacy-pinned session with no controller
and every later attempt appended to it—remains wholly legacy-owned; its queued,
...
scanner still starts with the event store when optional execution dependencies
are missing, so already queued eligible Bootstrap work can reach a bounded safe
failure rather than remain suppressed.

For `FR-EVENT-AUTOMATION-058`, an exact committed Bind retry is no-write only
after the same non-regressing-time and live-deadline checks as other mutation
operations. It is not replayable after expiry; an expired post-Bind retry enters
the schema-v12 recovery path only when no schema-v13 operation is active. For
`FR-EVENT-AUTOMATION-060`, a recoverable bound expiry starts only after Bind has
durably established `mutation_epoch == line_version + 1`; an unbound intent is
finalizable only while the Git workspace is still unadopted. A pre-v13 crash
between Git Adopt/Resume and eventing Bind remains deliberately ambiguous and
is never relabeled as a safe rotation. `FR-EVENT-AUTOMATION-061` prevents that
ambiguity for every newly prepared effect: while its operation is unfinished,
expiry changes and claims the operation row itself, and no pending v12 row may
compete. The mandatory v12 row linked during Adopt/Resume/Commit recovery is
inserted only in already-finalized audit form after the composite or rotated
effect has been proven.

For `FR-EVENT-AUTOMATION-061`, a caller-durable operation ID is exactly
`pdop_` plus 32 lowercase hexadecimal characters. Canonical request and result
evidence are each limited to 32 KiB, and one controller retains at most three
operations per possible line fence: 24,576 rows total. Capacity and required
controller-revision headroom are proven before prepare or recovery staging.
...
Public repair availability and new admission require the generation-owned
provider verifier, repair runtime and agent readiness, concrete controller
workspace manager, context agent and identity, and constructed local-CI runner.
The schema-v14 controller worker is still constructed with its Store-only
minimum when those execution dependencies are unavailable, so existing fresh
unpinned Bootstrap work can fail safely or durable checkpoints remain
reclaimable instead of being stranded. The legacy maintenance worker likewise
continues its compatibility reconciliation. Readiness checks use the configured
default before session creation and the stored immutable session agent
afterward, so reload cannot silently switch an established checkout/model
identity merely because the default changed. Durable and public repair
instructions and summaries are each bounded to 4 KiB. Together with the
existing case, 256-message/4-MiB transcript, and
...
one transaction: each row retains the exact old and fresh bearers only while
rotation is unresolved, their globally tracked digests, the expired mutation
lease proof, the exact unbound pin or bound-line fence, and a domain-separated
previous/intent hash. Claim identity, token, deadline, epoch, and count form a
separate reclaimable recovery lease that grants no mutation authority.
Finalization records the exact rotation-result and recovery-claim proofs,
chains a final hash, erases both row bearers, and installs only the fresh bearer
under a new controller mutation lease. Repeated recoveries are causally ordered;
each expiry adds one controller revision and each finalization adds one revision
and lease epoch without advancing the Git line epoch. The chain is capped at
8,192 intents to match the retained workspace's rotation inventory; capacity
failure occurs before another bearer is staged. A migrated legacy
`recovery_required` controller has no row and remains deliberately
unrecoverable rather than receiving synthesized authority.

Schema v13 adds controller-private
`pr_development_controller_operation_intents`. Operation identities, ordering,
requests, and hash links are append-only and contiguous per controller, with at
most one non-final row; only its fenced lifecycle and append-once result/proof
fields may advance. Each row retains a
canonical exact union of the Adopt/Resume/Commit/Park request, its request hash,
the prepared controller/attempt/source/line and mutation-lease fence, the
current reservation digest, previous/intent hash, and—only after success—the
canonical result, result hash, final controller revision/phase/fence, and final
hash. While recovery is unresolved the same row owns its status, recovery and
claim identities, epochs/deadlines, expired lease proof, and, except for Park,
one globally fresh replacement bearer; raw old authority remains on the
controller and any staged replacement is erased at finalization. It never owns
a second pending schema-v12 claim. A recovered Adopt/Resume/Commit links an
already-finalized v12 evidence row without making that row actionable. Park has
no replacement and atomically joins operation finalization to repair-attempt
completion when queued, session-version advancement, immutable review-fence
append, reservation retirement, and `review_pending`. A pre-controller already
completed attempt is unchanged. Migration creates no operation or inferred Git
evidence, and complete aggregate reads validate every operation and cross-table
high-water/hash/reservation invariant before returning private state.

Schema v14 adds the private
`pr_development_repair_orchestration_cohorts` eligibility table plus
controller-private `pr_development_repair_orchestrations`, keyed one-to-one by
repair attempt. A cohort row is inserted atomically only for a new
provider-thread session admitted by v14 and uniquely owns that thread; migration
deliberately marks no existing session. An exact existing `ready` retained
controller independently makes its matching owner eligible; other unmarked
controllerless sessions and their later attempts remain legacy. The
orchestration phase, claim, immutable provider/workspace pin, source tree,
controller binding, model markers, exact local-CI receipt, Park operation,
ledger result, timestamps, and hashes form the restart checkpoint for the
composed worker. Raw scheduling
tokens and failure replay proofs remain private; read-only lookup redacts the
live claim token. Scheduling claims are reclaimable only at `bootstrap`,
`edited`, and `validated`; those phase names do not assert that no external
effect was prepared. Exact operation intents independently fence any prepared
or ambiguous Adopt, Resume, Commit, or Park effect and route it through its
operation recovery contract. Receipt fields cover every terminal CI status and
bind the exact controller revision, line,
mutation epoch, lease and reservation digests, parent/candidate evidence,
attestation, effective plan, result key, execution, and model result. Normal and
recovered Park validate that tuple and append the attempt ledger entry inside
the same transaction that completes the operation, public attempt, session,
...
Schema v17 extends the controller phase domain with `suspension_pending` and
`suspended` and adds private append-only suspension-handoff plus
suspended-resume records. A `suspension_pending` controller retains its exact
source/workspace/line/version/epoch/tip/tree and recovery-source attempt but has
no controller mutation or review lease and no controller reservation. Its one
active handoff binds the tagged v12/v13 recovery identity and claim epoch,
operation kind, exact request/result hashes and canonical result bytes,
old/fresh reservation digests, rotation proof, a domain-separated suspension
intent distinct from every recovery/rotation intent, and the expected line
fence. Commit additionally binds the complete prepared `CommitPinned` request,
its exact result, and any recognized applied-with-workspace-drift outcome.
Only this unresolved handoff holds the fresh raw bearer after its checkpoint;
all raw old copies and all other fresh copies are erased atomically. A
`suspended` controller has the same retained line fence, no lease or raw bearer
of any kind, and one terminal handoff carrying only the exact Git suspension
result, candidate evidence, optional prepared-Commit child/tree/applied
evidence, non-authorizing digests, timestamps, and hash-chain proof. The source
v12/v13 recovery is terminal only in that same transaction. Park never creates
a handoff and retains the schema-v13 `review_pending` terminal tuple.

One tagged recovery-work claim operation first stages eligible expired bound
mutation/operation state under the existing v12/v13 rules, then selects the
oldest pending or expired-claimed actionable recovery by stable creation
position and identity. It never selects a live foreign claim or an unbound v12
row. Claim token, epoch, deadline, and worker label remain in the existing
private recovery source and may be renewed; `suspension_pending` remains
reclaimable through that same source claim. Exact checkpoint and terminal
replay validate the complete handoff and controller aggregate without another
write. Migration from v16 rebuilds and validates only the necessary phase
constraints and creates empty v17 tables/indexes; it creates no claim,
checkpoint, suspension, resume, reservation, attempt, or inferred result.

A suspended-resume record is prepared only for the exact latest suspended
handoff and one live queued-attempt orchestration claim. It binds that new
attempt, controller and expected revision, complete suspension fence, a fresh
globally unused bearer and digest, and a caller-durable Git resume intent before
the filesystem call. While pending, the controller remains `suspended` and the
row alone holds the raw bearer. Exact Git resume finalization atomically records
the result, changes the controller to `mutation` for that queued attempt under
one fresh mutation token/epoch/deadline, installs the bearer as its sole live
reservation, and erases the staged row copy. Exact replay after the transaction
recognizes only that immediate mutation owner and result. No context compactor,
edit model, CI, review, or gate may start while the resume record is pending.

Schema v18 adds one controller-private `pr_development_publications` table.
Each `pdpub_` row is unique for its exact passed review-ledger occurrence and
immutably binds case/thread/controller/owner session; attempt and completed
orchestration receipt; adjacent attempt/review ledger IDs and hashes; review
fence ordinal/hash; green CI plan/result digests; source repository/ref/original
commit/tree; and the complete workspace/line/version/mutation-epoch/Park/base/
tip/tree/no-change tuple. Supporting composite indexes let foreign keys bind the
same fence, receipt, and typed ledger hashes rather than only their mutable
parents. Migration creates no row; only the passed-review completion transaction
may append the first occurrence.

Progressive private fields create-once pin canonical policy and subject bytes
with source revisions and hashes, immutable caller-supplied provider facts with
their first `ProviderPinnedAt` instant and latest `ProviderObservedAt` instant,
the expected remote tip, a deterministic decision-run ID, the canonical full
push request, and an exact proven result. Start may refresh only
`ProviderObservedAt`; it cannot rewrite the provider facts or their original pin
instant. Lifecycle fields retain
`status`, the pre-claim `claim_from`, fresh scheduling owner/token/deadline/epoch,
claim and attempt counts, availability, bounded sanitized error code/detail,
and effect/completion timestamps. `pending`, `claimed`, `gate_waiting`, and
`push_ready` carry no mutation reservation. A partial unique controller index
permits at most one `push_started` row and is paired with controller mutation
admission so neither a new mutation nor push start can pass the other inside an
immediate transaction. Terminal proven publication retains `applied`,
`already_current`, or `reconciled` plus local-cleanliness/drift evidence;
unproved started work retains `outcome_unknown` without reusable effect
authority. Its optional reconciliation evidence is a distinct minimal
repository/pull/head-repository/head-ref/current-tip/observed-at record and
hash. Reconciliation preserves the original `outcome_unknown` completion
instant, neither replaces the original provider/review pin, nor carries review
state. No field has a JSON
projection, and credentials, raw provider/Git
output, runtime/system prompts, model responses, workflow-private roots or
sessions, raw conversation, credentials, checkout paths, or raw errors are not
stored. Configured gate criteria/questions inside the bounded policy pin and
the bounded canonical gate-subject envelope are the only workflow inputs the
journal deliberately retains.

The schema-v18 safe-requeue transition adds no state. It changes a live
pre-effect `claimed` row back to its exact `claim_from` origin and updates only
`available_at`, `updated_at`, and the cleared scheduling-owner fields. Policy,
subject, provider, decision-run, expected-tip, local evidence, counters, and
recorded parked-line identity survive unchanged. The retained claim epoch plus
restored origin and exact availability form the committed-replay fence. That
already-restored no-write replay resolves before the live clock/lease checks and
may therefore still succeed after its availability is due; no started or
terminal state has a requeue edge.
...
roots that contain no `.git` directory and exist only for one callback. A
`PinnedTreeManifest` canonically orders every materialized entry and binds its
repository-relative path, mode, type, size, and content with full SHA-256; its
digest, candidate parent/tree/digest, and repository namespace are immutable
inputs to local CI. The versioned local plan separately binds both snapshots'
definition digests, dependency digests, ordered required steps, invocations,
working directories, dependencies, limits, discovery/policy versions, and
completeness. An explicit `.picoclaw/ci.yml` or `.picoclaw/ci.yaml` suppresses
all inferred sources. Without it, discovery selects supported Make, package,
or Go repository-native quick steps before considering a supported GitHub
pull-request workflow. GitHub jobs with multiple executable steps are rejected
because accepted steps intentionally have independent fresh sandboxes. The
resolved plan graph is persisted under the exact parent/candidate manifest and
discovery/plan-version key. Definition bytes changing between parent and
candidate produce a non-passing `plan_changed` result; a dependency-only change
alters environment identity without changing the normalized plan definition.
The execution result binds that plan and manifest to exact platform, sandbox
backend/profile, trusted toolchain and dependency snapshot, allowlisted
environment, per-step outcome, and bounded output digest. Production execution
evidence persists, but exact-success result reuse remains disabled until mutable
host toolchains and controller-provided dependency mounts have complete
immutable manifests. Disposable paths, raw environment values, process
identifiers, and raw output are not cache keys or public/model evidence.
...
catalog. `rule_sets` maps at most 1,025 stable canonical IDs to permanent,
case-insensitively unique names and maps at most 128 canonical decision points
per set to ordered gate lists. The fixed `default` ID and `Default` name must
always exist, but `default_rule_set_id` may select any live set as the fallback.
`repository_assignments` maps at most 1,024 case-insensitively unique, trimmed
`owner/repo` identities of at most 256 bytes to live set IDs; multiple
repositories may select the same set, including the built-in `default` set.
The complete catalog contains at most 8,192 policies, at most 8,192 gate
entries, and at most 1 MiB of canonical JSON. Set rule maps are objects rather
than null and gate lists are arrays rather than null. Every gate uses the same
shared workflow bounds and JSON-compatible `questions` contract. Persistence
validates every stored set and assignment before API save or active runtime
use. Legacy `global` and repository override catalogs remain read-compatible
only long enough to materialize their effective behavior into this named-set
projection; a saved catalog uses only the named-set representation.
...
The browser policy editor holds a separate transient array-shaped draft so new
set names and repository assignments can be validated and duplicate or
case-collision errors can be shown before constructing the map-shaped transport.
Existing set names are rendered read-only. Gate question JSON
...
The editor presents the catalog as reusable rule sets. The built-in, permanently
named `Default` set always exists and initially contains no rules, so every known
moment is visibly Off and no attention decision is triggered by configuration.
Operators may edit that set, duplicate any set under a new permanent unique name,
and edit the independent copy. A separate `Used by default` selection chooses the
set used by repositories without an explicit assignment; it does not rename the
selected set. Exact `owner/repository` assignments select one complete set, many
repositories may share one set, and removing an assignment returns that repository
to the current fallback. Repository intake remains separate and is linked through
Event sources. Rules choose a known human-labeled moment or an advanced custom
identifier, and checks choose a working-agent, fresh-AI, fixed-condition, or
no-action starting point with generated valid IDs and editable starter values.
The exact decision keys, four gate kinds, ordered composition, custom points, and
lossless question JSON remain available. Switching an existing check to a type
that cannot retain its type-specific fields requires explicit confirmation before
the draft changes. Editing a shared set affects only future unpinned decisions for
its assigned repositories, or for unassigned repositories when it is the current
fallback; an already pinned decision remains immutable.

Known decision choices are editor affordances, not a restrictive enum.
`pr_development.before_push` and custom canonical decision points coexist in
every rule set, and the selected set's gates use the ordinary ordered
working-context AI, isolated AI, deterministic, and zero-gate resolver. The
selected-set editor is derived from that draft only; it neither selects nor pins
a runtime policy. Saving a before-push change can affect only a future
publication that has not pinned its policy under
`FR-EVENT-AUTOMATION-072`. Active execution consumes that immutable pin under
`FR-EVENT-AUTOMATION-075`, and a replacement runtime preserves it under
`FR-EVENT-AUTOMATION-078`. The editor warns that a before-push
working-context gate's agent must be the immutable repair-session owner; the
editor cannot grant or infer that case authority, and the runtime fails closed
on a mismatch. A policy GET, draft edit, assignment preview, or configuration
save runs no model, edits no code, executes no CI, invokes no Git or push,
acknowledges no review, resolves no review thread, and merges no pull request.

The immutable runtime source owns two revisions. `CatalogRevision` hashes a
domain-tagged canonical ordering of every detached set, immutable name, current
fallback, and case-normalized repository assignment. A selected snapshot hashes
a different domain-tagged value containing only the normalized authoritative
case repository, exact decision point, selected stable set ID, and its gates for
that decision. Thus an unrelated set, assignment, or decision edit changes the
catalog generation without changing an already selected decision, while a
selected set or gate change deterministically changes future decision identity.
An absent decision is a valid no-op selection rather than an unavailable policy
source.
...
Owns: CODE pkg/gateway/pr_development_attention.go
...
Owns: TEST pkg/gateway/pr_development_attention_composition_test.go
...
Owns: TEST web/frontend/src/api/pr-development-attention.test.ts
...
| Config | `reviews.attention.rule_sets`, `default_rule_set_id`, `repository_assignments` | Persist a built-in fixed-name empty-safe `Default` set, reusable immutably named decision-rule sets, one current fallback, and exact repository-to-set assignments. Known decisions including `pr_development.before_push` and custom canonical points coexist in each set; reviewed repositories cannot supply or weaken the catalog. Legacy global/repository trees are read compatibly and materialized without changing their effective behavior. | `FR-EVENT-AUTOMATION-045`, `FR-EVENT-AUTOMATION-046` |
...
| HTTP | `GET`, `PUT /api/reviews/attention-policies` | Authenticated bounded named-set projection and same-origin strict full replacement, fenced by the opaque public-plus-security revision, persisted by an atomic raw public-JSON patch that leaves unrelated values and the security sidecar unchanged, and returning canonical catalog revision plus current gateway effect. The server fixes the built-in identity/name, rejects renaming an existing set, requires every fallback and assignment reference to remain live, and permits one set to be shared by many repositories. Browser clients parse and serialize the complete transport losslessly rather than rounding arbitrary question numbers, and replacement itself performs no decision, development, CI, Git, push, acknowledgement, thread-resolution, or merge effect. | `FR-EVENT-AUTOMATION-046`, `FR-EVENT-AUTOMATION-047` |
...
| Frontend | authenticated `/reviews` route | Responsive case master/detail workbench with status/repository filters, pagination, finding edit/drop/restore, case/finding chat, rephrase preview and explicit apply, stale-version reload that preserves local text, all-dropped resolution, explicit submission confirmation, durable status polling, terminal-state locks, and warned human reconciliation of unknown submissions as found or verified absent. Its canonical Rule sets view keeps names read-only, duplicates sets under a new permanent name, edits known and custom decision moments with ordered mixtures of all four gate kinds, selects the current fallback, and assigns one complete set to any number of exact repositories. It preserves lossless question JSON, config-CAS conflict handling, unsaved-navigation protection, restart feedback, future-unpinned-decision scope, and the before-push owner-agent fail-closed warning. Rule-set work never launches a decision or performs model, code, CI, Git, push, acknowledgement, thread-resolution, or merge work. The canonical `/reviews?case={case}&focus=chat` handoff selects the case and focuses its existing conversation card after render, where bounded attention history, status, questions, and the current in-memory response editor appear without placing response authority in the URL or browser storage. | `FR-EVENT-AUTOMATION-042`, `FR-EVENT-AUTOMATION-043`, `FR-EVENT-AUTOMATION-047`, `FR-EVENT-AUTOMATION-048`, `FR-EVENT-AUTOMATION-049` |
...
| Internal Go API / storage | `eventing.PRDevelopmentLedgerStore`, `eventing.PRDevelopmentContextReader`, private schema-v11 ledger tables, `prdevelopment` thread-context projector | Append exact post-validation attempt accounts, atomically finalize lease-authenticated local reviews with their exact outcomes in fence order, retain structured findings and raw audit rows, append only digest-bound logical checkpoints, atomically snapshot thread/ledger high-water, and build one bounded ordinal-ordered untrusted context without running any model, Git, CI, filesystem, or provider effect. | `FR-EVENT-AUTOMATION-059` |
| Internal Go API / storage | `eventing.PRDevelopmentControllerStore`, private schema-v12 recovery-intent storage | Atomically bind each expired mutation to old-to-fresh reservation evidence, lease and reclaim only its exact idempotent transfer, then authenticate the matching transfer result while installing a fresh mutation lease and erasing staged bearer copies. | `FR-EVENT-AUTOMATION-060` |
...
| Internal Go API / storage / runtime | `eventing.PRDevelopmentControllerRecoveryScanner`, `eventing.PRDevelopmentControllerSuspensionExecutionStore`, `eventing.PRDevelopmentControllerSuspendedResumeStore`, `eventing.PRDevelopmentControllerSuspendedResumeRecoveryStore`, schema-v17 suspension handoff/resume storage, `prdevelopment.ControllerRecoveryWorker`, and gateway generation composition; controller-only Git rotation, composite recovery, Commit, Park, suspension, and suspended-resume methods | Atomically stage and claim/reclaim the oldest eligible bound v12/v13 recovery; renew its scheduling-only lease; checkpoint every exact non-Park recovery result before Git suspension; finalize only the exact suspension into bearer-free `suspended`; directly finalize Park; and prepare/replay/finalize one later exact suspended resume before model execution. All methods use tagged exact source identity and claim/revision fences, erase staged raw bearer copies at authority transitions, exclude legacy unbound v12 rows, and expose no generic executor or provider capability. | `FR-EVENT-AUTOMATION-068` |
...
| Private storage / Go API | `eventing.PRDevelopmentPublicationPushAuthentication`, `PRDevelopmentPublicationPushClaimAuthenticator`; `Store.AuthenticateClaimedPRDevelopmentPublicationPush` | Prove one exact live claimed-from-ready token/epoch against complete current local high-water and return only the authority-redacted authoritative publication, exact immutable case, and provider thread identity. It cannot renew/requeue, read a provider, start/finalize the journal, invoke Git, or acquire reservation/provider-write/acknowledgement/thread-resolution/merge authority. | `FR-EVENT-AUTOMATION-076` |
...
| Internal Go API | `prdevelopment.PublicationDispatcher`, `PublicationPendingClaimHandler`, `PublicationGateWaitingClaimHandler`, and `PublicationPushReadyClaimHandler` | Validate and route exactly one already-claimed publication by its durable origin without acquiring, renewing, requeuing, or transitioning it. Construction requires all three phase handlers; the ready handler completes that internal set, while the dispatcher still cannot own a production claim loop. | `FR-EVENT-AUTOMATION-075`, `FR-EVENT-AUTOMATION-076` |
| Internal Go API | `prdevelopment.PublicationPushReadyHandler`, `PublicationPushReadyStore`, `PublicationPinnedLinePusher`, and the separate queue/push heartbeat helpers | Authenticate and renew one caller-owned ready claim, repeat the immutable provider pin, derive and durably start the exact parked-line request, lend Git only when `newlyStarted=true`, hand renewal to the push journal, and classify uncertainty. It finalizes uncertainty while journal authority remains live; definitively stale started authority stays inert for the store expiry transition. Historical start/finalize replay never repeats Git, the branch remains parked and reservation-free, and no production queue/provider-write/acknowledgement/thread-resolution/merge authority is added. | `FR-EVENT-AUTOMATION-076` |
...
| Private storage / Go API | `eventing.PRDevelopmentPublicationPushJournal`; `Store.RenewPRDevelopmentPublicationPush`, `StartPRDevelopmentPublicationPush`, and `FinalizePRDevelopmentPublicationPush` | Own only exact live `push_started` renewal, the transactionally revalidated write-ahead effect boundary, and exact request-hash/result finalization. It cannot renew or claim pre-effect work and obtains no provider/Git capability; a caller may invoke an external effect only after separately receiving a newly started journal record. | `FR-EVENT-AUTOMATION-069` |
| Private storage / Go API | `eventing.PRDevelopmentPublicationOutcomeReconciler`; `Store.ExpirePRDevelopmentPublicationPushes` and `ReconcilePRDevelopmentPublicationOutcome` with `PRDevelopmentPublicationOutcomeReconciliation` and distinct minimal `PRDevelopmentPublicationRemoteObservation` | Convert expired started work to non-reclaimable unknown and record read-only reconciliation only when a separately obtained fresh head observation proves the desired tip. This capability cannot claim work, invoke push, or require/overwrite the original review-bearing provider observation. | `FR-EVENT-AUTOMATION-069` |
| Private storage / Go API | `eventing.PRDevelopmentPublicationDecisionRunStore`; `Store.GetPRDevelopmentPublicationDecisionRun` and `AdmitPRDevelopmentPublicationDecisionRun` | Create-once bind one immutable semantic decision key to one deterministic private run through the callback transaction. RunID creation must be idempotent and replayable; a non-nil callback error guarantees no external run was created. A nil return followed by commit uncertainty returns `ErrPRDevelopmentPublicationAdmissionUncertain`; automatic callers must not invoke create again and may use only an explicit exact pre-start terminalization or recovery path. | `FR-EVENT-AUTOMATION-069` |
| Private provider-read Go API | `prdevelopment.PublicationProviderObserver`, `PublicationRemoteHeadObserver`, `NewGitHubPublicationProviderObserver`, and `NewGitHubPublicationRemoteHeadObserver` | Issue separate non-widenable capabilities that project either one causally timed exact PR/review publication observation or one causally timed review-independent current head observation through generation-fenced `github/pull_request_read`; expose no generic runner, provider write, store, workflow, Git, push, retry, acknowledgement, or merge capability to callers. | `FR-EVENT-AUTOMATION-070` |
...
| Go API / storage / HTTP / UI | `eventing.PRDevelopmentWorkbench.LocalEvidence`, `prdevelopment.Detail.local_development`, the protected and launcher selected-case detail GETs, and the PR-development local-status card | Extend the existing atomic workbench snapshot with private controller/orchestration/ledger evidence, then declassify only the latest public attempt lifecycle, exact candidate/no-change fact, terminal CI status and opaque fingerprints, reservation-free review status/outcome/summary/count, and strictly derived local readiness. The browser strictly cross-binds the object to the latest public attempt, retains only monotonically newer memory state, polls while repair or review remains active, renders bounded evidence plus an explicit local-only notice, and receives no route, schema, write, raw evidence, or action authority. | `FR-EVENT-AUTOMATION-067` |
...
| Internal Go API | `reviews.ConfigAttentionPolicySource` | Validate and detach the operator rule-set catalog, case-fold trusted repository assignment selection, expose stable full-catalog and selection-scoped revisions, and enumerate configured AI/working-context agents for runtime preflight. | `FR-EVENT-AUTOMATION-046` |
| Internal Go API / storage / runtime | `attention.PolicySource`, `attention.PrivateRunner`, `eventing.PRDevelopmentAttentionSnapshotReader`, `eventing.PRDevelopmentAttentionDecisionRunStore`, `prdevelopment.AttentionLauncher`, and the inert gateway composition | Reuse ordered ordinary workflow gates from the assigned or fallback set; atomically snapshot and bind one exact reservation-free attention-required review, protected conversation revision, CI receipt, and parked Git review to one deterministic private run while preserving zero-gate no-op behavior and starting no trigger worker. | `FR-EVENT-AUTOMATION-065` |
| Storage / runtime / HTTP / UI | `eventing.PRDevelopmentAttentionTriggerQueue`, `prdevelopment.AttentionTriggerWorker`, `prdevelopment.AttentionBridge`, protected and launcher attention routes, and the PR-development conversation panel | Atomically emit and lease one exact local-review occurrence, pin its configured mixed-gate policy and subject before private effects, converge exact retries, project only a case-owned attention conversation, and resume one opaque-fenced human task while leaving chat, repair, branch mutation, and provider effects separately authorized. | `FR-EVENT-AUTOMATION-066` |
| Private storage / Go API | `eventing.PRDevelopmentPublicationAttentionProjection`, the publication fields on `PRDevelopmentAttentionTriggerCaseSnapshot`, `Store.GetCurrentPRDevelopmentAttentionTriggerForCase`, and the list-only case attention predicate | Under one atomic integrity-checked read, retain the local-review source and optionally bind the exact current passed-review publication, pinned private decision, and local high-water; reject simultaneous current sources and expose only the coarse safe union through the existing public list boolean. | `FR-EVENT-AUTOMATION-074` |
| Internal Go API / HTTP / UI | `prdevelopment.AttentionBridge`, publication decision-key helpers, the unchanged case-owned attention routes/DTO, and the own-PR `Needs input`/`PR decision gates` presentation | Select only the atomic snapshot's one current source, re-derive and validate its private decision/run chain, keep response fences source-separated, project completed publication-gate history read-only, and resume only a current waiting publication gate without adding a source field, route, DTO authority, or direct code/provider effect. | `FR-EVENT-AUTOMATION-074` |
...
| Build | `pkg/eventing` unsupported-platform implementation | Preserves the same construction, development-case capture/read/conversation, and private publication-store surface, including safe pre-effect requeue, and returns `ErrUnsupportedPlatform` without pulling SQLite into excluded targets. | `FR-EVENT-AUTOMATION-005`, `FR-EVENT-AUTOMATION-012`, `FR-EVENT-AUTOMATION-051` through `FR-EVENT-AUTOMATION-053`, `FR-EVENT-AUTOMATION-069`, `FR-EVENT-AUTOMATION-073` |

| Internal Go API | `prdevelopment.PublicationWorker`, `PublicationWorkerConfig`, and the private claim-only queue | Claim at most one due pre-effect publication with a stable bounded handoff lease strictly shorter than every composed handler renewal lease, then transfer the exact claim to the already-complete dispatcher. The worker owns no renewal, requeue, gate, run, provider, Git, journal, finalization, reconciliation, acknowledgement, or merge capability and returns phase-handler errors unchanged. | `FR-EVENT-AUTOMATION-078` |
| Gateway runtime | `prDevelopmentPublicationRuntime`, `prDevelopmentPublicationRuntimeConfig`, `prDevelopmentPublicationPusher`, and the two event-automation worker loops | Require the exact run store and every policy/evidence/workspace/runtime/full-observer/head-observer/pusher/store/generation capability as one static set; after the outer generation is held, resolve narrow Git readiness and construct both workers together. Run claim and reconciliation separately, cache adapters only for that service generation, and join both before event-store/evidence/provider/agent/Git-generation teardown or reload. | `FR-EVENT-AUTOMATION-078` |
...
42. To materialize one trusted policy generation, normalize legacy catalogs to
    reusable sets, validate and JSON-detach the complete operator catalog,
    normalize configured repository keys to lower case while rejecting
    collisions, and preflight all AI agents plus working-context session stores
    before opening event storage. Canonically sort set, decision, and assignment
    maps to derive the whole-catalog revision. During selection, resolve an exact
    assignment or the current fallback and hash only the domain tag, lower-cased
    authoritative case repository, exact decision point, selected stable set ID,
    and detached selected gates, then invoke the consumer
...
43. In the authenticated review rule-set view, strictly and losslessly parse the
    catalog before creating one transient array-shaped draft. Fence asynchronous
    catalog and agent reads to the selected hydration generation; never replace
    dirty state from a background read. Keep every existing name read-only,
    require a new permanent name when duplicating, expose the fallback selection
    separately from exact repository assignments, validate the complete
    projected catalog plus exact configured-agent identities, and issue one full
    PUT with the hydration config revision only after explicit save. On success,
    rebuild the
...
52. When opening schema v10, create and structurally validate the empty
    schema-v11 ledger tables before advancing `user_version`; infer no attempt,
    review, finding, or checkpoint evidence. Trusted later appends derive their
    immutable controller/fence facts and advance only one contiguous
    attempt/review chain, while compaction appends a digest-bound summary and
    preserves every raw row.
53. When opening schema v11, create and structurally validate empty schema-v12
    recovery storage. On a later mutation expiry, atomically generate and
    collision-check one fresh reservation, append the exact expired-lease and
    old-to-fresh hash-chained intent, and enter `recovery_required`. Claim only
    that immutable transfer under an independent renewable/reclaimable token.
    After the caller separately performs the exact Git-workspace rotation,
    finalize only its matching result by replacing the controller bearer,
    issuing a fresh mutation token/epoch/deadline, recording final proof, and
    erasing staged raw keys. Do not recover a legacy row, pending park, changed
    fence, or re-expired result, and execute no Git, filesystem, worker, model,
    CI, workflow, provider, UI, or publication effect in these store methods.
54. When opening schema v12, create and structurally validate empty schema-v13
    operation storage before advancing `user_version`; synthesize no operation
    for an old controller or recovery row. Under one live exact mutation lease,
    validate the complete controller, owner attempt, line, prior operation
    chain, kind-specific request, and required exit headroom, then append one
    caller-identified pending operation before any Git call. Permit only the
    causal Adopt-or-Resume, optional Commit when content changed, then Park
    order for that attempt; an explicit no-change Park skips Commit. As the
    sole migration prefix, an empty operation chain whose live v12 controller
    is already bound at its active mutation epoch may start with Commit or
    no-change Park; later attempts cannot skip Resume. On a live
    exact result, finalize Adopt/Resume into their line binding, record Commit's
    exact result without parking the mutating line, or finalize Park by
    atomically retiring the bearer, finalizing the operation, appending the
    review fence, entering
    `review_pending`, and—only when the owner attempt is still queued—recording
    its completion, clearing active ownership, and advancing the session once.
    Preserve an already completed legacy owner attempt exactly. Permit an exact
    Prepare replay to no-write return the already-finalized current-tail Commit
    only under its unchanged operation identity, request, and authority, so the
    trusted runner can reverify the idempotent `CommitPinned` result, no-write replay
    Finalize, and proceed to Park. An ambiguous non-finalized prepared effect
    remains recovery-owned. If the mutation
    lease expires while an operation is pending, transition and claim that same
    row under its own renewable/reclaimable recovery lease; do not create a
    pending v12 row. While continuously owning that claim, have the controller
    separately call the Git composite Adopt/Resume old-to-fresh recovery,
    rotate old-to-fresh then exact Commit for Commit, or exact old-bearer Park
    for Park. Finalize only the matching result and current claim; install fresh
    mutation authority for Adopt/Resume/Commit, retire it for Park, erase staged
    raw keys, and add linked already-finalized v12 audit evidence for
    the first three kinds. Exact replay of a normal finalized Commit is
    no-write when its causal result, operation-chain tail, controller
    revision/phase/current attempt, and live bearer token/epoch/reservation
    remain exact and only heartbeat has monotonically advanced its lease
    deadline and aggregate update time. Adopt, Resume, every recovered
    operation, and Park require their exact immediate resulting state. Changed
    authority or result, an expired normal Commit bearer, or non-immediacy
    outside the Commit-only heartbeat allowance fails closed. Store
    methods execute no Git, model, CI,
    workflow, provider, UI, or publication effect.
55. For one controller-requested local validation, keep the exact mutation
    reservation operation lock while Git Workspaces revalidates the supplied
    pin, exact pre-attempt parent, and current candidate evidence, materializes
    separate bounded `.git`-free disposable roots, and computes a canonical
    full SHA-256 candidate manifest plus tree/candidate identities. Discover
    supported validation definitions and dependency inputs from both roots
    without executing repository content. Give one explicit local plan complete
    authority; otherwise prefer repository-native quick steps and consult
    supported pull-request GitHub workflows only as fallback. Reject GitHub jobs
    with multiple executable steps because accepted steps receive fresh state.
    Canonically order and digest the resulting workflow-like steps; require at
    least one required step. If any definition or semantic step changes, return
    incomplete `plan_changed` evidence and execute nothing. Treat dependency-
    only drift as a changed environment/result identity. Persist/reuse discovery
    only under exact parent/candidate manifests and discovery/plan versions.
    Materialize only the candidate into the mandatory Bubblewrap sandbox after
    a successful user-systemd cgroup-v2 handshake, install a clean allowlisted
    environment and controller-provided read-only offline dependencies, disable
    network and ambient host authority, execute every required step in stable
    order within per-step and aggregate time/output/process limits, and terminate
    the complete cgroup process tree on cancellation or limit exhaustion. The
    current mutable-host backend persists execution evidence but does not promote
    or reuse successful results. Revalidate the manifest and candidate fence,
    remove disposable roots and proven-quiescent process/scratch state, and
    return canonical evidence with bounded per-step outcomes and output digests.
    If quiescence cannot be proven, quarantine the owner-local scratch and return
    non-green. Cleanup, postflight, environment, sandbox, evidence-validation,
    missing offline inputs, or any required step failure cannot produce `passed`.
    This algorithm creates no controller,
    attempt, ledger, commit, park, retained-line, review, workflow-run, gate,
    model, provider, HTTP/UI, acknowledgement, publication, push, or merge
    effect.
56. When opening schema v13, create and structurally validate the empty
    schema-v14 repair-orchestration and immutable session-cohort tables before
    advancing `user_version`; mark no existing session and synthesize no
    checkpoint, receipt, attempt outcome, or ledger entry. On first
    provider-session admission under v14, atomically mark that session eligible
    only when its thread has no prior session owner. A trusted worker claims the
    latest queued attempt only for that cohort or an exact existing `ready`
    retained-controller owner, or reclaims an expired exact `bootstrap`,
    `edited`, or `validated` checkpoint. Both eligible owner forms are excluded
    from the legacy scanner. Every other unmarked pre-v14 session and its later
    attempts remain legacy-owned. Expire `editing` to
    `recovery_required` instead of invoking its model again. Heartbeat the
    scheduling claim independently from the controller mutation lease and
    cancel all borrowed work if either renewal loses authority.
    Load the exact attempt, selected case, thread, admission-time conversation
    prefix, provider captures, and ledger high-water. When projection exceeds
    its limit, compact only a fully reviewed contiguous prefix through a
    separate isolated no-tool model, append its digest-bound checkpoint, retain
    all raw rows, reload the authoritative snapshot, and retry projection.
    Reverify the selected provider case before filesystem mutation. For the
    first attempt, acquire the session reservation at the exact refreshed head,
    prove the checkout clean and snapshot its source tree, then atomically pin
    those facts to the session and orchestration. For a retained-line attempt,
    require the immutable session pin and current controller workspace/tree
    instead of reacquiring the retired initial reservation.
    Acquire the exact orchestration controller, Adopt an unbound first line or
    Resume a `ready` line under its fresh mutation reservation, and persist the
    exact context plus fixed edit-prompt digests before calling the edit-only
    runner. Persist the result digest, bounded summary, and iterations after it
    returns. Snapshot the exact parent/candidate evidence and execute local CI
    under attempt-derived stable owner and attestation identities, including an
    explicit parent-tree-equal no-change candidate. Accept only a persisted,
    self-consistent terminal attestation and atomically record its complete
    controller, candidate, plan, result, execution, lease, reservation, model,
    and status binding as an immutable receipt. Treat only `passed` as green;
    preserve every other exact terminal status without relabeling it.
    Reconstruct the attempt-derived write-ahead identities after restart.
    Commit a changed validated candidate with the attempt-derived deterministic
    message and authored time, or create no Git commit for exact no-change.
    Before Park, build the bounded immutable review preview; reject unsafe or
    oversized review material while mutation authority remains recoverable.
    Keep both heartbeats active through review preview, operation prepare, Git
    Park, and the exact post-Park snapshot. Once that snapshot equals its
    preview, drain and pause both renewals at the immediate pre-finalize barrier
    while retaining the still-live credentials for one SQLite transition. In
    that same transaction finalize the Park operation, append the fence and
    next attempt ledger row from the receipt, complete the public attempt,
    advance the session once, retire mutation authority and the scheduling
    claim, and enter
    `review_pending`. Keep the parked branch and immutable snapshot retained.
    A valid non-green receipt follows the same commit, Park, and ledger path;
    later gates decide attention and publication. Before the first durable Pin
    invocation, a dependency failure that never called Acquire may fail the
    unpinned attempt directly. If Acquire may own an ordinary pin, first release
    it under a detached cancellation-safe context; only a separately confirmed
    release permits the later SQLite failure transition. That transition clears
    suppression only when no retained controller exists; a later attempt keeps
    controller-owned suppression. A canceled worker releases any possibly owned
    pin but leaves the unpinned orchestration baseline reclaimable.
    Once Pin is invoked, either success or error is commit-ambiguous: do not
...
58. When opening schema v16, transactionally add the empty schema-v17
    suspension-handoff and suspended-resume storage and extend the controller
    phase constraints before advancing `user_version`; infer no result, claim,
    bearer, suspension, resume, or attempt transition. Run one
    provider-independent recovery iteration only while holding the exact active
    event-runtime generation. In one bounded immediate transaction, stage any
    eligible expired bound v12/v13 recovery under its existing rules, skip every
    unbound v12 row, and claim or expired-claim-reclaim the oldest actionable
    source by stable creation position and identity. Heartbeat that source claim
    independently of controller mutation authority and cancel Git work on loss.
    Before creating a replacement owner, verify under the Git reservation locks
    that rotation, suspension, and one future resume all retain history capacity.
    For bound v12 call exact `RotatePinnedReservation`; for Adopt or Resume call
    the exact composite recovery; for Commit call exact rotation and then the
    immutable prepared `CommitPinned` request under fresh; for Park call only
    exact old-bearer `ParkPinnedLine`, snapshot its parked fence, and use the
    existing atomic Park recovery finalization without replacement or
    suspension. A Commit result that proves the deterministic child despite
    post-effect workspace drift remains exact recovery evidence rather than a
    reason to repeat or discard that effect.

    For every non-Park result, before invoking suspension, authenticate the
    still-live claim and append-once store the complete canonical recovery and
    rotation result, expected line fence, fresh bearer and digest, and a
    domain-separated suspension intent distinct from the recovery/rotation
    intent; include the complete prepared Commit request and exact Commit result
    for Commit. In the same transaction clear controller mutation authority,
    erase every raw old copy and duplicate fresh copy, retain the sole fresh raw
    value in that claim-scoped handoff, and enter `suspension_pending`. On retry,
    load that checkpoint rather than repeat a completed Git recovery. Call
    `SuspendPinnedLine` for bound-v12/Adopt/Resume or
    `SuspendPinnedLineCommitRecovery` for Commit. Then atomically validate its
    exact candidate, optional prepared child/tree/applied state, request/result,
    claim epoch, and controller revision; terminalize the source recovery, enter
    `suspended`, and erase the handoff bearer and every claim token. A crash
    before checkpoint exactly replays the kind effect; after checkpoint it
    replays only suspension; after terminal commit it returns the exact current
    suspended result without Git.

    When a later eligible queued attempt reaches its bootstrap checkpoint,
    before context compaction or edit-model invocation, atomically bind that
    attempt and live orchestration claim to the latest suspension, a globally
    fresh bearer, and one durable resume intent. Call exact
    `ResumeSuspendedPinnedLine`, then atomically verify its immediate result,
    move the bearer from the resume record into the sole live controller
    reservation under a fresh mutation lease, change current attempt and phase
    to `mutation`, and erase the staged copy. A crash before Git reuses the
    prepared intent; a crash after Git replays it; a lost finalization response
    recognizes only the exact immediate mutation owner. No model, CI, review,
    gate, or publication work may start earlier. Always derive
    `local_ready=false` for `suspension_pending` or `suspended`. On reload or
    shutdown, cancel, drain, and join the generation-wrapped worker before
    closing its Git manager or event store; provider/model readiness is not a
    worker dependency.
59. When opening schema v17, transactionally create the empty schema-v18
    publication journal, its exact-evidence supporting indexes, lifecycle
    checks, unique passed-review occurrence, unique nonempty decision-run
    binding, and one-controller `push_started` exclusion before advancing
    `user_version`; do not scan or backfill historical reviews. During one exact
    `passed` review completion, after validating the completed orchestration,
    green non-compatibility CI receipt, adjacent immutable attempt/review ledger
    pair, latest attempt, ready reservation-free controller, and complete parked
    fence, append the review and publication occurrence in the same immediate
    transaction. Exact replay returns the same occurrence; any occurrence error
    rolls back the complete review/fence/controller/ledger mutation.

    Claim due `pending`, `gate_waiting`, or `push_ready` in stable order under a
    fresh scheduling token, epoch, and deadline. The Queue renews only that live
    pre-effect `claimed` lease. Reclaim only an expired
    pre-effect `claim_from`; the distinct outcome reconciler scans expired
    `push_started` work into `outcome_unknown` without returning it for effect
    execution. Create-once pin
    canonical policy then subject and provider evidence, preserving the first
    provider-pin instant separately from the latest identical observation
    instant refreshed at push start; exact replay compares hashes and any change
    conflicts. The separate DecisionRunStore binds a semantic key over
    publication, review, policy, subject, and provider observation to one
    deterministic private run in the same transaction as its create callback.
    RunID creation must be idempotent and replayable; a non-nil callback error
    guarantees that no external run was created. A nil return followed by commit
    uncertainty returns `ErrPRDevelopmentPublicationAdmissionUncertain`;
    automatic callers must not invoke create again and may use only an explicit
    exact pre-start terminalization or recovery transition. Release the
    scheduling claim while the run waits for a human, or record completed/no-op
    gating as `push_ready`; neither state owns a controller mutation lease or
    blocks a later repair.

    Under one immediate transaction, authenticate a fresh push-ready claim,
    reload the complete local high-water, reject later repair or evidence drift,
    require a fresh exact provider observation, persist and hash the complete
    pinned-line request, and enter `push_started`. Controller mutation admission
    must check the same journal while serialized, so mutation and start cannot
    both win. Only after that committed return may a separately implemented
    trusted caller invoke the exact Git primitive. While it remains in flight,
    only the PushJournal can extend the exact live `push_started` lease; Queue
    renewal rejects it. Finalize only the same claim, epoch, and request hash:
    persist a canonical proven result as `published`,
    retain proven publication plus local drift separately, map exact CAS loss to
    `conflict`, and map unproved post-start state to `outcome_unknown`. A future
    read-only reconciler may finalize unknown only from a distinct minimal fresh
    remote-head observation whose current tip equals desired; it preserves the
    original unknown-completion instant and neither reloads nor overwrites the
    original review-bearing provider pin. Every other
    observation remains unknown and never
    re-enters `push_ready`. This algorithm itself invokes no workflow, model,
    provider, Git, gateway, worker, acknowledgement, or merge action.
60. For the initial publication-gate processor, require one caller-supplied
    integrity-checked publication with a live claim whose durable origin is
    `pending`; do not claim, renew, release, or enumerate work. If the policy is
    absent, prepare it once from the trusted reusable-set attention source for
    the exact case repository and fixed before-push decision point,
    then pin its canonical bytes and revisions. If it is present, decode only
    the pin and never consult current configuration. Return
    `requires_execution` immediately for any composition containing an active
    working-context AI, isolated AI, or deterministic gate.

    For an empty, disabled, or all-zero composition, require the ordinary gate
    compiler to confirm a no-op. If the subject is absent, atomically load the
    claimed gate-context snapshot, build one bounded canonical versioned zero
    subject over immutable publication/review identity and that conversation
    version/digest, and pin it through the existing conversation compare-and-
    swap. If a subject pin already exists, use it without rebuilding or reading
    later chat. If provider evidence is absent, make one full read-only
    observation and pin it; otherwise make no provider call. Finally mark the
    same claim push-ready with an empty run ID. Retain completed pins and the
    live claim after transient failure; a separately authorized caller may use
    the safe pre-effect requeue primitive in requirement 073. Terminalize only
    an existing store-proven provider/local conflict,
    supersession, or exact pre-start terminal outcome. Do not create a run,
    execute a workflow/model, touch a checkout/filesystem, invoke Git/push or a
    provider write, change config/schema, or construct a gateway/worker.
61. For safe transient pre-effect deferral, let the trusted caller classify the
    failure before storage and compute a delay with the pure
    `prdevelopment.PublicationRetryDelay` helper from the durable claim count.
    The helper returns deterministic exponential delays from one second through
    a one-minute saturation, with no jitter, counter overflow, storage access,
    or scheduling side effect. Add that delay to a caller-controlled canonical
    time and submit the exact publication ID, token, epoch, `claim_from`, and
    `AvailableAt` to `RequeuePRDevelopmentPublication`. In one immediate
    transaction, first resolve an already-restored no-write replay only when its
    retained epoch, origin, and availability match exactly, even if that
    availability is now due. Otherwise authenticate the still-live pre-effect
    claim and exact origin, reject availability before the sampled store time,
    restore only that origin, and clear only scheduling ownership. Preserve
...
62. For active before-push gates, begin with one already-claimed publication;
    ...
    construction. This gate slice supplies only pending/gate-waiting behavior;
    requirement 076 supplies the separately fenced push-ready implementation.
    Neither requirement constructs the later production dispatcher owner or
    starts a claim loop over the shared queue.
...
63. For a push-ready publication, begin with one caller-owned exact live claim
    whose durable origin is `push_ready`; do not scan the queue or trust a
    caller's detached pins. Immediately renew and authenticate the claim, then
    use the returned authoritative publication, case, and thread identity.
    Require complete gate/provider/local/Park evidence, obtain a fresh full
    provider observation while the queue heartbeat remains live, and require
    it to repeat the pinned provider facts. Derive the exact parked-line push
    request only from those durable facts and the repeated expected remote tip.

    At the effect barrier, drain queue renewal and write the complete request
    through `StartPRDevelopmentPublicationPush`. A missing response may be
    retried only with the identical observation, observation time, and request.
    Treat only `newlyStarted=true` as authority for one call to
    `PushPinnedLine`, and use the request returned by that start. An exact
    historical start or a replay after a lost successful start response
    performs no Git call. Once started,
    heartbeat only through the push journal until Git returns; drain it at the
    immediate finalization barrier, and never requeue started work or reacquire
    a mutation reservation. Map a complete success, including one joined with
    local drift, to published; map a definite pinned-line conflict to
    `conflict/push_conflict`; map invalid or definitely unavailable push to
    `failed/push_failed`; and map cancellation, uncertainty, invalid/partial
    result or unclassified post-start failure to `outcome_unknown` before
    considering any joined conflict or workspace-drift error. If journal
    renewal fails while the exact started claim remains live, finalize that
    uncertainty immediately; if authority is already stale, leave the inert
    `push_started` record for the store's expiry transition to
    `outcome_unknown`. Recover a lost finalization response only by replaying
    the identical finalization input, never by starting or invoking Git again.

    Before start, terminalize only observation-proven provider drift or exact
    store-proven local conflict/supersession, and requeue a classified transient
    failure only to the authenticated ready origin with every pin intact. Leave
    the local branch parked and reservation-free. Perform no provider write, review
    acknowledgement, thread/comment resolution, merge, configuration, UI,
    production queue, or unknown-outcome reconciliation action.
...
configuration outside every reviewed checkout; a repository assignment means a
trusted `owner/repo` selector in that catalog, not a file controlled by the PR.
Event automation also owns the structured review rule-set draft, all four gate
controls and their ordered mixtures, assignment preview, validation,
...
`FR-EVENT-AUTOMATION-067` extends only the selected-case detail projection with
one atomic, bounded view of the latest attempt's already-persisted local
candidate, CI, and reservation-free review evidence. It neither changes the
case list nor reads raw CI output/findings, and its derived local readiness is
not a provider or publication fence.
...
`FR-EVENT-AUTOMATION-055` owns public admission and the legacy edit-only
lifecycle: one explicit instruction creates one leased attempt, the legacy
worker re-verifies and pins before editing, and ambiguous execution is terminal
...
v13 Commit intent. Git Workspaces owns the candidate, reservation lock,
commit-object verification, compare-and-swap, and index reconciliation; Event
Automation owns the attempt ledger and recovery state that decide when this
primitive may be called.
...
using its absolute ordinal and authenticated mutation-stage hash. Migration
therefore fabricates no old description or review result. Each later
attempt/review pair is contiguous; logical checkpoints cover only a
review-complete prefix and never rewrite or delete raw evidence. The v2 context
...
claim, and atomic cross-table finalization. [Git workspaces](git-workspaces.md)
owns the exact filesystem effects and reservation inventory: composite
Adopt/Resume old-to-fresh recovery, rotation plus deterministic Commit, and
old-bearer Park. A trusted controller must keep the operation-recovery claim
...
Git prerequisite for releasing reconciled but idle mutation authority.
`FR-EVENT-AUTOMATION-068` owns the `#125b` generation-owned, provider-independent
worker and schema-v17 cross-store ordering. Its non-Park path uses the existing
v12/v13 recovery claim only to reconcile the exact Git effect, persists that
complete result before entering `suspension_pending`, and then retires the fresh
bearer into `suspended`; it does not use the older finalization path that would
issue another model-capable mutation lease. Park continues through the existing
...
parked one-ref remote-tip compare-and-swap primitive. Requirement 076 now calls
it only after exact ready-claim authentication and durable journal start; Event
Automation still does not acquire arbitrary transport authority, alter
`local_ready`, acknowledge a review, resolve a thread, or merge. Event Automation
...
start/result/unknown state can be fenced. Requirement 070 now supplies the
narrow fresh full and review-independent head-only GitHub observation boundary,
and requirement 071 supplies the atomic claimed gate-context and first-subject
conversation fence. Requirement 072 composes only an existing claimed input:
it calls the full observer after compiler-confirming a zero policy, while active
...
construction requires a real handler for that phase. PR `#136` now supplies the
exact `PushPinnedLine` adapter and fenced push-ready handler without installing
the queue loop. PR `#137` supplies restart-safe, read-only unknown-outcome
reconciliation. Requirement 078 now supplies the generation-scoped queue owner
and all-or-none gateway composition: one exact run store is shared by gate
execution and waiting observation, full and head-only observers remain distinct,
Git readiness is resolved through narrow adapters only after acquiring the
current runtime generation, and separate publication/reconciliation loops drain
before that generation is replaced.
...
reservation lock, and the full canonical manifest. Event Automation owns the
bounded deterministic plan, persistent exact-manifest discovery index,
environment/result identities, and result evidence. Explicit plan files are
authoritative; repository-native quick profiles precede the supported GitHub
fallback, whose multi-executable jobs are rejected because local steps run in
fresh independent sandboxes. [Security isolation](security-isolation.md) owns
the mandatory Bubblewrap plus user-systemd/cgroup-v2, no-network execution and
non-fallback boundary. Controller-provided read-only dependency mounts are the
only offline dependency input; provisioning is not part of this slice, so a
missing input is non-green. Production exact-success reuse is disabled until
toolchains and dependency mounts have complete immutable manifests, although
discovery and execution evidence persist. [Workflows](workflows.md) provides the
reusable workflow-like step vocabulary and ordering model only: local CI
neither invokes the generic workflow executor nor creates a workflow run, task,
gate, private context, event, or dispatch. The trusted repair controller in
`FR-EVENT-AUTOMATION-063` consumes the exact attested evidence before Commit and
Park; a `passed` result alone still does not mutate a controller or authorize
review, provider access, or publication.

| Concern | Owning slice | Explicit boundary after schema v13 |
| --- | --- | --- |
...
| Shared attention policy/runtime, durable PR-development decision admission, exact local context, and inert gateway composition | `#122` / `FR-EVENT-AUTOMATION-065` | Reuses ordinary mixed workflow gates and the repository-selected reusable rule-set catalog, but launches only from an explicit trusted local call and grants no trigger, response, repair, provider, publication, or browser authority. |
...
| Durable publication admission, private mixed-gate/provider pins, write-ahead push journal, terminal uncertainty, and mutation/start exclusion | `#126b1` / `FR-EVENT-AUTOMATION-069` / Security Isolation 043 | Implemented as schema-v18 private eventing state only. Passed-review admission is atomic and exact; pre-effect scheduling remains reservation-free; Queue and PushJournal renew only their respective pre-effect and started claims; only `push_started` excludes mutation; unknown never returns to execution. No provider, gate, workflow, model, Git, gateway, or worker effect is composed. |
| Fresh full and review-independent head-only provider observation | `#126b2` provider-observer prerequisite / `FR-EVENT-AUTOMATION-070` / Security Isolation 044 | Implemented as narrow timed, read-only GitHub adapters over the existing generation-fenced exact verifier. They own no store, gate, Git, push, provider-write, acknowledgement, merge, or retry authority; requirement 078 composes them only into their distinct full-preflight and head-only reconciliation consumers. |
| Atomic claimed publication gate context and conversation-fenced first subject pin | `#126b2` storage prerequisite / `FR-EVENT-AUTOMATION-071` / Security Isolation 045 | Implemented as one private, integrity-checked SQLite snapshot available only to an exact live claim originating from `pending`. It returns the claim-redacted publication plus the complete local evidence graph, and first-pin conversation version/digest CAS prevents concurrent chat drift while preserving exact post-pin replay. It runs no provider, workflow, model, Git, filesystem, gateway, or worker effect. |
...
| Active publication mixed-gate execution and already-claimed phase dispatch | `#135` / `FR-EVENT-AUTOMATION-075` / Security Isolation 049 | Implemented without schema/config/UI changes by reusing ordinary mixed workflow primitives, a canonical private subject/provider/decision pin chain, an exact protected transcript-prefix freeze for working-context gates, reservation-free wait, and run-status-to-journal transitions. The dispatcher accepts only one caller-owned claim and requires distinct handlers for all reclaimable origins; production queue ownership remains outside the dispatcher. |
| Fenced push-ready execution and exact `PushPinnedLine` adapter | `#136` / `FR-EVENT-AUTOMATION-076` / Security Isolation 050 | Implemented as an internal handler over one caller-owned live ready-origin claim. It repeats the immutable provider pin, derives and write-ahead records the exact parked-line request, calls Git only for a newly committed start, hands renewal from the pre-effect queue to the push journal, and deterministically classifies the result or uncertainty, finalizing only while exact journal authority remains live and otherwise leaving inert started state for store expiry, without acquiring a mutation reservation, changing the retained branch, or gaining provider-write/acknowledgement/thread-resolution/merge authority. |
| Restart-safe read-only unknown-outcome reconciliation | `#137` / `FR-EVENT-AUTOMATION-077` / Security Isolation 051 | Implemented as bounded durable unknown enumeration plus a least-authority worker that expires abandoned starts first, rotates past locally backed-off rows, reads only the exact provider head, and records only exact desired-tip proof. It never repeats Git and treats process-local backoff as a safe read optimization; requirement 078 now supplies its generation-owned production loop without widening the worker. |
| Generation-owned publication claiming/renewal, publisher cleanup, and full publication/reconciliation loop wiring | `#138` / `FR-EVENT-AUTOMATION-078` / Security Isolation 052 | Implemented as a claim-only one-record worker plus an all-or-none gateway runtime. The runtime resolves parked-workspace and narrow push adapters only inside the current outer generation, constructs every phase handler and the distinct head-only reconciler together over the exact shared run store, and starts separate generation-fenced loops that join before service, store, evidence, provider, agent, or Git-generation teardown. Disabled, incomplete, or dynamically Git-unready composition leaves durable work untouched. No journal row, decision, run, review, observation, retry value, primitive result, or `local_ready` projection grants review acknowledgement or merge authority. |
| Before-push rule-set editor and publication-scope guidance | `#139` / `FR-EVENT-AUTOMATION-046` / `FR-EVENT-AUTOMATION-047` | Keeps `pr_development.before_push` and custom decision points in reusable named sets with ordered mixed gates, one current fallback, and exact repository assignments. It explains the `FR-EVENT-AUTOMATION-072`/`075`/`078` pin boundary and owner-agent fail-closed rule; saving remains configuration-only and cannot start development or publication effects. |
...
- Attention-policy configuration never trusts a repository checkout or request
  path as policy authority. Invalid set references, case-colliding
...
- A crash before schema-v13 prepare has no authorized Git effect. A crash after
  prepare but before exact finalization leaves that operation—not a new v12
  row—as the sole reclaimable recovery unit. Adopt/Resume recovery holds the
  old-to-fresh composite boundary, Commit recovery rotates before exact replay,
  and Park recovery replays and retires only old; a stale worker therefore has
  no valid bearer with which to race the recovered effect.
- A failed Park database transaction cannot expose `review_pending`, a fence,
  or a completed queued attempt independently. All remain in their prior state
  with the operation unresolved; an exact retry or recovery claim must prove
  the Git Park result before the complete terminal tuple can commit. The parked
  branch remains retained after success even though its mutation reservation is
  gone.
- Local-CI discovery with no required step, an unsupported or incomplete
  definition, or any parent-to-candidate definition/semantic-plan drift is
  explicitly non-green. Drift returns `plan_changed`, runs no candidate step,
  and cannot be relabeled from candidate-only discovery or any future result
  cache hit.
  Dependency-only drift changes environment/result identity and invalidates
  reuse even when the ordered plan definition is unchanged. An explicit local
  plan is authoritative; otherwise repository-native quick steps precede
  GitHub workflow fallback. A fallback GitHub job with multiple executable
  steps is rejected because the validator cannot preserve its shared job state
  across intentionally fresh step sandboxes. Exact parent/candidate manifests
  and discovery/plan versions are the complete persistent discovery-cache key.
- A missing or unsupported mandatory local-CI sandbox never falls back to host
  execution or the optional generic isolation runtime. The Linux backend
  requires Bubblewrap and a successful user-systemd delegated cgroup-v2
  supervisor handshake. Candidate commands see neither `.git` nor the retained
  checkout, ambient credentials/configuration, network, provider/agent sockets,
  sibling workspaces, event storage, or the writable evidence cache. Timeout,
  output exhaustion, cancellation, escaped process descendants, failed
  termination, cleanup, or candidate postflight prevents `passed`. If cgroup
  quiescence cannot be proven, the owner-local execution scratch is quarantined
  for operator cleanup instead of being unsafely deleted.
- Dependency mounts are explicit controller-provided read-only offline inputs;
  local CI never downloads or resolves missing packages. A missing required
  mount or executable is non-green `environment_unavailable`; a command that
  discovers missing package contents may instead be non-green `failed`. Later
  controller provisioning must supply either before validation can pass.
- Discovery records and execution evidence remain owner-local and persistent.
  Invalid encoding, failed/nonterminal status, hash mismatch, missing step,
  changed candidate/manifest/plan/environment/toolchain/sandbox/platform
  identity, or altered output digest never produces green. The production
  backend does not promote or reuse an exact success while host toolchains or
  dependency mounts are mutable; a future reusable result requires complete
  immutable manifests and still has no prefix, partial, or fallback match.
- The orchestration scheduling lease and controller mutation lease are separate
  authorities and both must remain live during borrowed mutation work. Expired
  Bootstrap, Edited, or Validated scheduling ownership can be reclaimed only
  from its exact checkpoint; operation recovery independently owns every
  prepared or ambiguous Git effect.
- Before Pin, a failure that never invoked Acquire can fail directly. A possible
  ordinary-pin owner is released under detached cancellation and only a
  confirmed release permits the separate failure transition. Once Pin is
  invoked, success or error is commit-ambiguous and cannot trigger guessed
  release or terminalization.
- A valid non-green receipt still follows Commit-or-no-change, Park, and ledger
  accounting without ever becoming green; only later policy may decide
  attention or publication.
- Successful Park retires normal mutation authority without deleting the
  branch, so `review_pending` is reservation-free. Ambiguous model,
  controller-rotation, Commit, or Park state remains durably recovery-bound
  rather than being guessed or repeated.
- Automatic recovery has three durable replay boundaries. Before the schema-v17
  checkpoint it may repeat only the exact kind-specific recovery; after the
  checkpoint it may repeat only the exact suspension; after terminal
  finalization it may only return the already validated `suspended` result. A
  stale claim or runtime generation cannot cross any boundary. Park has no
  checkpoint/suspension branch and converges only through its exact old-bearer
  effect and atomic existing finalizer.
- Rotation/suspension/resume capacity is proven before old-to-fresh mutation.
  Exhaustion leaves the original recovery and bearer state unchanged rather
  than stranding a fresh `suspension_pending` owner that cannot be retired.
  Changed checkpoint, candidate, prepared Commit, controller revision, result,
  or bearer proof fails closed without reset, release, another Commit, or a
  model call.
- A schema-v12 recovery with no complete bound-line fence is never claimed or
  reclaimed by the generation worker, even when its old claim expires. It stays
  `recovery_required`, retains its existing private evidence, and reports
  `local_ready=false`; neither `ReleasePinned` nor another best-effort cleanup
  substitutes for a separately approved idempotent unbound-retirement policy.
- A queued attempt over `suspended` cannot compact context or invoke its edit
  model until the exact prepared suspended resume has been replayed and
  finalized into the sole live mutation owner. Pending resume or a lost
  generation leaves the candidate and branch unchanged and remains locally not
  ready. Finalization erases the staged resume copy; an exact lost response is
  recognized from the immediate controller owner rather than issuing another
  bearer.
- A fresh unpinned orchestration claim requires public revision capacity for
...
- A passed review and its schema-v18 publication occurrence are one atomic
  database effect. Missing green receipt binding, ledger adjacency, latest
  attempt, or exact controller/fence/source/line evidence rejects both; a
  trigger, storage, or insert failure cannot leave a finished review with
  no corresponding occurrence. Migration and replay never manufacture an
  occurrence for a passed review completed under an older schema.
- Publication scheduling is deliberately nonexclusive before push start. An
  expired claim from `pending`, `gate_waiting`, or `push_ready` may be reclaimed
  with a new token/epoch, while waiting releases its claim entirely. These
  states cannot block a new repair; changed high-water causes the old occurrence
  to become `superseded`. Conversely, `push_started` and controller mutation are
  transactionally exclusive, so exactly one may cross its admission boundary.
- Safe requeue accepts only a still-live pre-effect claim, its exact
  `claim_from`, and an availability no earlier than the store-sampled live
  transition. It restores that origin and clears only scheduling ownership;
  every pin, decision, counter, local-evidence field, and recorded parked-line
  fact is retained without reading current branch state. A lost successful
  response replays only for the same epoch, origin, and availability. That
  already-restored replay resolves before live clock/lease validation and may
...
- The already-claimed dispatcher rejects malformed or non-claimed input and
  ...
  of the pending, gate-waiting, or push-ready handlers fails. The fenced
  push-ready handler now completes the internal phase set, but no gateway/worker
  may connect the dispatcher to `ClaimPRDevelopmentPublications` until the
  separate generation-owned queue runtime exists.
- The ready handler authenticates and renews the exact ready-origin claim before
  observing the provider. It drains that queue renewal before write-ahead start,
  then renews only the push journal. Only `newlyStarted=true` authorizes one Git
  call; historical start replay and exact finalization replay authorize none.
  The branch stays parked and reservation-free, and no provider review-write,
  acknowledgement, thread resolution, or merge authority is introduced.
- Once `push_started` may represent an external effect, expiry or missing proof
  ...
  record reconciled publication; expected, another tip, missing, or unavailable
  evidence remains unknown because remote OID ABA cannot prove no prior effect.
- The schema-v18 store performs no model, gate, workflow, provider, Git,
  gateway, or worker call and projects none of its state to HTTP/UI. Publication
  success does not acknowledge the submitted review or merge the pull request;
  both behaviors remain unimplemented until their user-visible meaning and
...
### Approved PR workflow acceptance matrix

“Boundary path” means one test crosses real SQLite, workflow execution, and a
real local Git effect, with any adapted seams named in the explicit limit.
“Segmented” means deliberately separated capabilities are proved by adjacent
tests; those rows do not claim one browser-to-provider or worker-to-remote
fixture.
For the repair-through-push portion, the boundary fixture substitutes a fake
repair LLM provider, a fake local-review executor, fake gate agents, and fake
provider reads at their named interfaces plus a constrained SSH transport to a
local bare remote. It retains the production repair model runner, controller,
SQLite, local-CI, workflow, Git-workspace, review worker, publication, and push
implementations.

| Approved outcome | Owning requirements | Evidence shape and direct evidence | Supporting evidence | Explicit limit |
| --- | --- | --- | --- | --- |
| Repository opt-in and reusable attention rule-set assignment | `FR-EVENT-AUTOMATION-035`, `FR-EVENT-AUTOMATION-039`, `FR-EVENT-AUTOMATION-046`, `FR-EVENT-AUTOMATION-047`; [Workflow requirement 045](workflows.md); [Launcher requirement 019](launcher-management.md) | Segmented: [event webhook tests](../../pkg/gateway/event_webhook_test.go), [GitHub scope tests](../../pkg/config/events_github_scope_test.go), and [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) | [gate-policy tests](../../pkg/workflows/gate_policy_test.go), [policy API tests](../../web/backend/api/review_attention_policies_test.go), and [policy UI tests](../../web/frontend/src/components/reviews/review-attention-policies-page.test.tsx) | An empty repository allowlist means all repositories. Rule sets and assignments come only from trusted local configuration, never from the PR head. |
| Review another user's PR with AI findings and explicit human-controlled submission | `FR-EVENT-AUTOMATION-041` through `FR-EVENT-AUTOMATION-043`; [Launcher requirement 013](launcher-management.md) | Segmented: [workflow template tests](../../pkg/workflows/templates_test.go), [review worker SQLite tests](../../pkg/reviews/worker_sqlite_test.go), and [review service tests](../../pkg/reviews/service_handler_test.go) | [submitter tests](../../pkg/reviews/submitter_test.go), [review-store tests](../../pkg/eventing/review_store_sqlite_test.go), and [review workbench UI tests](../../web/frontend/src/components/reviews/reviews-page.test.tsx) | The path starts from a targeted `review_requested` event after explicit template installation and submits a `COMMENT`; it is not an arbitrary-PR-URL launcher. |
| A submitted review on the user's own PR is captured and starts a durable local development case | `FR-EVENT-AUTOMATION-050` through `FR-EVENT-AUTOMATION-052`, `FR-EVENT-AUTOMATION-057`; [Workflow requirement 047](workflows.md); [Launcher requirement 015](launcher-management.md) | Boundary path after the authenticated-event and successful-workflow seams: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) invokes the production capture sink, performs fresh fake-provider reads, and persists the case in real SQLite | [capture tests](../../pkg/prdevelopment/capture_test.go), [gateway capture tests](../../pkg/gateway/pr_development_capture_test.go), [development-store tests](../../pkg/eventing/pr_development_review_store_sqlite_test.go), and [development UI tests](../../web/frontend/src/components/reviews/pr-development-page.test.tsx) | The boundary fixture inserts an already-authenticated occurrence and supplies a successful run record; webhook authentication, template installation, and workflow execution are separate tests. Capture still requires the explicitly installed ordinary workflow and fresh provider verification in production. |
| Case chat preserves ordered conversation and lets the user steer each explicit repair | `FR-EVENT-AUTOMATION-053`, `FR-EVENT-AUTOMATION-055`, `FR-EVENT-AUTOMATION-063`; [Launcher requirement 016](launcher-management.md), [Launcher requirement 017](launcher-management.md) | Boundary path: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) records the advisory turn and explicit repair admission, then proves the real repair controller and production `agent.LocalRepairRunner` give the fake LLM provider the exact instruction and bounded admitted conversation context before executing only its guarded `edit_file` request in the manager-owned pinned workspace | [chat tests](../../pkg/prdevelopment/chat_test.go), [local-repair runner tests](../../pkg/agent/local_repair_test.go), [repair-controller tests](../../pkg/prdevelopment/repair_controller_worker_test.go), and [development UI tests](../../web/frontend/src/components/reviews/pr-development-page.test.tsx) | Chat is advisory until the user explicitly starts repair. Only the provider response is fake; operation locking, pre/postflight pin checks, path guarding, edit-only tools, context loading, workspace acquisition, and the resulting lifecycle are production implementations. A later message cannot retroactively alter the frozen prompt of an active attempt. |
| Every changed repair attempt runs bounded local validation, creates a commit, records its concise log with commit/evidence, parks the line, and releases mutation ownership | `FR-EVENT-AUTOMATION-054`, `FR-EVENT-AUTOMATION-056`, `FR-EVENT-AUTOMATION-058`, `FR-EVENT-AUTOMATION-059`, `FR-EVENT-AUTOMATION-061` through `FR-EVENT-AUTOMATION-063`; [Git-workspace requirement 012](git-workspaces.md), [Git-workspace requirement 013](git-workspaces.md), [Git-workspace requirement 016](git-workspaces.md); [Security-isolation requirement 033](security-isolation.md), [Security-isolation requirement 035](security-isolation.md), [Security-isolation requirement 036](security-isolation.md) | Boundary path: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) runs the real repair controller and production edit-only local-repair runner over a real Git-workspace manager, runs production local CI, deterministically commits and parks the resulting guarded edit, appends the CI-linked concise attempt entry in real SQLite, and proves the parked controller has neither mutation reservation nor lease | [repair-controller tests](../../pkg/prdevelopment/repair_controller_worker_test.go), [controller-effect tests](../../pkg/prdevelopment/controller_effects_test.go), [pinned-commit tests](../../pkg/gitworkspace/pinned_commit_test.go), [ledger tests](../../pkg/eventing/pr_development_ledger_store_sqlite_test.go), and [retained-line tests](../../pkg/gitworkspace/development_line_test.go) | The LLM provider response is fake, as allowed by the boundary fixture; it does not replace the repair runner, controller, Git, CI, commit, ledger, Park, or ownership-release transition. Proven no-change and non-green behavior remains segmented evidence, and those attempts cannot create an empty commit or become locally ready or publishable. |
| Local CI/CD is autodiscovered, and exact discovery and execution evidence is persisted for later attempts; failed or incomplete CI never publishes | `FR-EVENT-AUTOMATION-062`; [Git-workspace requirement 016](git-workspaces.md); [Security-isolation requirement 036](security-isolation.md) | Boundary path for the passing case: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) uses the production Linux sandbox and file evidence store to discover the repository's exact targeted plan, execute its required check against the disposable candidate, and bind the persisted plan/execution digests to the committed tree and ledger | [discovery tests](../../pkg/prdevelopment/localci/discover_test.go), [runner tests](../../pkg/prdevelopment/localci/runner_test.go), [evidence-store tests](../../pkg/prdevelopment/localci/store_test.go), [review-worker tests](../../pkg/prdevelopment/review_worker_test.go), and [publication-admission tests](../../pkg/eventing/pr_development_publication_admission_store_sqlite_test.go) | The boundary test runs only on Linux and reports a named skip when the host lacks bubblewrap or canonical user-systemd/DBus/cgroup delegation; supported hosts execute the real sandbox. Failed, incomplete, plan-changed, and unavailable outcomes are deliberately negative segmented cases. The production backend persists discovery and evidence but disables cross-attempt passing-result reuse while its toolchain and dependency mounts remain mutable; exact same-attempt recovery may replay its immutable attestation, while every new attempt executes its checks. |
| Ordered reviews, chat, and attempt logs carry forward; an oversized logical prompt is compacted without deleting raw history | `FR-EVENT-AUTOMATION-059`, `FR-EVENT-AUTOMATION-063`; [Security-isolation requirement 033](security-isolation.md) | Segmented: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go), [thread-context loader tests](../../pkg/prdevelopment/thread_context_loader_test.go), and [thread-context tests](../../pkg/prdevelopment/thread_context_test.go) | [ledger tests](../../pkg/eventing/pr_development_ledger_store_sqlite_test.go) | Compaction changes only the bounded model projection; raw durable history remains available and ordered. |
| Local AI review runs against the parked branch without retaining the repair reservation, and only clean review plus green local CI exposes local readiness | `FR-EVENT-AUTOMATION-064`, `FR-EVENT-AUTOMATION-067`; [Git-workspace requirement 013](git-workspaces.md) | Boundary path for the passing case through the worker boundary: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) runs the real review worker, reloads production file-backed CI evidence, snapshots the exact parked candidate through the real Git-workspace manager under a reservation-free review lease, gives that bounded evidence to a fake `LocalReviewExecutor`, and atomically records the passed review and publication admission | [controller local-review runner tests](../../pkg/agent/controller_local_review_test.go), [review-worker tests](../../pkg/prdevelopment/review_worker_test.go), [review-store tests](../../pkg/eventing/pr_development_review_store_sqlite_test.go), and [development UI tests](../../web/frontend/src/components/reviews/pr-development-page.test.tsx) | The agent-runner/model portion of local review is segmented rather than replaced only at the provider call; review claiming, evidence validation, parked-line reading, reservation release, ledger completion, and admission are production. Non-green, findings, retry, and failure cases remain segmented. `local_ready` itself grants no acknowledgement, push, or merge authority. |
| One selected rule-set decision accepts two or more ordered gates in any mix: working-context AI, isolated-context AI, deterministic workflow checks, and zero gate | `FR-EVENT-AUTOMATION-046`, `FR-EVENT-AUTOMATION-047`, `FR-EVENT-AUTOMATION-065`, `FR-EVENT-AUTOMATION-066`, `FR-EVENT-AUTOMATION-071`, `FR-EVENT-AUTOMATION-072`, `FR-EVENT-AUTOMATION-074`, `FR-EVENT-AUTOMATION-075`, `FR-EVENT-AUTOMATION-078`; [Workflow requirement 043](workflows.md) through [Workflow requirement 046](workflows.md); [Launcher requirement 018](launcher-management.md), [Launcher requirement 019](launcher-management.md); [Security-isolation requirement 052](security-isolation.md) | Boundary path for the four-kind execution: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) pins one ordered zero, false deterministic, isolated-AI, and owner working-context-AI policy, runs it through the real publication gate processor and workflow executor, proves isolated then working model profiles over the exact candidate, and reaches `push_ready` before any receive-pack effect | [publication-gate executor tests](../../pkg/prdevelopment/publication_gate_executor_test.go), [gate compiler tests](../../pkg/workflows/gates_test.go), [rule-set policy tests](../../pkg/reviews/attention_config_test.go), [gate-handler tests](../../pkg/prdevelopment/publication_gate_handler_test.go), and [private-context tests](../../pkg/workflows/private_context_security_test.go) | Repeated kinds and human-wait resume remain segmented. The working-context agent must equal the repair-session owner; isolated, deterministic, and zero gates receive no repair session. Both boundary-path AI gates decide without asking the user. |
| A working-context model may escalate to the case-owned PR chat; an exact response resumes the same run, while the human wait releases publication scheduling and owner-runtime leases and retains the parked branch | `FR-EVENT-AUTOMATION-048`, `FR-EVENT-AUTOMATION-049`, `FR-EVENT-AUTOMATION-065`, `FR-EVENT-AUTOMATION-066`, `FR-EVENT-AUTOMATION-074`, `FR-EVENT-AUTOMATION-075`, `FR-EVENT-AUTOMATION-078`; [Launcher requirement 014](launcher-management.md), [Launcher requirement 018](launcher-management.md); [Security-isolation requirement 048](security-isolation.md), [Security-isolation requirement 049](security-isolation.md), [Security-isolation requirement 052](security-isolation.md) | Segmented: [gate compiler tests](../../pkg/workflows/gates_test.go) prove the working model's ask and same-run resume; [publication-gate executor tests](../../pkg/prdevelopment/publication_gate_executor_test.go) prove owner-runtime acquisition and release; [publication attention-bridge tests](../../pkg/prdevelopment/publication_attention_bridge_test.go) prove the exact case/version/response fence and same publication run; [gate-handler tests](../../pkg/prdevelopment/publication_gate_handler_test.go) prove durable claim release | [attention-trigger tests](../../pkg/prdevelopment/attention_trigger_worker_test.go), [attention-store tests](../../pkg/eventing/pr_development_attention_store_sqlite_test.go), [publication-attention store tests](../../pkg/eventing/pr_development_publication_attention_store_sqlite_test.go), [case-attention tests](../../pkg/prdevelopment/attention_test.go), [retained-line tests](../../pkg/gitworkspace/development_line_test.go), [gateway composition tests](../../pkg/gateway/pr_development_attention_composition_test.go), and [development UI tests](../../web/frontend/src/components/reviews/pr-development-page.test.tsx) | No single fixture currently combines a working-context ask, browser response, and live Git-workspace inventory; the separately fenced tests prove those composed capabilities. Attention is in-app only and adds no email, chat-service, or provider notification. |
| Successful gates publish exactly the reviewed parked tip through a write-ahead, at-most-once push boundary | `FR-EVENT-AUTOMATION-069`, `FR-EVENT-AUTOMATION-070`, `FR-EVENT-AUTOMATION-076`, `FR-EVENT-AUTOMATION-078`; [Git-workspace requirement 018](git-workspaces.md); [Security-isolation requirement 042](security-isolation.md) through [Security-isolation requirement 044](security-isolation.md), [Security-isolation requirement 050](security-isolation.md), [Security-isolation requirement 052](security-isolation.md) | Boundary path: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) crosses the real SQLite write-ahead journal and private workflow run into `push_ready`, then gives the real push handler and `gitworkspace.Manager.PushPinnedLine` the exact parked fence; the constrained SSH shim routes that production Git transport to a local bare remote, whose source ref reaches exactly the reviewed commit in one receive-pack call | [bare-remote push acceptance](../../pkg/gitworkspace/development_line_push_bare_remote_test.go), [publication admission tests](../../pkg/eventing/pr_development_publication_admission_store_sqlite_test.go), [push-handler SQLite tests](../../pkg/prdevelopment/publication_push_handler_sqlite_test.go), [push-handler tests](../../pkg/prdevelopment/publication_push_handler_test.go), and [publication runtime tests](../../pkg/gateway/pr_development_publication_runtime_test.go) | At the client boundary, the shim replaces network and credential handling while requiring the production noninteractive SSH hardening options; it starts the local bare server with a separate clean environment. The local bare repository substitutes for hosted-endpoint policy, but does not replace the manager's local, parked-fence, cleanliness, remote-CAS, refspec, or result validation. The fixture proves terminal replay causes no second receive-pack, the base ref is unchanged, and the reservation-free parked line remains readable. Browser-visible publication lifecycle remains intentionally private. |
| Gate failure, provider drift, stale local evidence, pre-effect local drift, authority loss, and supersession prevent push; proven remote success followed by local drift records published-with-drift instead of pretending rollback | `FR-EVENT-AUTOMATION-069` through `FR-EVENT-AUTOMATION-076`, `FR-EVENT-AUTOMATION-078`; [Workflow requirement 046](workflows.md); [Git-workspace requirement 018](git-workspaces.md); [Security-isolation requirement 043](security-isolation.md) through [Security-isolation requirement 050](security-isolation.md), [Security-isolation requirement 052](security-isolation.md) | Segmented: [gate-processor tests](../../pkg/prdevelopment/publication_gate_processor_test.go), [gate-executor tests](../../pkg/prdevelopment/publication_gate_executor_test.go), [push-handler tests](../../pkg/prdevelopment/publication_push_handler_test.go), [publication-store tests](../../pkg/eventing/pr_development_publication_store_sqlite_test.go), and [bare-remote push acceptance](../../pkg/gitworkspace/development_line_push_bare_remote_test.go) | [gate-context store tests](../../pkg/eventing/pr_development_publication_gate_context_store_sqlite_test.go), [publication-attention store tests](../../pkg/eventing/pr_development_publication_attention_store_sqlite_test.go), and [production push tests](../../pkg/gitworkspace/development_line_push_test.go) | A later repair may supersede only a pre-effect publication. Once exact remote-tip success is proven, local workspace or configuration drift is recorded as `published` with `local_drift`; unknown or ambiguous remote effect state is never treated as executable authority. |
| Crash or restart at policy pin, subject pin, provider pin, run admission, human wait, push start, Git return, or finalization cannot duplicate a model or Git effect | `FR-EVENT-AUTOMATION-069` through `FR-EVENT-AUTOMATION-078`; [Workflow requirement 046](workflows.md); [Security-isolation requirement 043](security-isolation.md) through [Security-isolation requirement 052](security-isolation.md) | Segmented fault injection: [gate-processor tests](../../pkg/prdevelopment/publication_gate_processor_test.go) cover policy/provider pin replay; [gate-executor tests](../../pkg/prdevelopment/publication_gate_executor_test.go) cover subject pin and run admission convergence; [gate-handler tests](../../pkg/prdevelopment/publication_gate_handler_test.go) cover durable human wait; [push-handler tests](../../pkg/prdevelopment/publication_push_handler_test.go) cover push-start and finalize replay without a second Git call | [publication-store tests](../../pkg/eventing/pr_development_publication_store_sqlite_test.go), [gate-context store tests](../../pkg/eventing/pr_development_publication_gate_context_store_sqlite_test.go), [workflow gate tests](../../pkg/workflows/gates_test.go), and [publication runtime tests](../../pkg/gateway/pr_development_publication_runtime_test.go) | These are deterministic seam-level restart simulations, not one operating-system process-kill fixture. The nominal boundary path does not claim process restart; durable pins either resume the same effect, fail closed, or move an unprovable post-start outcome to reconciliation. |
| An ambiguous post-push result survives store reopen and a new worker, then reconciles read-only from exact remote-head proof without another push | `FR-EVENT-AUTOMATION-077`; [Security-isolation requirement 051](security-isolation.md) | Segmented: [unknown-outcome store tests](../../pkg/eventing/pr_development_publication_unknown_outcome_store_sqlite_test.go) prove reopen persistence; [outcome reconciler SQLite tests](../../pkg/prdevelopment/publication_outcome_reconciliation_worker_sqlite_test.go) prove the durable lifecycle; [outcome reconciler tests](../../pkg/prdevelopment/publication_outcome_reconciliation_worker_test.go) prove new-worker retry and no Git capability | [bare-remote push acceptance](../../pkg/gitworkspace/development_line_push_bare_remote_test.go) proves exact remote refs, while [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) proves only the nominal push path | No one fixture deliberately makes a real Git push ambiguous, reopens SQLite, and then observes that remote. Reconciliation is read-only, never retries Git, and accepts only exact desired-tip proof. |
| No workflow, gate, repair, review, publication, or reconciliation result is interpreted as provider acknowledgement, thread resolution, or merge approval | `FR-EVENT-AUTOMATION-043`, `FR-EVENT-AUTOMATION-070`, `FR-EVENT-AUTOMATION-076` through `FR-EVENT-AUTOMATION-078` | Boundary-path negative evidence for own-PR development: [PR-development acceptance](../../pkg/prdevelopment/pr_development_acceptance_test.go) proves capture and repair use only provider reads, publication performs exactly one source-branch Git receive, the base ref stays unchanged, and the retained candidate is not merged; reviewing another user's PR remains segmented through [review-worker SQLite tests](../../pkg/reviews/worker_sqlite_test.go) and the explicit [submitter tests](../../pkg/reviews/submitter_test.go) | [bare-remote push acceptance](../../pkg/gitworkspace/development_line_push_bare_remote_test.go), [push-handler tests](../../pkg/prdevelopment/publication_push_handler_test.go), and [publication runtime tests](../../pkg/gateway/pr_development_publication_runtime_test.go) | The fixture's provider implementations expose no write, acknowledgement, resolution, approval, or merge capability. External review submission exists only behind its separate human-confirmed outbox path; publication-status UI, review acknowledgement, thread/comment resolution, and pull-request merge are not implemented or inferred. |
...
| `FR-EVENT-AUTOMATION-065` | [pkg/attention/policy_test.go](../../pkg/attention/policy_test.go), [pkg/attention/private_run_test.go](../../pkg/attention/private_run_test.go), [pkg/eventing/pr_development_attention_store_sqlite_test.go](../../pkg/eventing/pr_development_attention_store_sqlite_test.go), [pkg/prdevelopment/attention_test.go](../../pkg/prdevelopment/attention_test.go), [pkg/gateway/pr_development_attention_composition_test.go](../../pkg/gateway/pr_development_attention_composition_test.go) |
| `FR-EVENT-AUTOMATION-066` | [pkg/eventing/pr_development_attention_trigger_store_sqlite_test.go](../../pkg/eventing/pr_development_attention_trigger_store_sqlite_test.go), [pkg/eventing/pr_development_store_sqlite_test.go](../../pkg/eventing/pr_development_store_sqlite_test.go), [pkg/attention/private_run_test.go](../../pkg/attention/private_run_test.go), [pkg/attention/conversation_test.go](../../pkg/attention/conversation_test.go), [pkg/prdevelopment/attention_test.go](../../pkg/prdevelopment/attention_test.go), [pkg/prdevelopment/attention_trigger_worker_test.go](../../pkg/prdevelopment/attention_trigger_worker_test.go), [pkg/prdevelopment/attention_bridge_test.go](../../pkg/prdevelopment/attention_bridge_test.go), [pkg/prdevelopment/service_handler_test.go](../../pkg/prdevelopment/service_handler_test.go), [pkg/gateway/pr_development_attention_composition_test.go](../../pkg/gateway/pr_development_attention_composition_test.go), [web/backend/api/pr_development_test.go](../../web/backend/api/pr_development_test.go), [web/frontend/src/api/pr-development.test.ts](../../web/frontend/src/api/pr-development.test.ts), [web/frontend/src/api/pr-development-attention.test.ts](../../web/frontend/src/api/pr-development-attention.test.ts), [web/frontend/src/components/reviews/attention-conversation.test.tsx](../../web/frontend/src/components/reviews/attention-conversation.test.tsx), [web/frontend/src/components/reviews/pr-development-page.test.tsx](../../web/frontend/src/components/reviews/pr-development-page.test.tsx), [web/frontend/src/routes/-reviews-route.test.tsx](../../web/frontend/src/routes/-reviews-route.test.tsx), [web/frontend/src/routes/-reviews.test.ts](../../web/frontend/src/routes/-reviews.test.ts), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts) |
| `FR-EVENT-AUTOMATION-067` | [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_repair_store_sqlite.go](../../pkg/eventing/pr_development_repair_store_sqlite.go), [pkg/eventing/pr_development_ledger_store_sqlite.go](../../pkg/eventing/pr_development_ledger_store_sqlite.go), [pkg/eventing/pr_development_orchestration_store_sqlite.go](../../pkg/eventing/pr_development_orchestration_store_sqlite.go), [pkg/eventing/pr_development_review_store_sqlite_test.go](../../pkg/eventing/pr_development_review_store_sqlite_test.go), [pkg/prdevelopment/service.go](../../pkg/prdevelopment/service.go), [pkg/prdevelopment/service_handler_test.go](../../pkg/prdevelopment/service_handler_test.go), [web/frontend/src/api/pr-development.ts](../../web/frontend/src/api/pr-development.ts), [web/frontend/src/api/pr-development.test.ts](../../web/frontend/src/api/pr-development.test.ts), [web/frontend/src/components/reviews/pr-development-page.tsx](../../web/frontend/src/components/reviews/pr-development-page.tsx), [web/frontend/src/components/reviews/pr-development-page.test.tsx](../../web/frontend/src/components/reviews/pr-development-page.test.tsx), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts) |
| `FR-EVENT-AUTOMATION-068` | [pkg/eventing/pr_development_suspension_store_sqlite_test.go](../../pkg/eventing/pr_development_suspension_store_sqlite_test.go), [pkg/eventing/pr_development_suspended_resume_store_sqlite_test.go](../../pkg/eventing/pr_development_suspended_resume_store_sqlite_test.go), [pkg/eventing/pr_development_suspended_resume_recovery_store_sqlite_test.go](../../pkg/eventing/pr_development_suspended_resume_recovery_store_sqlite_test.go), [pkg/eventing/pr_development_recovery_queue_sqlite_test.go](../../pkg/eventing/pr_development_recovery_queue_sqlite_test.go), [pkg/gitworkspace/pinned_recovery_suspension_capacity_test.go](../../pkg/gitworkspace/pinned_recovery_suspension_capacity_test.go), [pkg/prdevelopment/controller_recovery_worker_test.go](../../pkg/prdevelopment/controller_recovery_worker_test.go), [pkg/prdevelopment/repair_controller_worker_test.go](../../pkg/prdevelopment/repair_controller_worker_test.go), [pkg/gateway/pr_development_repair_runtime_test.go](../../pkg/gateway/pr_development_repair_runtime_test.go) |
| `FR-EVENT-AUTOMATION-069` | [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_publication_schema_sqlite.go](../../pkg/eventing/pr_development_publication_schema_sqlite.go), [pkg/eventing/pr_development_publication_store_sqlite.go](../../pkg/eventing/pr_development_publication_store_sqlite.go), [pkg/eventing/pr_development_publication_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_store_sqlite_test.go), [pkg/eventing/pr_development_publication_renewal_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_renewal_store_sqlite_test.go), [pkg/eventing/pr_development_publication_admission_store_sqlite.go](../../pkg/eventing/pr_development_publication_admission_store_sqlite.go), [pkg/eventing/pr_development_publication_admission_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_admission_store_sqlite_test.go), [pkg/eventing/pr_development_ledger_store_sqlite.go](../../pkg/eventing/pr_development_ledger_store_sqlite.go), [pkg/eventing/pr_development_review_store_sqlite_test.go](../../pkg/eventing/pr_development_review_store_sqlite_test.go), [pkg/eventing/pr_development_controller_store_sqlite.go](../../pkg/eventing/pr_development_controller_store_sqlite.go), [pkg/eventing/pr_development_controller_store_sqlite_test.go](../../pkg/eventing/pr_development_controller_store_sqlite_test.go), [pkg/eventing/pr_development_orchestration_store_sqlite.go](../../pkg/eventing/pr_development_orchestration_store_sqlite.go), [pkg/eventing/store_sqlite.go](../../pkg/eventing/store_sqlite.go), [pkg/eventing/store_types.go](../../pkg/eventing/store_types.go), [pkg/eventing/store_schema_test.go](../../pkg/eventing/store_schema_test.go), [pkg/eventing/store_unsupported.go](../../pkg/eventing/store_unsupported.go), [pkg/eventing/store_unsupported_test.go](../../pkg/eventing/store_unsupported_test.go) |
| `FR-EVENT-AUTOMATION-070` | [pkg/prdevelopment/publication_provider.go](../../pkg/prdevelopment/publication_provider.go), [pkg/prdevelopment/publication_provider_test.go](../../pkg/prdevelopment/publication_provider_test.go), [pkg/prdevelopment/github.go](../../pkg/prdevelopment/github.go), [pkg/prdevelopment/github_case_test.go](../../pkg/prdevelopment/github_case_test.go) |
| `FR-EVENT-AUTOMATION-071` | [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_publication_store_sqlite.go](../../pkg/eventing/pr_development_publication_store_sqlite.go), [pkg/eventing/pr_development_publication_gate_context_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_gate_context_store_sqlite_test.go), [pkg/eventing/pr_development_publication_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_store_sqlite_test.go), [pkg/eventing/store_types.go](../../pkg/eventing/store_types.go), [pkg/eventing/store_unsupported.go](../../pkg/eventing/store_unsupported.go), [pkg/eventing/store_unsupported_test.go](../../pkg/eventing/store_unsupported_test.go) |
...
| `FR-EVENT-AUTOMATION-076` | [pkg/prdevelopment/publication_push_handler.go](../../pkg/prdevelopment/publication_push_handler.go), [pkg/prdevelopment/publication_push_handler_test.go](../../pkg/prdevelopment/publication_push_handler_test.go), [pkg/prdevelopment/publication_push_handler_sqlite_test.go](../../pkg/prdevelopment/publication_push_handler_sqlite_test.go), [pkg/prdevelopment/publication_push_heartbeat.go](../../pkg/prdevelopment/publication_push_heartbeat.go), [pkg/prdevelopment/publication_gate_handler.go](../../pkg/prdevelopment/publication_gate_handler.go), [pkg/prdevelopment/publication_gate_handler_test.go](../../pkg/prdevelopment/publication_gate_handler_test.go), [pkg/prdevelopment/publication_retry.go](../../pkg/prdevelopment/publication_retry.go), [pkg/prdevelopment/publication_retry_test.go](../../pkg/prdevelopment/publication_retry_test.go), [pkg/prdevelopment/publication_dispatcher.go](../../pkg/prdevelopment/publication_dispatcher.go), [pkg/prdevelopment/publication_dispatcher_test.go](../../pkg/prdevelopment/publication_dispatcher_test.go), [pkg/prdevelopment/publication_provider.go](../../pkg/prdevelopment/publication_provider.go), [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_publication_store_sqlite.go](../../pkg/eventing/pr_development_publication_store_sqlite.go), [pkg/eventing/pr_development_publication_push_claim_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_push_claim_store_sqlite_test.go), [pkg/eventing/pr_development_publication_requeue_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_requeue_store_sqlite_test.go), [pkg/eventing/pr_development_publication_renewal_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_renewal_store_sqlite_test.go), [pkg/eventing/pr_development_publication_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_store_sqlite_test.go), [pkg/eventing/store_unsupported.go](../../pkg/eventing/store_unsupported.go), [pkg/eventing/store_unsupported_test.go](../../pkg/eventing/store_unsupported_test.go), [pkg/gitworkspace/development_line_push.go](../../pkg/gitworkspace/development_line_push.go), [pkg/gitworkspace/development_line_push_test.go](../../pkg/gitworkspace/development_line_push_test.go) |
| `FR-EVENT-AUTOMATION-077` | [pkg/prdevelopment/publication_outcome_reconciliation_worker.go](../../pkg/prdevelopment/publication_outcome_reconciliation_worker.go), [pkg/prdevelopment/publication_outcome_reconciliation_worker_test.go](../../pkg/prdevelopment/publication_outcome_reconciliation_worker_test.go), [pkg/prdevelopment/publication_outcome_reconciliation_worker_sqlite_test.go](../../pkg/prdevelopment/publication_outcome_reconciliation_worker_sqlite_test.go), [pkg/prdevelopment/publication_provider.go](../../pkg/prdevelopment/publication_provider.go), [pkg/prdevelopment/publication_provider_test.go](../../pkg/prdevelopment/publication_provider_test.go), [pkg/prdevelopment/publication_retry.go](../../pkg/prdevelopment/publication_retry.go), [pkg/prdevelopment/publication_retry_test.go](../../pkg/prdevelopment/publication_retry_test.go), [pkg/eventing/pr_development_types.go](../../pkg/eventing/pr_development_types.go), [pkg/eventing/pr_development_publication_store_sqlite.go](../../pkg/eventing/pr_development_publication_store_sqlite.go), [pkg/eventing/pr_development_publication_unknown_outcome_store_sqlite_test.go](../../pkg/eventing/pr_development_publication_unknown_outcome_store_sqlite_test.go), [pkg/eventing/store_unsupported.go](../../pkg/eventing/store_unsupported.go), [pkg/eventing/store_unsupported_test.go](../../pkg/eventing/store_unsupported_test.go) |

| `FR-EVENT-AUTOMATION-078` | [pkg/prdevelopment/publication_worker.go](../../pkg/prdevelopment/publication_worker.go), [pkg/prdevelopment/publication_worker_test.go](../../pkg/prdevelopment/publication_worker_test.go), [pkg/prdevelopment/publication_worker_sqlite_test.go](../../pkg/prdevelopment/publication_worker_sqlite_test.go), [pkg/prdevelopment/publication_dispatcher.go](../../pkg/prdevelopment/publication_dispatcher.go), [pkg/prdevelopment/publication_dispatcher_test.go](../../pkg/prdevelopment/publication_dispatcher_test.go), [pkg/prdevelopment/publication_gate_handler.go](../../pkg/prdevelopment/publication_gate_handler.go), [pkg/prdevelopment/publication_gate_handler_test.go](../../pkg/prdevelopment/publication_gate_handler_test.go), [pkg/prdevelopment/publication_push_handler.go](../../pkg/prdevelopment/publication_push_handler.go), [pkg/prdevelopment/publication_push_handler_test.go](../../pkg/prdevelopment/publication_push_handler_test.go), [pkg/prdevelopment/publication_outcome_reconciliation_worker.go](../../pkg/prdevelopment/publication_outcome_reconciliation_worker.go), [pkg/prdevelopment/publication_outcome_reconciliation_worker_test.go](../../pkg/prdevelopment/publication_outcome_reconciliation_worker_test.go), [pkg/prdevelopment/publication_provider.go](../../pkg/prdevelopment/publication_provider.go), [pkg/gateway/pr_development_publication_runtime.go](../../pkg/gateway/pr_development_publication_runtime.go), [pkg/gateway/pr_development_publication_runtime_test.go](../../pkg/gateway/pr_development_publication_runtime_test.go), [pkg/gateway/event_automation.go](../../pkg/gateway/event_automation.go), [pkg/gateway/event_automation_test.go](../../pkg/gateway/event_automation_test.go), [pkg/agent/runtime_gate.go](../../pkg/agent/runtime_gate.go), [pkg/agent/runtime_gate_test.go](../../pkg/agent/runtime_gate_test.go) |
| `FR-EVENT-AUTOMATION-079` | [pkg/reviews/provider.go](../../pkg/reviews/provider.go), [pkg/reviews/provider_test.go](../../pkg/reviews/provider_test.go), [pkg/reviews/provider_artifact_other_test.go](../../pkg/reviews/provider_artifact_other_test.go), [pkg/reviews/provider_handler_test.go](../../pkg/reviews/provider_handler_test.go), [pkg/gateway/event_review_readiness_test.go](../../pkg/gateway/event_review_readiness_test.go), [web/backend/api/reviews_test.go](../../web/backend/api/reviews_test.go), [web/frontend/src/api/review-provider.test.ts](../../web/frontend/src/api/review-provider.test.ts) |
| `FR-EVENT-AUTOMATION-080` | [web/frontend/src/components/reviews/review-portfolio-model.test.ts](../../web/frontend/src/components/reviews/review-portfolio-model.test.ts), [web/frontend/src/components/reviews/review-query-model.test.ts](../../web/frontend/src/components/reviews/review-query-model.test.ts), [web/frontend/src/components/reviews/review-portfolio-page.test.tsx](../../web/frontend/src/components/reviews/review-portfolio-page.test.tsx), [web/frontend/src/components/reviews/reviews-page.test.tsx](../../web/frontend/src/components/reviews/reviews-page.test.tsx), [web/frontend/src/routes/-reviews-route.test.tsx](../../web/frontend/src/routes/-reviews-route.test.tsx), [web/frontend/src/routes/-reviews.test.ts](../../web/frontend/src/routes/-reviews.test.ts), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts) |
...
[pkg/agent/local_repair_prompt_test.go](../../pkg/agent/local_repair_prompt_test.go),
[pkg/agent/git_workspace.go](../../pkg/agent/git_workspace.go),
[pkg/agent/git_workspace_controller_test.go](../../pkg/agent/git_workspace_controller_test.go),
[pkg/gitworkspace/development_line.go](../../pkg/gitworkspace/development_line.go), and
[pkg/gitworkspace/development_line_review_test.go](../../pkg/gitworkspace/development_line_review_test.go).
...
`FR-EVENT-AUTOMATION-059` additionally relies on the schema-v10 controller and
fence aggregate validation in
[pkg/eventing/pr_development_controller_store_sqlite.go](../../pkg/eventing/pr_development_controller_store_sqlite.go).
...
- [pkg/gateway/pr_development_local_ci.go](../../pkg/gateway/pr_development_local_ci.go)
