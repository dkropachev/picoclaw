...
| Generation-owned publication claiming/renewal, publisher cleanup, full provider-observer worker wiring, and unknown-outcome reconciliation scheduling | Planned `#137` and `#138` / later requirements | Not implemented. Production composition must consume every dispatcher phase, journal, observer, requeue, and Git seam through distinct least-authority interfaces, and receives no authority merely from a schema-v15/16 decision, private workflow run, completed attempt, ledger row, gate answer, review outcome, primitive push result, provider observation, processor result, retry-delay value, or browser-visible `local_ready`. |
