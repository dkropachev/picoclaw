...
| Automatic attention triggering, PR-chat steering/UI, provider refresh/write, push/publication, acknowledgement, and merge | `#123+` | Receives no authority from the schema-v15 decision link, private workflow run, completed attempt, ledger row, or review outcome without its own later response and provider fences. |
