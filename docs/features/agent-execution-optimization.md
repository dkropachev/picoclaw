...
| `FR-AGENT-EXECUTION-OPTIMIZATION-019` | MUST | A managed structured request used for repository evaluation supplies identical frozen scope chunks and two to eight reviewer aliases. | Child planning takes the Cartesian product of scope chunks and aliases with model/adaptive optimization disabled and bounded parallelism; every child records requested alias, actual concrete-model metadata, safe scope references, structured result, usage, and failure. | Only normal managed diagnostics are persisted in the workflow step; the evaluation controller separately checkpoints bounded judgments and rotates reviewer order between batches. | Unavailable aliases fail before child execution, partial failures remain explicit, and alias/concrete identity is removed before the judge receives candidate outputs. | Comparing models requires the same evidence and contract per alias without optimization silently changing the candidate or order bias being mistaken for quality. |
...
continue_on_child_error      emit a partial aggregate when at least one child succeeds
...
| `FR-AGENT-EXECUTION-OPTIMIZATION-016` | [pkg/agent/workflow_runtime.go](../../pkg/agent/workflow_runtime.go), [pkg/agent/workflow_runtime_test.go](../../pkg/agent/workflow_runtime_test.go), [pkg/workflows/private_session_test.go](../../pkg/workflows/private_session_test.go), [pkg/session/session_store.go](../../pkg/session/session_store.go), [pkg/session/frozen_media_test.go](../../pkg/session/frozen_media_test.go), [pkg/media/frozen_test.go](../../pkg/media/frozen_test.go), [pkg/agent/turn_coord.go](../../pkg/agent/turn_coord.go) |
