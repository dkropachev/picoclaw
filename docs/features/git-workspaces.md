...
launcher API, and frontend routes. The browser separates the standard
List/Table/Grid inventory, direct item maintenance, operational List/Table
history, and global limits at `/agent/git-workspaces`, `/:id`, `/history`, and
`/settings`. Inventory and history use server-authoritative query and opaque
cursor paging; direct item routes never depend on a previously loaded list.
Trusted controllers can instead request a
...
| `FR-GITWS-010` | MUST | An authenticated operator opens `/agent/git-workspaces`, `/:id`, `/history`, or `/settings`, refreshes, or confirms Maintain, Clean ignored, or Drop. | The inventory uses the standard List/Table/Grid collection with compact List default; direct detail shows one generic workspace and its confirmed maintenance actions; history uses shared List/Table operational primitives; settings shows configured and effective generic size/cleanup/drop limits and delays without embedding inventory or history. Safe normalized SSH remotes and compact checkout paths retain explicit copy actions. | Reads and navigation mutate nothing. Reconcile, cleanup, and drop call only their existing API authorities after explicit action or confirmation. | Collection loading, query, cursor, direct not-found, config, and manager failures use bounded operable states. Clean and Drop are disabled for locked workspaces, and every controller-private workspace, repository, history record, path, lock, count, and byte remains structurally absent. | Users need stable inventory, maintenance, audit, and policy routes without exposing private controller state or combining unrelated collections and settings. |
...
| `FR-GITWS-019` | MUST | A generic acquire supplies `fresh: true` with one repository, optional validated ref, and session key. | The manager validates ref syntax independently of ambient checkout metadata, then clones a new checkout or reuses only an unlocked clean fresh snapshot after fetching/pruning the exact origin and resolving the currently available requested ref. A local source repository may project its single safe network origin as `upstream_url` and a fixed `picoclaw-upstream` remote for later immutable reads. | Inventory records the fresh-snapshot marker and safe upstream identity. Clean release keeps the snapshot reusable; changed release preserves work and removes it from the fresh pool. | A dirty cached snapshot is preserved and skipped. A missing/deleted ref, option-like, checkout-history shorthand such as `@{-1}`, or changed same-session ref, credential-bearing repository URL, changed origin, unsafe control state, or refresh failure fails without silently using stale cached `HEAD`. | Repository review needs a current immutable source snapshot without converting the generic agent tool into pinned-controller authority. |
...
Owns: HTTP PUT /api/git-workspaces/settings
...
Owns: TEST web/frontend/src/routes/agent/-git-workspaces-route.test.tsx
...
| HTTP | `GET /api/git-workspaces`, `GET /api/git-workspaces/{id}`, `GET /api/git-workspaces/history`, `GET/PUT /api/git-workspaces/settings` | Shared query/cursor inventory and history envelopes, direct generic detail, and configured/effective global limits with strict same-origin revision-fenced update; controller-private state is structurally absent. | `FR-GITWS-005`, `FR-GITWS-010`, `FR-GITWS-021` |
| HTTP | `POST /api/git-workspaces/reconcile`, `POST /api/git-workspaces/cleanup`, `DELETE /api/git-workspaces/{id}` | Existing generic maintenance authorities for explicit reconcile, clean-ignored, and confirmed drop actions. | `FR-GITWS-006` through `FR-GITWS-010` |
| Frontend | `/agent/git-workspaces`, `/:id`, `/history`, `/settings` | Standard inventory, direct maintenance detail, operational history, and global generic limits on dedicated routed pages. | `FR-GITWS-001`, `FR-GITWS-010`, `FR-GITWS-021` |
...
workspace fields and routed browser behavior. Trusted development controllers may
...
| `FR-GITWS-010`, `FR-GITWS-021` | [web/backend/api/git_workspaces.go](../../web/backend/api/git_workspaces.go), [web/backend/api/git_workspaces_test.go](../../web/backend/api/git_workspaces_test.go), [web/frontend/src/api/git-workspaces.ts](../../web/frontend/src/api/git-workspaces.ts), [web/frontend/src/api/git-workspaces.test.ts](../../web/frontend/src/api/git-workspaces.test.ts), [web/frontend/src/components/agent/git-workspaces](../../web/frontend/src/components/agent/git-workspaces), [web/frontend/src/routes/agent](../../web/frontend/src/routes/agent), [web/frontend/src/routes/agent/-git-workspaces-route.test.tsx](../../web/frontend/src/routes/agent/-git-workspaces-route.test.tsx), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts), [web/frontend/tests/collection-visual.spec.ts](../../web/frontend/tests/collection-visual.spec.ts) |
...
- [web/frontend/src/api/git-workspaces.ts](../../web/frontend/src/api/git-workspaces.ts)
- [web/frontend/src/components/agent/git-workspaces/git-workspace-route-state.ts](../../web/frontend/src/components/agent/git-workspaces/git-workspace-route-state.ts)
- [web/frontend/src/components/agent/git-workspaces/git-workspace-collections.tsx](../../web/frontend/src/components/agent/git-workspaces/git-workspace-collections.tsx)
- [web/frontend/src/components/agent/git-workspaces/git-workspace-detail-page.tsx](../../web/frontend/src/components/agent/git-workspaces/git-workspace-detail-page.tsx)
- [web/frontend/src/components/agent/git-workspaces/git-workspace-settings-page.tsx](../../web/frontend/src/components/agent/git-workspaces/git-workspace-settings-page.tsx)
- [web/frontend/src/routes/agent](../../web/frontend/src/routes/agent)
