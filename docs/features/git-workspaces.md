...
- Core types/functions: `gitworkspace.Manager`, `Options`, acquire/release/stat
  ...
  `Manager.SnapshotPinnedLineReview`, `NewGitWorkspaceTool`, API routes under
...
- Runtime ordering: load config, construct the manager, acquire and lock before
  ...
  only from its complete version/epoch/tip/tree fence; then reconcile generic
...
Inventory version 2 uses a canonical string-valued version discriminator and
additionally stores a private development-line map and one
...
pending-park write-ahead tuple, and complete last-park replay evidence. The
reservation bearer remains only in the ordinary live workspace lock. Private
...
neutral. The string discriminator makes a version-1 decoder fail before it can
rewrite version-2 state. A newer version, orphaned/multiply owned workspace, inconsistent
...
| Tool | `git_workspace` | Agent-callable generic acquire/list/status/release/clean/drop/reconcile operations with JSON results. It has no pinned or development-line actions, cannot supply pinned or line identity fields, generic release skips ordinary pinned reservations, every pinned workspace/repository/history/ID/path/lock/count/byte projection is absent from acquisition onward, and maintenance treats guessed pinned IDs as missing. | `FR-GITWS-002` through `FR-GITWS-009`, `FR-GITWS-011`, `FR-GITWS-013` |
...
13. To resume a parked line, take the fresh reservation operation lock and
    ...
    or fence conflicts.
14. For stats, skip every controller-pinned workspace before inspecting its path or
...
15. Reconcile skips locked and controller-pinned workspaces, cleans old generic
...
call them, persist attempt-ledger state, run CI, interpret a review, or publish a
  commit. A model, workflow, generic tool, launcher route, and browser cannot
  discover a line identity/ref/checkout or invoke it. The security contract
  additionally constrains the bounded review projection and makes repository
  paths and lifecycle authority unrepresentable outside the trusted controller.
...
- The persistent `inventory.lock` file fences directory-lock binaries, while
  the string-valued version-2 discriminator makes the immediately preceding
  numeric-version decoder fail before mutation. Upgrades must still be
