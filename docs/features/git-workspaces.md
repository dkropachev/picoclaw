...
`FR-GITWS-018` is invoked only by the unified PR-workspace branch
publisher after implementation validation and its publication gate. The PR
...
An ambiguous transport outcome remains a PR-workspace `unknown` publication
...
mark the local line as durably published. The unified PR-workspace publication
...
one-ref remote compare-and-swap primitive. The unified PR workspace owns
