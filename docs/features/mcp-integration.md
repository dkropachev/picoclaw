...
| `FR-MCP-014` | MUST   | Successful runtime initialization stages one deterministic detached catalog containing strict MCP factories plus configured destination-bound Regex/BM25 factories and commits all admitted agent registries through one all-or-none transaction. Invalid discovery configuration fails before network access. Partial server connection exposes only connected configured servers; identical exact declarations deduplicate and conflicting/canonical identities fail. Per-agent MCP-server allowlists select staged remote capabilities, tool allowlists decide transaction admissions, and exact same-identity refresh uses occupant CAS without overwriting another tool. Candidate manager ownership transfers irreversibly at registry commit, so every precommit error/panic/cancellation closes it and every postcommit prompt/projection fault retains it beneath published wrappers. MCP and discovery prompt text is absent before success and is generated only from exact admissions, with collision-resistant exact-server source IDs and exact allowed-tool intersection. Factory-backed owner selections receive distinct wrapper/media/discovery state while borrowing the live generation manager and event bus. Lazy initialization and every factory-backed selection stay beneath the exact live `(config, registry, execution policy, diagnostic policy)` generation lease; catalogs or managers cannot transfer authority across reload, while detached, startup, paused, or missing-origin entry is diagnostic-safe-only. First-party roots install zero diagnostic policy, so this propagation is dormant. | Lazy multi-agent registration must not leave a partial tool surface, dangling prompt, stale SDK schema, closed borrowed manager, or lossy prompt authorization after a late collision or reload. |
| `FR-MCP-015` | MUST | Each MCP manager retains one exact immutable subprocess policy for stdio initial connection, duplicate replacement, and reconnect. Stdio environment starts with no ambient inheritance, merges bounded validated env-file entries before sorted config `env`, preserves explicit blanks, folds Windows names deterministically, and lets the policy add captured/reserved values. `~/` command expansion uses only frozen policy home values. A successfully connected stdio process is manager/session-owned rather than connect-call-context-owned; cancellation before publication closes it, manager close always performs bounded wait/terminate/kill even when stdin close fails, and replacement/reconnect closes the retired session. Remote HTTP/SSE transport behavior is unchanged. | A reconnect or management probe must not switch host environment/executable generation, leak a retired child, or inherit credentials absent from explicit server policy. |
...
| Integration | Host-supervised MCP streamable suite          | Real server protocol compatibility over an ephemeral loopback endpoint; signal tests publish child readiness only after handlers are installed and wait for parent supervision capture before cancellation. | `FR-MCP-001`, `FR-MCP-004`                             |
...
- Host-supervised integration cancellation reaps the test process group and
  fixture server before returning the signal exit status. Test readiness is
  published only after child signal handlers are installed, and cancellation
  waits for the supervising parent to capture its child PID.
...
| `FR-MCP-015`                                           | [pkg/mcp/manager.go](../../pkg/mcp/manager.go), [pkg/mcp/isolated_command_transport.go](../../pkg/mcp/isolated_command_transport.go), [pkg/mcp/manager_test.go](../../pkg/mcp/manager_test.go), [cmd/picoclaw/internal/mcp/command_test.go](../../cmd/picoclaw/internal/mcp/command_test.go), [web/backend/api/mcp_test.go](../../web/backend/api/mcp_test.go) |
