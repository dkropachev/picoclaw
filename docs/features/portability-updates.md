...
6. Coverage-delta creates separate temporary runtime homes for the base and head
   ...
   evolution draft timeout in its exact coverage sandbox, or the repository-review
   auto-continuation completion timeout for one canonical generated identity—rerun
   that complete base command once. Do not retry head, an unrelated or additional
   assertion, a near-miss timeout, panic, build failure, unrelated test or package,
   or any repeated failure.
...
- Coverage comparison retries an exact recognized baseline flake at most once:
  ...
  draft timeout inside the base coverage sandbox, or the exact repository-review
  auto-continuation completion timeout for a generated `rra_` identity. The
  classifier rejects any extra test diagnostic, failure marker, failed package,
  malformed dynamic identity, or near-miss path and preserves the second attempt's
  result; all head, unrecognized, or repeated failures remain visible.
