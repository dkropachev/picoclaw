...
| `FR-PORT-004` | SHOULD | Docker and release workflows keep dependency setup explicit for Go, Node, pnpm, QEMU, and GoReleaser. Repository-wide pull-request tests partition the complete tagged Go package list exactly once between balanced `slow`, `workspace`, and `remaining` jobs, keep every package intact, execute every shard through the hermetic runner, and bound each job's ordinary `go test` package parallelism to four. Frontend unit tests use four bounded worker threads in CI while retaining the local fork pool. Frontend browser validation partitions each desktop smoke, mobile smoke, and cross-viewport visual suite into two complete Playwright shards while retaining two workers per runner; every shard remains part of the merge-gating aggregate and uses a collision-free failure artifact identity. Successful first attempts avoid trace recording overhead; the first retry records diagnostics for a failed initial attempt and captures any terminal retry failure. Local failure tracing remains enabled. Serialized trusted default-branch core and launcher build jobs own daily immutable cross-platform and host Go cache generations respectively; pull-request jobs exact-match the current UTC generation when available, fall back only to other trusted v3 generations, and never publish partial role caches. Self-contained integration suites run directly with the exact host Go toolchain, reusable caches, uncached test results, and test-owned fixtures; suites requiring isolated service topology retain Docker mode. The MCP fixture binds a kernel-assigned IPv4 loopback port, publishes readiness atomically, and is reaped on every suite exit. Docker integration runners pin the repository's exact Go toolchain, bind repository-local or explicitly authorized Go caches at host-identical absolute paths across suite projects, and choose a rootful/rootless-compatible numeric identity; suite teardown cannot delete the bind-mounted caches, ambient global caches are not mounted implicitly, and pull requests remain restore-only. Protected default-branch pushes run only those cache producers; manual dispatch retains integration and frontend-unit fallback validation. Scheduled rolling-release rehearsal runs weekly and remains manually dispatchable. Coverage-delta disables cached test results with `-count=1`, retains per-ref instrumented package serialization at `-p 1`, serially prepares detached base and head worktrees, collects both isolated refs concurrently with two Go processors each, fully joins them, and then removes their worktrees serially. Integration coverage assigns Docker refs distinct Compose project namespaces and translates host-suite coverage outputs to their physical worktree paths. Coverage execution uses a short physical temporary root and distinct sibling `TMPDIR`, `TEMP`, and `TMP` roots while isolating the compared refs' product, credential, home, Git, and user-bus state; derives integration and package scope from the head; and applies statement-weighted thresholds with exact integer ratios. Distinct head coverage blocks intersecting changed production Go lines must reach 90%. Scoped-global and impacted-feature percentages and uncovered-statement debt are reported as informational and do not decide the gate; no separate new-feature minimum applies. Coverage commands, package enumeration, ref resolution, and coverage-profile/Git-diff parsing errors continue to fail closed. A complete, byte- and mode-identical `R100` relocation of one module package from `pkg/` to its matching `internal/` path does not fan import-only consumer changes out to those consumers' feature owners, while the moved package owner remains impacted and every changed consumer package remains in coverage and test scope. This classification fails closed for partial or inexact moves, copies, module-file changes, extra source or asset changes, and any consumer edit beyond a token-scoped module import replacement plus its canonical `gofmt` ordering. Rare cryptographic, cleanup, and uncertain-failure branches compiled for the active target remain in the denominator, with no source exclusions or waivers; target-specific builds remain mandatory for build-constrained code. A failing immutable-base command is rerun once, while the second result and every head failure are final. | CI and release validation must be repeatable, isolated between refs, exact at threshold boundaries, and strict about changed-code coverage without forcing test-only production seams. |
...
Owns: CODE scripts/ci-changed-paths.sh
Owns: CODE scripts/run-integration-tests.sh
Owns: CODE scripts/run-go-test-shard.sh
Owns: TEST pkg/updater/*
Owns: TEST cmd/membench/*
Owns: TEST integration/*
Owns: TEST scripts/coverage_delta_test.go *
Owns: TEST scripts/ci_changed_paths_test.go *
Owns: TEST scripts/go_test_shard_test.go *
...
| CI | GitHub Actions PR, release, nightly, build, Docker, and DMG workflows | Scoped validation with one stable required result plus repeatable packaging and release automation. | `FR-PORT-004`, `FR-PORT-006` |
...
6. Coverage-delta creates a short physical temporary root, serially prepares
   detached base and head worktrees, gives each ref separate runtime homes and
   sibling temporary directories, clears inherited `PICOCLAW_HOME`, and replaces
   inherited `HOME`, `TMPDIR`, `TEMP`, and `TMP`. It then collects both refs
   concurrently with two Go processors each, waits for both outcomes, and
   removes their worktrees serially. Each Go coverage command requests uncached
   results with `-count=1` and retains `-p 1` package serialization; concurrent
   integration runs use distinct Compose project namespaces. Comparison
   intersects added or modified production Go lines with the head coverage
   profile, deduplicates every
   matching file/range block, and uses statement weights with exact integer
   comparison to require at least 90% changed-code coverage. Scoped-global and
   impacted-feature base/head percentages and uncovered-statement debt remain
   visible as informational output, but do not decide the gate; no separate
   new-feature minimum applies. Overlapping ownership does not double-count a
   changed block. Coverage commands, package enumeration, ref resolution, and
   coverage-profile/Git-diff parsing errors fail closed. Rare
   cryptographic, cleanup, and uncertain-failure branches compiled for the
   active target remain in the denominator without source-level exclusions or
   waivers; target-specific builds validate code excluded by active build
   constraints. If the immutable-base coverage command fails, rerun that
   complete command once and preserve the second result; do not retry head or
   any repeated base failure.
...
- Coverage comparison fails when changed production Go code is below 90%.
  Exact thresholds pass; rounded output does not decide the gate. Scoped-global
  and impacted-feature coverage and debt are informational. Coverage-command,
  package-enumeration, profile-parsing, and Git-diff parsing errors still fail
  closed. Rare cryptographic, cleanup, and uncertain-failure branches compiled
  for the active target remain in the denominator without source-level
  exclusions or waivers; build-constrained code remains subject to
...
| `FR-PORT-006` | [scripts/ci_changed_paths_test.go](../../scripts/ci_changed_paths_test.go), [scripts/portability_requirements_test.go](../../scripts/portability_requirements_test.go), [.github/workflows/pr.yml](../../.github/workflows/pr.yml) |
...
- [scripts/ci-changed-paths.sh](../../scripts/ci-changed-paths.sh)
