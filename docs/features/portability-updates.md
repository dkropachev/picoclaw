...
6. Coverage-delta creates separate temporary runtime homes for the base and head
   ...
   the repository-review assignment late-controller disappearance of either
   `repository-reviews.db-wal` or `repository-reviews.db-shm` with its exact
   diagnostic, adjacent `lstat` ENOENT continuation, and base-sandbox path—rerun
   that complete base command once. Do not retry head, an unrelated or
   additional assertion, a detached or near-miss continuation, panic, build failure,
   unrelated test or package, or any repeated failure.
...
- Coverage comparison retries an exact recognized baseline flake at most once:
  ...
  auto-continuation completion timeout for a generated `rra_` identity, or the exact
  repository-review assignment late-controller private-file-change diagnostic and
  adjacent `lstat` ENOENT for either `repository-reviews.db-wal` or
  `repository-reviews.db-shm` under the base coverage sandbox.
  The classifier rejects any extra test diagnostic, failure marker, failed package,
  malformed dynamic identity, detached continuation, or near-miss path and preserves
  the second attempt's result; all head, unrecognized, or repeated failures remain
  visible.
