...
| `FR-CHANNEL-009` | SHOULD | Browser chat UI preserves readable message layout, accessible composer labeling, responsive controls, and non-overlapping code/message surfaces in light and dark themes. The gateway-status projection carries model-setup readiness separately from process-start readiness, so a healthy limited-mode gateway does not hide the actionable model-configuration notice while the predefined `chat` alias is unmapped. | Chat delivery remains user-facing even when the gateway is stopped, unavailable, or running before model setup is complete. |
