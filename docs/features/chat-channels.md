...
| `FR-CHANNEL-016` | MUST | Each enabled event-automation service generation composes PR-development execution only when its verifier, repair and review runtime factories, repair-and-review readiness for the same concrete agent, workspace manager, context agent, concrete agent identity, local-CI sandbox runner, and durable evidence store are present. Readiness is conjunctive: public repair availability and new repair admission require both the edit runtime and isolated local-review runtime for that exact agent. The gateway constructs the reservation-free review worker from the event store, generation-owned concrete Git manager, local-CI evidence, ordered-context agent/compactor, and isolated review runtime; this composition is independent of GitHub MCP/provider-object readiness so an already parked review can continue during provider outages. Independently, every enabled generation starts the controller worker against durable storage even when repair or review execution composition is incomplete, and retains the legacy worker for its disjoint isolated-legacy and pre-v14 compatibility claims. A review worker starts only when its local dependencies and evidence store are complete. Every controller or review iteration holds the exact agent-runtime generation lease around the complete claim/use/terminalization call. Setup failure closes any opened local-CI evidence and event store. Reload or shutdown first cancels and joins the legacy, controller, and any review worker, then closes local-CI evidence and the event store. | No repair may be admitted unless its later local review can use the same immutable agent generation, a provider-object outage must not strand already parked local work, incomplete review composition must not stop controller reconciliation, legacy records must remain serviceable without competing claims, and reload or shutdown must not close providers, Git managers, evidence, or storage beneath an active iteration. |
| `FR-CHANNEL-017` | MUST | Each enabled event-automation service generation may claim or reconcile PR-development publications only when the workflow executor and exact run store, resolved global/repository policies, persisted local-CI evidence, parked-line reader, owner-agent runtime acquisition, distinct full and head-only provider observers, narrow parked-line pusher, durable publication store, and outer runtime-generation acquisition are all present. After acquiring that generation and before any durable publication operation, one initializer resolves and caches the generation-owned Git projections, constructs every pending/waiting/push handler plus the claim and reconciliation workers together, and starts separate publication and reconciliation loops. Reload or shutdown cancels and joins both loops before retiring the event store, CI evidence, provider, agent, or Git generation; the replacement service resolves its own adapters while immutable publication pins remain authoritative. Disabled, incomplete, generation-mismatched, or dynamically Git-unready composition stays inert and leaves durable work for a later complete generation. | Gateway reload must not let partial publication wiring steal work, let a cached provider/Git adapter cross generations, or close a dependency beneath an active gate, push, or outcome reconciliation. |
...
11. When composing an enabled event-automation generation, resolve every
    PR-development execution dependency before creating its gateway-owned
    local-CI sandbox and evidence store beside the effective event database.
    Require the same exact agent's repair and review readiness before exposing
    new repair admission. Start the store-backed controller reconciliation
    worker even when execution composition is incomplete; when review-local
    dependencies are complete, also start the reservation-free reviewer without
    requiring GitHub MCP/provider-object readiness. Run them beside the
    claim-partitioned legacy worker and retain the exact agent-runtime generation
    across every complete controller/reviewer iteration. On failed setup,
    reload, or shutdown, cancel and join all started workers before closing the
    local-CI evidence handle and event store.
12. Compose publication claiming and outcome reconciliation only when their
    complete shared policy, workflow, CI, provider, parked-Git, store, and
    generation dependency set exists. Resolve generation-owned Git adapters
    before either loop touches durable work, run the loops separately, and join
    both before a reload or shutdown can retire that generation's dependencies.
...
| `FR-CHANNEL-016`                                                                         | [pkg/gateway/pr_development_local_ci_test.go](../../pkg/gateway/pr_development_local_ci_test.go), [pkg/gateway/pr_development_repair_runtime_test.go](../../pkg/gateway/pr_development_repair_runtime_test.go), [pkg/gateway/event_automation_test.go](../../pkg/gateway/event_automation_test.go) |
| `FR-CHANNEL-017`                                                                         | [pkg/gateway/pr_development_publication_runtime_test.go](../../pkg/gateway/pr_development_publication_runtime_test.go), [pkg/prdevelopment/publication_worker_sqlite_test.go](../../pkg/prdevelopment/publication_worker_sqlite_test.go) |
...
- [pkg/gateway/pr_development_publication_runtime.go](../../pkg/gateway/pr_development_publication_runtime.go)
