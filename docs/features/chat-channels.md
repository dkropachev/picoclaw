...
| `FR-CHANNEL-016` | MUST | Each enabled event-automation service generation composes one unified PR-workspace service and protected HTTP handler from its exact v19 aggregate store, provider resolver/evidence loader, isolated AI, gate evaluator, scope/nudge policies, and optional implementation/publication capabilities. Review and implementation share that service/store/generation; implementation is exposed only when the same exact agent, production Git-workspace manager, edit-only repair runner, local-CI validator, and candidate finalizer are ready. Review, branch, and issue publication capabilities are detected independently and a missing effect capability disables only that effect. | Reload must not split one pull request across generations or let partial composition weaken a gate, evidence fence, implementation prerequisite, or provider action. |
| `FR-CHANNEL-017` | MUST | Gateway startup registers only the canonical `/runtime/eventing/pr-workspaces` subtree and starts the unified durable operation/publication workers against the generation-owned v19 store. Every iteration holds the exact runtime generation while claiming, executing, and terminalizing work. Reload or shutdown first stops admission, cancels and joins those workers, and only then retires the event store, workflow executor, agent, provider, Git manager, and CI evidence. No legacy review/development worker or route is composed. | A replacement generation must resume durable unified work without competing legacy claims or closing a dependency beneath an active model, gate, Git, CI, or provider operation. |
...
11. Compose the unified PR workspace only after resolving its exact v19 store,
    provider/evidence, isolated-AI, gate, policy, and optional implementation
    and publication capabilities for the active generation. Share one service
    and aggregate across review and implementation; do not create a legacy
    worker, sink, route, or case store.
12. Start durable PR operation/publication workers only after complete
