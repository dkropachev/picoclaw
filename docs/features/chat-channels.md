...
| `FR-CHANNEL-016` | MUST | Each enabled event-automation service generation composes PR-development execution only when its verifier, repair runtime factory, agent-readiness check, workspace manager, context agent and concrete agent identity are all present. With those dependencies, the gateway owns one local-CI runner backed by a sandbox and durable file-evidence store under canonical, non-symlink, mode-`0700` `pr-development-local-ci/{tmp,evidence}` directories beside the effective event database. Public repair availability and new repair admission require that complete composition and a successfully constructed runner; local-CI initialization failure leaves the gateway running but repair unavailable. Independently, every enabled generation starts the controller worker against its durable store even when execution dependencies are incomplete, retains the legacy worker for its disjoint isolated-legacy and pre-v14 compatibility claims, and places each worker iteration under the exact agent-runtime generation lease when that lease seam is available. Setup failure closes any opened local-CI evidence and event store; reload or shutdown first cancels and joins both workers, then closes local-CI evidence and the event store. | No repair may be admitted without local validation capability, incomplete runtime composition must not strand controller-owned durable work, legacy records must remain serviceable without competing claims, and reload or shutdown must not close generation-owned state beneath an active repair iteration. |
...
11. When composing an enabled event-automation generation, resolve every
    ...
    Expose repair availability only after the runner exists, but start the
    store-backed controller reconciliation worker even when that composition is
    incomplete. Run it beside the claim-partitioned legacy worker and acquire
    the exact agent-runtime generation around each worker iteration. On failed
    setup, reload, or shutdown, cancel and join the workers before closing the
