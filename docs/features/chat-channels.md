...
In the WebUI deployment, the launcher hosts the Gateway runtime inside the
launcher OS process and keeps the runtime's internal HTTP listener on loopback by default;
the standalone headless Gateway is an explicit alternative deployment.
...
| `FR-CHANNEL-006` | MUST   | Gateway HTTP and websocket routes expose only configured channel, Pico, health, and explicitly registered feature behavior. Additive shared-mux registration detects collisions, returns an identity-owned release function, and never holds the route-map lock while a downstream handler runs. The health server can wrap an additive runtime handler in the same constant-time bearer check used by built-in protected endpoints, verify the configured credential without exposing it, and fail the wrapper closed when no credential exists. Read-only workflow-authoring capability and agent-activity routes use that existing protected listener, are registered and released with their exact runtime generation, and create no additional listener or authorization path. When the launcher embeds the Gateway, this shared listener remains runtime-owned inside the launcher process and loopback-bound by default; no separate Gateway process is created. A hostname or adaptive wildcard bind records a reachable numeric PID probe derived from the listener family that actually opened. The manager propagates a 30-second read timeout and a 135-second write timeout into the shared `http.Server` for both pre-opened-listener and address-based startup. The write deadline must remain greater than the protected local AI routes' 120-second application budget so a completed durable response is not discarded by the shared transport first. | Launcher proxies, native Pico clients, and opt-in Gateway features use one runtime listener without silently replacing or blocking one another, duplicating process-local authorization logic, or sending bearer authority to an occupied address in another IP family. |
...
| `FR-CHANNEL-009` | SHOULD | Browser chat UI preserves readable message layout, accessible composer labeling, responsive controls, and non-overlapping code/message surfaces in light and dark themes. The gateway-status projection carries model-setup readiness separately from runtime-start readiness, so a healthy limited-mode gateway does not hide the actionable model-configuration notice while the predefined `chat` alias is unmapped. | Chat delivery remains user-facing even when the gateway is stopped, unavailable, or running before model setup is complete. |
| `FR-CHANNEL-010` | MUST | Pico browser chat selects two independent exact references: an enabled concrete account or account router in `account_ref`, and a configured model alias or enabled model router in `model_name`. When the persisted account is valid but its persisted alias is empty, browser chat uses an explicitly configured `chat` alias as a turn-local fallback without rewriting configuration. The wire payload contains those two fields and never carries a raw upstream model ID. The channel copies both into normalized inbound metadata for that turn. | Chat-window selection must use the same explicit account-plus-alias contract as every other execution path and must not rely on hidden config rewrites or provider defaults. |
| `FR-CHANNEL-011` | MUST | Gateway cold start keeps ordinary runtime admission behind the construction barrier while synchronous event, workflow, and eager MCP readiness setup uses one exclusive, revocable owner for the exact startup generation; startup releases that owner before marking ready and opening admission, and background workers start with fresh contexts. Each startup or reload generation constructs and publishes one immutable `(config, registry, execution policy, diagnostic policy)` owner tuple before its provider, agent loop, cron service, and ASR backend; every component retains its exact tuple member through commit or rollback, and a failed candidate publishes none of it. The Gateway root installs zero diagnostic policy, so the propagated preview capability remains dormant. Reload serializes the A snapshot, B publication, service commit, and rollback beneath one transaction; its single-use retained-A capability preserves the exact config, provider, execution policy, diagnostic policy, and successor fence rather than a registry, so rollback rebuilds the registry under a fresh generation identity with the exact retained policies. Gateway config reload marks readiness false, preflights replacement storage and event runtime dependencies, pauses new agent runtime admission, drains active turns and reload-owned services, retains the prior provider/config through replacement startup, and resumes turns only after commit or successful rollback. The reload pause owns an equivalent revocable context across validation, restart, recovery, and rollback, then revokes it before resume. An inbound message holds one generation across workflow-trigger evaluation, route/session reservation, worker-queue wait, and turn completion. Replacement event/heartbeat workers and scheduled jobs are fenced to their exact config generation, the runtime-event workflow subscription is absent throughout the outer transaction, and cron starts only after other fallible replacement initialization. Terminal shutdown remembers Stop before a delayed AgentLoop start, quiesces producers, drains the permanent runtime boundary, then stops outbound channel/media dependencies, closes the Cron SQLite handle, and closes providers; a close failure joins cleanup error reporting. Cleanup, rollback, or shutdown-drain failure leaves readiness/resources fail-safe and never closes a provider or dependency still reachable by active work. | Channels remain live during reload, so readiness alone cannot prevent a turn, queued lifecycle event, background service, or subprocess from observing provisional, ambient, or closed runtime state. |
...
| `FR-CHANNEL-020` | MUST | WeCom request-to-chat routes persist as typed, expiry-indexed, versioned rows in `$PICOCLAW_HOME/channels/wecom/reqid-store.db`; Weixin account cursors and ordered user context tokens persist as typed, account-keyed, versioned rows in `$PICOCLAW_HOME/channels/weixin/state.db`. Both stores use the common hardened SQLite contract, process-local plus cross-process locking, full-range seconds/nanoseconds, bounded identities and sources, transactional replacement or expiry cleanup, exact schema validation, and no mutable JSON write or fallback. First open deterministically imports selected valid legacy WeCom `wecom/reqid-store.json` and Weixin `channels/weixin/{sync,context-tokens}/*.json` records, records payload-free skips, and archives exact sources without overwrite under the component's `legacy-json/*-v1/` tree; an invalid database or unsafe source fails channel construction/use instead of restoring JSON authority. | Route continuity and Weixin synchronization must survive restarts and concurrent processes without stale whole-file overwrites, duplicate-key ambiguity, secret-bearing diagnostics, or agent-editable runtime authority. |
...
provider binding set, not only the default provider. Rollback seeds registry
construction with those exact objects and transfers ownership only after
publication; commit closes the complete retired set after releasing transaction
...
message context, media references, and Gateway log/status runtime state.
...
| Runtime | Gateway publication and outcome-reconciliation loops | Acquire one exact runtime generation per complete iteration, resolve the all-or-none publication graph only inside that generation, and drain both loops before replacement or teardown. | `FR-CHANNEL-017` |
...
| Runtime | `Manager.SetupHTTPServer`, `Manager.SetupHTTPServerListeners`, shared `http.Server` | Propagate the fixed 30-second read and 135-second write deadlines to the one shared listener; keep the write deadline above every registered protected AI route's 120-second application budget. | `FR-CHANNEL-006` |
...
3. HTTP callback channels and opt-in gateway features register routes before
   ...
   identity-safe release closure. Configure the shared server with its
   30-second read deadline and 135-second write deadline before either
   pre-opened listeners or address-based serving begins; the write deadline
   stays beyond the protected AI routes' 120-second application budget. Socket/
   polling channels start their own workers.
...
11. Compose development workspaces only after resolving their exact v20 store,
    source/provider/evidence, isolated-AI, gate, policy, notification, and
    optional implementation/publication capabilities for the active
    generation. Keep feature implementation and PR pickup mutually exclusive
    within one service; do not create a legacy worker, sink, route, or case
    store.
12. Start durable development operation/publication workers only after complete
    generation construction. Stop admission, cancel, and join every loop before
    reload or shutdown closes the store, CI evidence, agent, provider, workflow,
    or Git dependencies. Missing review, branch, or issue write capability
    disables that effect independently.
...
- The chat account selector groups concrete Accounts and Account Routers. The
  ...
  blocked until both are selected. Missing aliases are not replaced by fetched
  IDs, raw model strings, or provider defaults.
...
- Incomplete publication composition and unavailable generation-owned Git
  resolution perform no durable claim, expiry, scan, or reconciliation. A
  reload waits for active publication work, drains the old loops, and prevents
  their cached adapters from entering the replacement generation.
...
| `FR-CHANNEL-006`                                                                         | [pkg/channels/manager.go](../../pkg/channels/manager.go), [pkg/channels/dynamic_mux_test.go](../../pkg/channels/dynamic_mux_test.go), [pkg/channels/manager_test.go](../../pkg/channels/manager_test.go), [pkg/health/server_test.go](../../pkg/health/server_test.go), [pkg/gateway/workflow_authoring_test.go](../../pkg/gateway/workflow_authoring_test.go), [pkg/gateway/agent_activity_test.go](../../pkg/gateway/agent_activity_test.go), [pkg/gateway/listen_test.go](../../pkg/gateway/listen_test.go), [web/backend/api/pico_test.go](../../web/backend/api/pico_test.go), [web/backend/api/channels_test.go](../../web/backend/api/channels_test.go) |
...
| `FR-CHANNEL-011`                                                                         | [pkg/gateway/event_startup_runtime_test.go](../../pkg/gateway/event_startup_runtime_test.go), [pkg/gateway/event_automation_test.go](../../pkg/gateway/event_automation_test.go), [pkg/gateway/gateway_test.go](../../pkg/gateway/gateway_test.go), [pkg/agent/runtime_gate_test.go](../../pkg/agent/runtime_gate_test.go), [pkg/agent/workflow_eventing_test.go](../../pkg/agent/workflow_eventing_test.go), [pkg/agent/execution_policy_generation_test.go](../../pkg/agent/execution_policy_generation_test.go), [pkg/agent/runtime_policy_constructors_test.go](../../pkg/agent/runtime_policy_constructors_test.go), [pkg/agent/runtime_policy_late_work_test.go](../../pkg/agent/runtime_policy_late_work_test.go), [pkg/agent/runtime_reload_transaction_test.go](../../pkg/agent/runtime_reload_transaction_test.go), [pkg/audio/asr/audio_model_transcriber_test.go](../../pkg/audio/asr/audio_model_transcriber_test.go), [pkg/health/server_test.go](../../pkg/health/server_test.go) |
