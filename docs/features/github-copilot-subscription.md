...
- Core types/functions: `auth.AuthCredential`, `auth.NormalizeCredentialID`,
  ...
  `GitHubCopilotProvider`, `config.ModelRouterCredentialAccountProvider`,
  `/api/models/fetch`, and the frontend `OAuthProvider` account flow.
...
8. For credential-backed model fetch, resolve the selected stored credential,
   ...
   return all non-empty model IDs. For provider metadata without a selected
   credential, return only curated common-model hints; for status, validate the
   selected stored credential for account-backed entries and keep the existing
   local TCP probe only for bridge configs.
...
| `FR-GITHUB-COPILOT-011`                                                                            | Expected implementation tests: `pkg/config/model_router_test.go`, `web/backend/api/models_test.go`, `pkg/agent/model_router_test.go`, `pkg/modelrouter/router_test.go`. Tests must cover Copilot credential refs in both account blocks and load-balance blocks for all supported strategies.                                                                                                                                          |
...
Owns: TEST pkg/agent/model_router_test.go
