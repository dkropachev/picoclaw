...
any configured decision. That existing editor presents
`pr_development.before_push` as a known decision alongside validated custom
decision points, preserves ordered mixed gate lists, and previews the effective
global-plus-repository policy before saving. A save changes only future
...
| `FR-LAUNCHER-019` | MUST   | The existing revision-fenced attention-policy editor presents `pr_development.before_push` as a known decision without restricting the existing free-form custom decision-point input. At one decision point it preserves and edits an ordered mixture of working-context AI, isolated-context AI, deterministic, and zero/no-op gates with each kind's relevant criteria, condition, and structured-question controls and explanatory text. It preserves global policies and the exact repository-local `inherit`, `overlay`, `replace`, and `disable` modes, and previews the effective global-plus-repository gate order, kinds, and resolution actions before saving. For a before-push working-context gate, it warns that the selected agent must be the same agent that owns the PR's local development and that an owner mismatch fails closed before publication. The editor reuses the existing strict `GET`/revision-fenced full-replacement `PUT /api/reviews/attention-policies` contract and canonical catalog schema with no config migration. Saving affects only future publication decisions that have not pinned a policy revision and only updates configuration: it does not run a gate or model, edit code, run CI, invoke Git or push, acknowledge a review, resolve a review thread, or merge a pull request. | Before-push policy must be understandable and previewable without creating a second policy language, invalidating custom integrations, changing an already-admitted publication, or turning configuration management into development or publication authority. |
...
the same validated catalog. Repository previews resolve the current in-memory
global and repository layers using the same ordered gate and override semantics
that the runtime consumes. Existing pinned publication decisions keep their
captured policy; neither opening the editor nor saving a later catalog revisits
them.
...
| HTTP     | Feature-owned `/api/reviews*` routes                                                                                                                                                        | Register review-case proxy operations, the case-owned attention projection/response proxy, and the launcher-local revision-fenced attention-policy subresource behind dashboard authentication and shared strict JSON/config-mutation boundaries. The existing policy schema accepts the known before-push decision and custom points without migration. | `FR-LAUNCHER-013`, `FR-LAUNCHER-014`, `FR-LAUNCHER-019` |
...
| Frontend | `/reviews`, `/reviews?view=policies`, `/reviews?case={case}&focus=chat`                                                                                                                       | Expose the default case inbox, one canonical policy-editor view, and the selected case's existing focused conversation card in the authenticated shared shell; retain policy and response drafts only in memory, protect dirty policy navigation, surface authoritative conflict/restart/attention state, and never place policy or response authority in the route. The editor offers known before-push and custom points, ordered mixed gates, exact repository overrides, owner-agent guidance, and an effective-policy preview without execution authority. | `FR-LAUNCHER-013`, `FR-LAUNCHER-014`, `FR-LAUNCHER-019` |
...
8. Register the review route and sidebar destination behind dashboard
   ...
   effect without invoking any review decision or runtime capability. Offer
   `pr_development.before_push` as a known value while preserving free-form
   custom decision points. Edit ordered mixed working-context AI,
   isolated-context AI, deterministic, and zero gates; preserve global policies
   plus exact inherit/overlay/replace/disable repository overrides; and resolve
   their effective order, kinds, and actions in memory before save. Warn that a
   before-push working-context gate must select the local-development owner
   agent. Submit through the unchanged policy API and schema without migration;
   the replacement applies only to future unpinned decisions and performs no
