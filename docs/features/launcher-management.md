...
`attention_required` boolean supports five-second in-app polling, an accessible
marker, and canonical chat focus; it is not an out-of-band notification and is
absent from case detail. Repository and pull-number filtering, cursor pagination,
...
`attention_required` boolean while detail continues to reject it. While the
development view is open, the UI polls the list every five seconds, presents an
accessible `AI attention` marker, and selecting a marked case replaces route
...
does not claim email, provider, or channel notification.
