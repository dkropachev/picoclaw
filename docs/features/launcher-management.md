...
3. Serialize full-config writes and workflow-specific config-coupled
   ...
   transaction. For workflow Run and Retry, acquire the workflow mutation lock
   first, repeat the complete readiness fence, then acquire the config-path
   advisory lock and reject any generation change before holding both locks
   through exact compatibility validation and durable root-run creation.
