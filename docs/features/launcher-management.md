...
external PR/review links remain inert. One exact same-origin chat route adds a
...
| Frontend | `/reviews?view=development`                                                                                                                                                                  | Expose the own-PR captured-feedback list/detail and explicit advisory conversation, with only optional repository, optional positive pull number, and public case selection in canonical navigation. Render feedback/messages as plain text; keep cursors, DTOs, versions, transcripts, drafts, mutation, and ambiguity state out of persistence; adopt detail monotonically with strict case/version/message binding; distinguish committed-human and completed-turn reload recovery; retain detail/draft through announced refetch failure; and preserve mobile focus order. | `FR-LAUNCHER-015`, `FR-LAUNCHER-016` |
...
12. Return JSON for success and error paths with status codes that match
