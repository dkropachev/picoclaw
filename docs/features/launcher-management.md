...
| --- | --- | --- | --- |
| Storage | `<PICOCLAW_HOME>/launcher-auth.db`; `legacy-json/launcher-auth-v1/launcher-config.json` | Private versioned bcrypt authority plus one digest-verified, no-overwrite legacy snapshot; active launcher settings contain no auth hash or token. | `FR-LAUNCHER-001` |
