...
models, OAuth credentials, tools, skills, MCP servers, sessions, embedded
Gateway runtime lifecycle, startup behavior, updates, and runtime metadata. Scoped
...
proxies `/api/development-workspaces` to the launcher-hosted Gateway, replaces browser
...
WebUI, launcher API, Gateway runtime, and local SQLite state share one OS process
and PicoClaw home. The runtime's internal listener stays on loopback by default.
Only launcher port `18800` is mapped by default; direct Gateway port `18790`
requires an explicit Compose override. Host port `18800` binds only to loopback
unless the operator explicitly selects another host address after completing
password setup. A standalone headless Gateway remains a separate, mutually
exclusive opt-in deployment.
...
- Core types/functions: API handler/router, dashboard auth middleware, config
  mutation coordinator, embedded Gateway runtime host, development-workspace and
...
- Non-obvious constraints: launcher reads never spawn, attach to, or repair an
  external Gateway process; browser credentials are never forwarded; scoped
  config writes use the public-plus-security revision; workspace mutations use workspace version,
...
| --- | --- | --- | --- |
| `FR-LAUNCHER-001` | MUST   | Dashboard access requires password setup/login and an HttpOnly session cookie; local bootstrap auto-login is loopback-only. Before dashboard authentication, the launcher ensures the canonical-home database supervisor and requires the opaque `launcher-auth` store to be ready. The bcrypt hash is stored only through that typed broker client; an existing current-schema row remains authoritative, and the provider admits only the exact credential, import-ledger, horizon, and required-index schema-object set. An unversioned/outdated store, unrelated table/view/index/trigger, or legacy `dashboard_password_hash`/`launcher_token` returns a maintenance response without authenticating or changing configuration. Under the exclusive migration fence, `picoclaw database migrate` prefers an existing unversioned password row, then a valid removed `dashboard_password_hash`, then a valid removed `launcher_token`; it serializes snapshot, archive, cleanup, and ledger completion, archives the exact source before stripping both auth fields from the retained settings-only `launcher-config.json`, and safely resumes an interrupted pending phase. No platform or error path writes a JSON password fallback. Every agent runtime obtains the launcher-auth generation, complete active `legacy-json` namespace, and exact retained credential archive from the broker catalog as protected mutation roots for `write_file`, `edit_file`, `append_file`, `apply_patch`, their owner factories, reload generations, and controller local repair; it does not reconstruct database or sidecar filenames. Active `config.json`, settings-only `launcher-config.json`, and ordinary source files remain editable. | Browser management must be gated, credentials must not remain mixed with human-authored launcher settings, and unsupported, unavailable, or migration-required storage and model-authored file mutation must fail closed rather than weaken authentication. |
...
| `FR-LAUNCHER-005` | MUST   | Gateway lifecycle endpoints report status/logs and start, stop, or restart one Gateway runtime hosted inside the launcher process; they never spawn or attach to a separate Gateway process. Stop and restart cancel and join the exact runtime generation before replacement, and a failed or timed-out retirement blocks another generation until the launcher restarts. A missing global model alias does not block runtime startup: the embedded runtime uses allow-empty semantics, remains available in limited mode, and rejects model-dependent execution locally until an execution context supplies a configured alias. Start readiness constructs its probe provider with the exact immutable execution policy derived from the inspected config. Gateway status separately reports whether model setup is required, and the dashboard displays that notice independently of runtime state until the predefined `chat` alias has an explicit mapping. | Runtime availability and model selection are separate concerns; desktop users still need lifecycle control and a visible configuration path before a model is selected without introducing a second OS process. |
...
| `FR-LAUNCHER-011` | MUST   | Full-config PUT/PATCH/reset, generic tool-state writes, agent policy mutations, workflow-specific settings, template-install, publish, and workflow Run/Retry admission are serialized by one handler mutation boundary. Every cooperating `SaveConfig` call also holds a config-path advisory process/file lock, with the opaque generation covering both public JSON and the security sidecar. Full-config PUT/PATCH, generic tool-state, agent policy, and workflow settings mutations load an update-safe snapshot and perform final compare-and-swap saves against that exact generation; reset holds the lock across backup, secret preservation, and replacement. Runtime and update-safe snapshot loaders retain the shared lock while deriving the public-plus-security revision before a parse attempt and deriving it again afterward, including after a failed parse. They return a config and the initial revision only when parsing succeeds and both revisions match. A final revision-read error or changed revision takes precedence over a parse error; detected public or security-sidecar drift returns a nil config, empty revision, and `ErrConfigRevisionMismatch`. An expected-revision read compares before parsing. Cooperating writers cannot enter this interval. The current-schema-only loaders still perform no migration, backup, or save; migration-capable snapshot loaders apply the same pre/post fence to their final current-schema attempt without changing their existing legacy-migration behavior. This is drift detection rather than a hostile-filesystem snapshot: uncooperating sequential file replacement, an A-to-B-to-A change, or a change only to environment, externally referenced credential bytes, or other loader input may remain undetected, and process-global loader effects already performed are not rolled back after a mismatch. Agent responses derive restart effects from that captured config and a read-only in-memory gateway snapshot without discovering processes, attaching to them, or sanitizing PID metadata. Workflow Run and Retry reacquire that same advisory lock after their final readiness fence, compare the current public-plus-security generation with the admitted generation, and retain the lock through exact compatibility checking and durable root-run creation. The authenticated launcher registers agent management routes and navigation, and the gateway restart signature includes the complete ordered agent policy plus a canonical effective isolation-policy digest: omitted or nil defaults equal explicit defaults, explicit empty remains distinct, and expose paths are normalized. | Scoped or merge-patch management must not return values or effects from a detectably changed config generation with its prior revision, lose a cooperating concurrent secret-only update, overwrite a mutation from another launcher or gateway process, hide an unapplied agent or subprocess policy change, mutate gateway process metadata during an agent read, or admit execution from one generation while another process publishes a replacement before the run exists. |
...
| `FR-LAUNCHER-031` | MUST | The default Compose topology uses the stable `picoclaw` project identity and defines and starts exactly one launcher container without selecting a profile; optional one-shot agent and standalone headless Gateway services live in a separate Compose file and cannot be co-started by enabling a profile on the default file. Documented launcher/headless transitions remove alternate orphan services before startup so a launcher-hosted runtime and standalone Gateway never share one PID/SQLite home, while the fixed project identity prevents orphan cleanup from selecting an unrelated directory-named project. The launcher container embeds the WebUI, API, and Gateway runtime in one OS process, persists the complete PicoClaw home, maps launcher port `18800` only to host loopback by default, keeps the runtime-owned Gateway listener on container loopback unless explicit operator configuration widens it, and receives a 120-second Compose stop grace period for joined runtime and SQLite cleanup. Explicit operator input may widen the host-side launcher bind after password setup; a separate Compose override may select a wildcard Gateway bind and publish `18790` from that same process for HTTP callbacks. Public `GET`/`HEAD /health` and `/ready` report launcher availability without depending on Gateway runtime or model state; unsupported methods remain unavailable, all management routes retain dashboard authentication, a loopback health command remains available under restrictive CIDR policy, and both source-built and release image healthchecks probe launcher liveness. | Single-node installation should work by default without exposing setup or internal runtime control and without a second Gateway process against the same SQLite home, while operators retain explicit remote-access and opt-in headless modes and can observe or recover the launcher when its embedded runtime is stopped. |
...
a shared config mutation lock, public-plus-security config revisions, its
Gateway runtime generation and same-process PID metadata, PWA registration, and
browser push subscription UX. It
does not own durable development aggregates, notification rows, private push
keys, or delivery ledgers.

Dashboard password state is a singleton typed record in the opaque
`launcher-auth` broker store. The SQLite provider owns its physical generation
and schema fence. Its offline launcher-config import ledger retains only a
relative source identity, digest, size/mode bounds, source classification, safe
issue code, counts, timestamps, and archive status. `launcher-config.json`
retains network and startup settings only after migration; its archived
pre-cleanup bytes are never a live write target.

The model-facing mutation boundary resolves relative paths according to the
actual host or workspace backend, consumes the database provider's protected
artifact catalog, rejects lexical, resolved, platform-alias, and exact-file
hardlink aliases of home storage authorities and their retained credential,
catalog, and adaptation sources, and gives protected roots precedence over
outside-workspace allow paths. Writes pin the resolved
...
while normal provider creation or replacement of a generation artifact does not stale
...
Owns: TEST pkg/config/mutation_revision_drift_unix_test.go *
...
| --- | --- | --- | --- |
| Storage | Logical store `launcher-auth`; `legacy-json/launcher-auth-v1/launcher-config.json` | Typed broker-owned bcrypt authority plus one digest-verified, no-overwrite offline-migration snapshot; provider artifacts and the archive namespace are immutable to agent file tools, while active launcher settings contain no auth hash or token and remain editable. | `FR-LAUNCHER-001` |
| Storage | Logical store `model-catalogs`; `legacy-json/model-catalogs-v1/model_catalogs.json` | Typed broker-owned catalog/model authority with bounded canonical metadata BLOBs plus one digest-verified, no-overwrite offline-migration snapshot; provider artifacts and exact archive aliases are immutable to agent file tools. | `FR-LAUNCHER-003` |
...
| HTTP/UI | `/api/model-aliases*`; `/models/aliases*` | Name-addressed typed query/list/item CRUD, revision-fenced explicit-name bulk delete, catalog templates on creation only, and standard collection/new/edit-first item routes. | `FR-LAUNCHER-025` |
...
Launcher startup loads bounded settings, canonicalizes `PICOCLAW_HOME`, ensures
the database supervisor, and requires the `launcher-auth` store to be ready
before opening dashboard authentication. A missing empty store may be
initialized by the broker, but startup never upgrades a schema, imports a
legacy credential, archives a source, or strips auth fields. Existing outdated,
unversioned, or legacy state returns a maintenance response directing the
operator to `picoclaw database migrate`; integrity or availability failure also
aborts authentication. Password setup begins only after typed broker readiness.
...
provenance. It then peeks—but never attaches to—the launcher-hosted runtime record,
...
For a config snapshot attempt, the loader acquires the shared process/advisory
mutation lock, derives the exact public-plus-security revision, and compares any
caller-supplied expected revision before parsing. It then attempts to parse and
validate the requested runtime or update-safe view and derives the revision
again regardless of that parse outcome. A failed final revision read returns
that read error, and a changed final revision returns
`ErrConfigRevisionMismatch`; either takes precedence over a simultaneous parse
error. Only a stable revision permits the parse error or successful config plus
initial revision to return. Every failure returns no config and no revision.
Migration-capable loaders repeat their existing legacy-migration flow and apply
this ordering to the final current-schema attempt, while current-schema-only
loaders continue to reject legacy input without migration, backup, or save.
...
shutdown cancels that controller before dependent runtime state disappears.
...
The config mutation lock serializes only cooperating readers and writers.
Revision derivation reads the public file and security sidecar sequentially, so
an uncooperating writer can still present a torn pair, replace a path outside the
advisory lock, or perform A-to-B-to-A changes that restore the original digest
before the final check. The revision covers exact public/security bytes and
missing-file markers; it does not bind environment-derived values, external
`file://` credential contents, passphrase state, ambient defaults, or other
inputs consulted while parsing. Post-parse mismatch suppresses the returned
config/revision but does not undo process-global loader state already changed,
including credential-resolver selection. These limits make the check a
best-effort detector for noncooperating drift, not authentication, ABA fencing,
or a no-follow filesystem security boundary.
