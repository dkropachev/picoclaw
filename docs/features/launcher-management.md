...
owned by event automation and workflows. Launcher management owns the exact
authenticated policy subresource, its config-revision fence, its isolation
from generic config writes, and the gateway-effect projection returned by that
subresource to authenticated management clients.
