...
| `FR-LAUNCHER-023` | MUST | The authenticated shared shell exposes a separate `/model-evaluations` Model review probes destination and `/api/model-evaluations*` control surface. The responsive page creates and version-fences evaluation configuration, runs asynchronous preflight and evaluation actions, locks controls during mutations, polls only active durable state, lets the operator set 1–20 files per detected language, pages safe corpus references, shows bounded run history, and labels completed comparison scores AI judged with honest partial/failure and unknown-cost states. Basic repository/model choices remain visible while ref, corpus, selector, and judge controls are hidden in a closed Advanced section until requested. The page explicitly states that probes do not start repository reviews or write finding ledgers. | Model comparison must be a discoverable, restart-safe workload rather than a hidden repository-review mode or browser-owned timer. |
