...
| `FR-LAUNCHER-009` | SHOULD | Shared launcher layout, theme, and primitive controls remain responsive, token-driven, keyboard-accessible, and free of clipped controls across desktop and narrow mobile widths. The Services group includes the feature-owned `/repository-reviews` destination and automatically reveals it when active without changing the route's behavior. Destructive controls use paired background/foreground theme tokens with sufficient contrast in light and dark modes instead of translucent destructive text treatments that fail automated accessibility checks.                                                                                                                                                                                                                                                                                                                                                                                | Dashboard navigation and process controls must stay usable while visual styling evolves.                                                                       |
...
| `FR-LAUNCHER-022` | MUST | The launcher renders the unified PR workspace and Gate V3 configuration surfaces. | The workspace renders generic Gate forms and submits only `field-values`; the workflow configuration list/editor shows the Review and Implementation flow graph and a compact gate-action modal. Its selector names the workflow default, its answer preview retains human-facing prompt, labels, requirements, choices, and select cardinality, and it omits internal workflow/gate/revision identity plus redundant default/override/effective summaries. A deterministic override alone exposes an on-demand reference for the field and option IDs required by its JSON values. AI overrides expose ephemeral, private-snapshot, and originating-snapshot profiles; enforced history, cache, agent, and tool properties appear in one concise read-only summary, only properties editable for the selected profile render as controls, and an inherited AI default remains visible read-only. Source mode is offered only for a catalog Gate capable of supplying one source-bearing finding. Repository assignment is a separate page backed by a safe name/deferred-policy summary that exposes no bindings, actions, prompts, or Gate catalog. Global Nudging and Scope grades remain separate settings tabs. Both scoped saves use the same full-config revision fence, preserve the other surface's fields, and never execute a gate. Gateway and deferred-policy restart effects are distinct, so unrelated pending lifecycle edits do not disable issue controls. The shared draft survives configuration-route changes and explicit discard restores the server snapshot. | Malformed static forms, invalid kebab IDs or refs, partial action overrides, unknown agents, unavailable or ambiguous originating provenance, stale revisions, assigned-configuration deletion, retired Gate V2 configuration and result fields, unsafe URLs, and unknown provider outcomes fail closed. | The UI must expose one generic gate contract without leaking private workflow state or giving configuration reads execution authority. |

| `FR-LAUNCHER-023` | MUST | The authenticated shared shell exposes a separate `/model-evaluations` destination and `/api/model-evaluations*` control surface. The responsive page creates and version-fences evaluation configuration, runs asynchronous preflight and evaluation actions, locks controls during mutations, polls only active durable state, lets the operator set 1–20 files per detected language, pages safe corpus references, shows bounded run history, and labels completed comparison scores AI judged with honest partial/failure and unknown-cost states. | Model comparison must be a discoverable, restart-safe workload rather than a hidden repository-review mode or browser-owned timer. |
...
| `FR-LAUNCHER-004`                    | [web/backend/api/oauth_test.go](../../web/backend/api/oauth_test.go), [web/backend/api/codex_account_limits_test.go](../../web/backend/api/codex_account_limits_test.go), [pkg/auth/store_test.go](../../pkg/auth/store_test.go), [pkg/accountrouter/router_test.go](../../pkg/accountrouter/router_test.go), [pkg/providers/factory_provider_test.go](../../pkg/providers/factory_provider_test.go), [web/frontend/src/components/credentials/account-onboarding-sheet.test.tsx](../../web/frontend/src/components/credentials/account-onboarding-sheet.test.tsx), [web/frontend/tests/ui-smoke.spec.ts](../../web/frontend/tests/ui-smoke.spec.ts), [cmd/picoclaw/internal/auth](../../cmd/picoclaw/internal/auth) |
