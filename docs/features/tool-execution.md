...
owned subset without invoking or publishing excluded entries. The first
runtime-dormant production catalog classifies base filesystem, web, Git,
hardware, delivery, image, plan, and skill-search wrappers for later child
construction while current root execution and shallow SubTurn cloning remain
unchanged.
