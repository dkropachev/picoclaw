...
construct fresh selected turn owners.
...
| `FR-TOOL-011` | MUST | Before `spawn` launches background subturn work, an AgentLoop-backed spawner synchronously retains the caller's runtime generation and a short parent tool-source construction lease, then transfers both through detached snapshots of the exact spawner, model/fallback, token, and temperature inputs. At most the configured child-concurrency count of these queued source admissions may coexist per parent; an excess request fails synchronously without launching work. Within that bound, asynchronous acknowledgement remains immediate even when child execution is saturated. The background goroutine never rereads the owner-local wrapper; it waits only to the configured concurrency timeout or retained-context cancellation for one execution slot before consuming the source lease or calling a factory. The child consumes that lease only through strict registry construction and exact attachment, then releases it; the execution slot is released after `runTurn` and before callback delivery, while runtime-generation ownership remains through callback completion. Failure to retain the launch boundaries returns a tool error without launching work; later slot/construction failure is delivered through the existing asynchronous result path. | A parent turn may finish immediately after spawn acknowledgement; reload must still drain the admitted child, parent cleanup must wait for scheduled construction, and queued requests must not construct unbounded owner-local resources outside the concurrency limit. |
...
| Tool | `spawn` with optional asynchronous context preparation | Retain AgentLoop runtime ownership and the short parent construction source while snapshotting wrapper inputs before background launch; acquire the child execution slot in the background before construction and release each boundary at its owning phase. | `FR-TOOL-011` |
...
12. For background spawn, ask an AgentLoop-backed spawner to retain runtime
    ...
    wrapper inputs before starting the goroutine. The goroutine waits boundedly
    for one child-concurrency slot before consuming the source or constructing
    tools. Release the source after exact child construction/attachment, the
    slot after `runTurn`, and runtime ownership after callback completion; every
    failure path releases each acquired boundary exactly once.
...
| `FR-TOOL-011` | [pkg/agent/runtime_gate_test.go](../../pkg/agent/runtime_gate_test.go), [pkg/agent/subturn_effective_tools_test.go](../../pkg/agent/subturn_effective_tools_test.go), [pkg/agent/turn_state.go](../../pkg/agent/turn_state.go), [pkg/tools/spawn.go](../../pkg/tools/spawn.go) |
