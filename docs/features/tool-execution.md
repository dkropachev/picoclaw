...
| `FR-TOOL-008` | MUST | Chat model selection excludes internal virtual model entries but keeps virtual account-router entries and virtual credential-account entries selectable as default chat targets; the selector groups choices into Accounts and Account Routers only. | Account routers are materialized from `account_routers[]`, and stored credentials are exposed as generated account choices; both must remain usable from Chat without exposing unrelated generated rows or mixing routers into API-key account choices. |
