...
| Tool | `apply_patch` result | Preserve the authored-order `ForLLM` summary and return one complete, bounded, path-safe aggregate candidate diff in `ForUser` only after successful sequential commit; admission failure returns no diff and does not imply rollback. | `FR-TOOL-027`, `FR-TOOL-028` |
...
20. For `apply_patch`, parse exact text, acquire the canonical-workspace gate,
    ...
    recheck the whole plan and cancellation. Cross the mutation boundary only
    after that succeeds; the existing commit consumes the immutable plan
    sequentially and does not reinterpret patch or filesystem semantics. Return
    the retained candidate result only after every commit operation succeeds;
    discard it on every earlier or commit-time error. P011b, not candidate-result
    construction, replaces this effect loop with staging and rollback.
...
- Candidate output uses authorized authored labels rather than resolved paths,
  ...
  content.
- Planning, candidate generation, cancellation, final revalidation, and commit
  errors return no `ForUser` diff. This result rule does not repair the existing
  post-point-of-no-return caveat: a runtime failure in the legacy sequential
  commit may leave an earlier operation visible. P011b owns transaction,
