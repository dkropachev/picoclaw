...
capabilities, but no worker calls them. The later `#126b2` gate/push runtime
must consume the journal, observations, and Git primitive through separate
capabilities; neither a journal row, observation, successful primitive call,
nor `local_ready` supplies effect authority. Acknowledgement and merge stay
...
The provider-observation boundary is implemented by `FR-SEC-044`;
