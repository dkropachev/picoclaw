...
review occurrence; case detail remains unchanged. The open workbench polls the
list every five seconds and uses that marker only for in-app discovery and
canonical chat focus, never as an out-of-band notification or action
capability. A separate exact chat POST may append a bounded local transcript
