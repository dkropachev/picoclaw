...
capabilities, but no worker calls them. `FR-SEC-045` now supplies the atomic
claimed-from-pending local gate-context read and conversation-fenced first
subject pin without a gate or model invocation. The later `#126b2` gate/push
runtime must consume the journal, context, observations, and Git primitive
through separate capabilities; neither a journal row, context snapshot,
observation, successful primitive call, nor `local_ready` supplies effect
authority. Acknowledgement and merge stay separate undefined features pending
explicit user-facing policy.
...
implemented by `FR-SEC-045`; provider/gate/push runtime composition remains
`#126b2`. Acknowledgement and merge remain separately unimplemented and
