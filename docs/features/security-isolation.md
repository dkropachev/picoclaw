...
| `FR-SEC-021` | MUST | A whole-config read-modify-write operation captures the parsed update-safe configuration and one opaque revision of the exact public JSON plus security sidecar under the shared process/advisory lock, validates a complete candidate, and saves only when that revision is still current. The revision hashes security bytes without returning them. A stale save returns `config revision mismatch`, writes neither file, and preserves the winning writer's aliases, credentials, and unrelated configuration; callers surface a reload/retry conflict instead of blindly retrying an operation whose intent may no longer be valid. Current-schema read-only snapshots never migrate or save, while update snapshots may complete the explicit legacy migration lifecycle before returning a coherent current generation. A scoped review-attention replacement uses the same combined revision but raw-patches only `reviews.attention` in the public JSON: it never serializes environment/default-derived state, rewrites the security sidecar, or migrates a legacy read. The authenticated review-attention UI holds one memory-only lossless draft from one captured revision, rejects malformed or numerically lossy projections before hydration, sends at most one full replacement for an explicit save, and never automatically retries, rebases, persists the draft in the browser, or turns validation/preview into execution authority. Its known decision-point choices include `pr_development.before_push` without excluding syntactically valid custom decision points. For every decision point it preserves multiple ordered global gates and exact repository-local `inherit`, `disable`, `overlay`, or `replace` policy, exposes working-context AI, isolated-context AI, deterministic, and zero gates in any valid ordered mixture, and previews the effective global-plus-repository resolution without launching it. Saving can affect only a future publication decision whose policy is still unpinned; an existing canonical policy pin remains immutable across configuration and runtime-generation replacement under `FR-SEC-046`, `FR-SEC-049`, and `FR-SEC-052`. A configured working-context agent is not publication authority: active execution independently requires the immutable repair owner, and an owner-agent mismatch fails closed under `FR-SEC-049`. Editing, preview, reload, discard, and save have no authority to invoke a model, edit code, run CI, invoke Git or push, acknowledge a review, resolve a review thread, or merge a pull request. This extension reuses the existing schema and revision-fenced policy API and adds no migration, route, DTO, or security identifier. | CLI, launcher, runtime, and browser config writers must not lose concurrent public or secret state, persist ambient runtime state through an unrelated scoped save, corrupt arbitrary JSON numbers, retain trusted policy in a less protected store, retarget an already-pinned publication decision, or turn a read/edit path into implicit model, repository, provider, or publication authority. |
...
The `pr_development.before_push` choice is only an editor preset over that
existing catalog and transport; custom decision-point identities remain
available under the same validation. Policy inspection or replacement never
enumerates, claims, pins, or rewrites publication work. `FR-SEC-046` alone
captures the then-current policy for an authenticated unpinned publication,
`FR-SEC-049` rejects a foreign working-context owner before active execution,
and `FR-SEC-052` preserves an already-pinned policy across runtime-generation
replacement. Thus a save may influence future unpinned selection but cannot
retarget a durable pin or acquire model, code, CI, Git, push, review
acknowledgement, review-thread-resolution, or merge authority.
...
| Config / HTTP / UI | `reviews.attention`, `GET` and `PUT /api/reviews/attention-policies`, `/reviews?view=policies` | Keep gate authority in operator-owned configuration outside reviewed checkouts; expose only bounded non-secret policy plus opaque revisions/effect status, parse and serialize arbitrary question JSON losslessly, and retain the editable projection only in memory. The editor offers `pr_development.before_push` beside existing presets while retaining validated custom decision points, multiple ordered working-context AI, isolated-context AI, deterministic, and zero gates, exact global plus repository `inherit`/`disable`/`overlay`/`replace` policy, and a side-effect-free effective-resolution preview. Replace the complete catalog only through one explicit strict same-origin public-plus-security compare-and-swap that raw-patches only persisted `reviews.attention`, preserves unrelated persisted values and numeric tokens, and leaves security state byte-identical; a conflict retains the local draft for explicit reload/discard and never retries or rebases automatically. A save can affect only future unpinned publication selection; `FR-SEC-046`, `FR-SEC-049`, and `FR-SEC-052` keep existing pins immutable and reject a foreign working-context owner. Route state contains only the fixed policy-view selector, and editing, validation, preview, reload, discard, or save grants no model, code, CI, Git, push, review-acknowledgement, thread-resolution, merge, workflow, or provider authority. Broad config GET omits this subresource; broad PUT accepts only an empty compatibility placeholder, broad PATCH rejects the field, and both preserve its exact value during unrelated updates. The preset reuses this schema and API without migration, route, DTO, or new security requirement. | `FR-SEC-021`, `FR-SEC-046`, `FR-SEC-049`, `FR-SEC-052` |
