...
Resolver-dependent configuration operations use one process-local scope so two
concurrent config directories cannot cross-resolve their secure-string file
references during those complete operations.
...
- Core types/functions: secure config and credential stores, dashboard auth,
  URL/network guards, secure-string resolver-operation scope, immutable
  subprocess `ExecutionPolicy`, isolation runner,
...
- Non-obvious constraints: signed provider payloads remain untrusted content;
  ...
  read-only code display must remain bound to exact parked Git objects; and the
  resolver scope is process-local, non-reentrant, and intentionally narrower
  than filesystem or revision integrity.
...
| `FR-SEC-060` | MUST | The runtime-policy structural gate fixes the exact constructor graph that carries a loop-frozen model-file mutation protected-root slice from `AgentLoop` through initial and reloaded registries into every `AgentInstance`. Public compatibility constructors must derive an explicit environment/default snapshot, while production initial, provider-generation reload, and ordinary reload paths pass detached copies of the original loop snapshot; owner factories detach again. Each instance adds its resolved workspace-owned runtime and account-router roots. Its write, edit, append, and apply-patch tools protect database, WAL/SHM, lock, active legacy-source/sidecar, and retained archive paths for launcher authentication, channel-local state, workspace runtime state, account routing, the active configured Git-workspace `inventory.json` and `inventory.db`, and the complete checkpoint-local namespace containing selected legacy JSON, `checkpoints.db`, and retained archives, even when a custom state root falls inside the model workspace or an outside-write allowlist. Selected active checkpoint sources and every retained archive file are safely enumerated at construction so existing exact-file hardlink aliases outside their lexical namespaces are protected; an unsafe root, source, archive entry, or enumeration failure aborts construction. Controller local repair retains the loop-frozen home roots, adds the selected agent workspace roots, and refuses an overlapping checkout. Synthetic replacement with a broader compatibility constructor, argument omission, or constructor-count drift fails the gate. Legacy injectable registry-factory fields remain package-private test seams and are not production construction paths. | A reload, owner product, custom Git-state location, path alias, or constructor refactor must not silently recapture changed configuration or turn live SQLite authority and retained migration archives into model-writable files. |
| `FR-SEC-061` | MUST | Every complete `LoadConfig`/`LoadConfigForUpdate` operation, runtime or update-safe snapshot parse attempt, and `SecurityCopyFrom`/`SecurityCopyFromForUpdate` overlay enters one process-wide non-reentrant resolver scope before resolver-dependent JSON/YAML decode, security overlay, environment secure-string decode, channel decode, webhook-secret resolution, validation, or legacy-migration parsing. Saving similarly enters the exact-path resolver scope through the already mutation-locked save primitive, so operations holding both locks always acquire the config mutation lock before the resolver scope. A scope serializes these trusted operations across all config paths, installs a resolver bound to the explicit config file's directory, and releases serialization on every return or during panic unwinding without containing the panic. It deliberately does not restore the prior resolver: success, failure, and panic leave the last begun scoped path installed for legacy global behavior. A migration-capable load releases its parse scope before calling the public save path, which reacquires mutation then resolver locks; internal helpers never recursively acquire the scope. The scope is process-local and has no context, timeout, fairness, or reentrancy support, so a slow or FIFO-backed config, security sidecar, or referenced credential can head-of-line block every scoped config operation and, when a mutation lock is already held, same-path cooperating writers. Calling another scoped config operation while already scoped is unsupported. Standalone `SecureString` construction/unmarshal and later status re-resolution do not take this scope and may observe its transient or last-installed global resolver. This change does not harden path spelling, symlink/reparse/special-file handling, permissions, file-size bounds, cross-process access, sequential public/security revision reads, ABA, or revision coverage of environment, passphrase, or referenced credential bytes. | Concurrent trusted config loads must not silently resolve one config's `file://` secret relative to another config directory, while a narrow serialization fix must not overstate filesystem or snapshot authority. |

For `FR-SEC-060`, each current and retained workspace also contributes one
validated absolute-parent sibling prefix for account-router authentication
invalidation sidecars. The prefix predicate is part of the immutable prepared
mutation policy shared by ordinary file tools, apply-patch, owner products,
and local repair; it closes the post-catalog, pre-router-open reload window
without protecting unrelated workspace files.

Under `FR-SEC-002`, `auth.db` storage MUST durably close its shared legacy
import horizon after the first complete enumeration, including an empty one.
Any later `auth.json` is safely audited and retained below
`legacy-json/auth-v1/`, but MUST NOT be domain-imported or finalized; SQLite
...
Secure-string resolution retains one legacy process-global resolver pointer.
`FR-SEC-061` adds a separate process-local operation mutex. A scoped operation
installs a resolver for its explicit config directory while holding that mutex,
then releases serialization without restoring the pointer; no credential bytes
or durable state are retained by the scope itself. The pointer therefore names
the last begun scoped path after the operation, while an unscoped secure-string
API can still observe it during or after that scope and receives no isolation
guarantee.
...
| Config resolver | Config loads/snapshots, saves, and `SecurityCopyFrom*` | Serialize each complete trusted resolver-dependent operation under the resolver for its explicit config directory while preserving legacy last-begun global resolver behavior. | `FR-SEC-061` |
...
For a resolver-dependent config operation, acquire any required per-path config
mutation lock first, then acquire the process resolver scope, install a resolver
for the explicit config file's directory, and retain the scope across public
decode, security overlay, environment/channel decoding, referenced-secret
resolution, validation, and migration parsing. Already-scoped internal helpers
do not reacquire it. Release the scope on every exit while leaving its resolver
installed. Before a legacy migration calls the public save path, release the
parse scope once; the save then acquires mutation and resolver locks in the
standard order.
...
Resolver-operation serialization changes neither the public-plus-security
revision contract nor any database catalog, IPC, provider, readiness, or
runtime publication behavior. Launcher config flows consume these scoped
operations but gain no filesystem or cross-process trust from the mutex.
...
Resolver scoping supplies no bounded wait: one blocked resolver-dependent read
blocks later scoped operations even for unrelated config paths. Nested scope
acquisition is unsupported because the mutex is non-reentrant; any operation
that needs both locks must preserve config mutation before resolver, except that
legacy parsing releases its resolver scope before entering the public save
path. Unscoped secure-string parsing can still observe the installed global
resolver. Cross-process writers, path aliases, unsafe file types, changed
external credential bytes, and a config revision that omits those bytes remain
...
| `FR-SEC-061` | [pkg/config/security_resolver_scope_test.go](../../pkg/config/security_resolver_scope_test.go), [pkg/config/security_resolver_scope_unix_test.go](../../pkg/config/security_resolver_scope_unix_test.go), [pkg/config/config.go](../../pkg/config/config.go), [pkg/config/config_struct.go](../../pkg/config/config_struct.go) |
...
- [pkg/config/security_resolver_scope_test.go](../../pkg/config/security_resolver_scope_test.go)
- [pkg/config/security_resolver_scope_unix_test.go](../../pkg/config/security_resolver_scope_unix_test.go)
