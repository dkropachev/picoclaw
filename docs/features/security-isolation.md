...
heartbeat transfer. The shared queue still has no production owner. PRs `#137`
and `#138` must add read-only outcome reconciliation and the complete publisher
runtime while consuming the journal, observations, requeue, and Git primitive
...
adapter are implemented by `FR-SEC-050`. Generation-owned claiming/renewal and
publisher cleanup, provider wiring in a complete publisher, and unknown-outcome
reconciliation scheduling remain planned for `#137` and `#138`.
...
- The phase dispatcher receives no queue or transition capability. It routes
  ...
  The ready handler completes that internal set, but attaching it to the shared
  production claim source remains forbidden until the distinct generation-owned
  runtime exists.
