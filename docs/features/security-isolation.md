...
| `FR-SEC-008` | MUST | Model-list validation rejects unsupported provider-control values such as invalid `reasoning_effort`, invalid account-router account references, and invalid model-router target references before those values are persisted or used; account routers and model routers are stored in top-level router lists rather than as secret-bearing `model_list[]` entries. | Invalid config should fail early instead of producing unsafe or broken provider requests. |
