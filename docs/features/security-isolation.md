...
alone may additionally expose one coarse
`attention_required` boolean derived from the authoritative current local
review occurrence; attention delivery adds no case-detail field. The open
workbench polls the list every five seconds and uses that marker only for
in-app discovery and canonical chat focus, never as an out-of-band notification
or action capability. Separately, the selected-case detail may contain one
...
current `attention_required` review whose current occurrence is `pending`,
`claimed`, or `delivered`; absent, superseded, no-op, recovery, and failed
occurrences project false. It conveys no private status or identity, case detail
rejects it, and polling or selecting the marker grants no notification,
workflow, code, Git, or provider authority.
...
35. For the uncomposed publication-gate processor, accept only one already
    ...
    the current claim pending a separate safe-requeue contract, and grant no
...
does not yet call it, execute an active publication gate, acknowledge a review,
or merge. Event Automation requirement 069 and `FR-SEC-043` now own
...
The later `#126b2` active-gate/push runtime and safe requeue worker must consume
the journal, context, observations, and Git primitive through separate
capabilities; neither a journal row, context snapshot, observation, processor
result, successful primitive call, nor `local_ready` supplies effect authority.
...
is implemented by `FR-SEC-046`; active mixed-gate execution, safe transient
requeue, provider wiring in a generation-owned worker, and push runtime
composition remain `#126b2`. Acknowledgement and merge remain separately
unimplemented and deliberately undefined.
