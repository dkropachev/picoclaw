...
feedback as plain text, and grants no provider refresh, model, gate,
conversation, workflow, checkout, filesystem, Git, CI, or mutation authority.
...
The own-PR development projection also adds no security or browser-persistent
state. Its opaque cursor is bound to validated filters and retained only for the
active read; the safe DTO is constructed from immutable capture columns, never
from a raw case serialization, and contains no internal identity or authority
that a later action could reuse.
...
push, merge, acknowledgement, or GitHub action authority. Its later read-only
workbench composes event automation's safe runtime projection with launcher
authentication and authority replacement. Security restricts that projection
to captured public facts and plain-text feedback, keeps provenance and browser
persistence outside the boundary, and makes replay visible as another case;
the viewer still acquires none of the omitted development or provider
authority. Any future action must independently establish current local and
provider state rather than inheriting authority from the capture or DTO.
