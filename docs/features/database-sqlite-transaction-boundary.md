...
for transactions executed by `sqlitestore.Immediate`. It installs hooks before
`BEGIN`, rejects callback-controlled transaction termination, and authorizes
exactly one owner commit. Schema, legacy-import, horizon, and domain changes
therefore cannot become durable merely because a trusted callback issued
`COMMIT`, `END`, `ROLLBACK`, or a rollback conflict.
...
| `FR-DATABASE-SQLITE-TRANSACTION-BOUNDARY-004` | MUST | Repository code consumes or extends this boundary. | Only `internal/sqlitestore/open.go` may construct or consume the opaque provider boundary. Only `internal/sqliteprovider/transaction_boundary.go` may register commit/rollback hooks. Repository scans reject direct callback-accessible `Raw`, `Close`, assignment, return, channel-transfer, and goroutine escape forms. | Architecture tests only. | Direct aliases, dot imports, inferred locals, method values, and hook registration fail the guard. Dynamic callbacks and arbitrary wrapping remain inside the explicit trusted-synchronous contract; only #436 removes that capability by construction. | Hook containment depends on singular provider ownership and reviewed synchronous callbacks until the restricted facade replaces this compatibility API. |
...
4. Always call `Check`, even when the callback returned an error, so rollback or
   denied-commit evidence overrides a raw driver-only result.
...
provides the internal provider capability used by that function. The sealed
